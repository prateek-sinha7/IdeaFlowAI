...
F3 lift (D-08 / SKILL-01): the factory's inline flattened skill list (``for skill in
ctx.attached_skills: blocks.append(skill["content"])``) becomes registered
``SkillProvider`` capabilities with a provider INTERFACE + VERSION metadata. The four
