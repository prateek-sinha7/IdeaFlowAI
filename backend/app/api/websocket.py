...
# W4a (44-03, INV-12): the transport-neutral run infrastructure was RELOCATED to
# app.api.run_engine (extract-before-delete). The still-present /ws/chat endpoint +
# WS drivers below re-import the SAME symbols from their new home so behavior is
# byte-identical this wave (44-07 deletes the endpoint). The two auto-resume
# registrars (_register_resume_queue/_register_resume_task) are consumed only by
# app/main.py, so they are NOT re-imported here.
