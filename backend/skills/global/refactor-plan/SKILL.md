...
description: Create a concrete plan before starting a multi-file refactor with safe sequencing, verification steps, and rollback procedures.
...
8. Stop after the plan and ask for confirmation before implementing.
...
### Final Validation
[Command(s) that prove the refactor is complete and correct]
```

## Best Practices

1. **Investigate first**: Read code before planning changes
2. **Safe sequencing**: Types → Implementation → Callers → Tests → Cleanup
3. **Verify each phase**: Don't proceed if previous phase has errors
4. **Small commits**: One commit per phase for easy rollback
5. **Keep tests green**: Never have a broken test suite between phases
