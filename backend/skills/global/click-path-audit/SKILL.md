...
display_name: /click-path-audit — Behavioural Flow Audit
category: debugging
description: Trace every UI button through its full state-change sequence to find handlers that cancel each other out or leave inconsistent state. Use when debugging finds nothing but buttons report broken.
tags:
- click
- path
- audit
