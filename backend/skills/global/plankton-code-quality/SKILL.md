...
display_name: Plankton Code Quality Skill
category: planning
description: Write-time code quality enforcement using Plankton — auto-formatting,
  linting, and AI-powered fixes on every file edit via hooks.
tags:
- plankton
- code
- quality
...
Integration reference for Plankton (credit: @alxfazio), a write-time code quality enforcement system for coding agent harnesses. Plankton runs formatters and linters on every file edit via PostToolUse hooks, then spawns agent subprocesses to fix violations the agent didn't catch.
...
├─ Spawns an agent subprocess with violations JSON
...
# Start your agent session — hooks activate automatically
```

No install command, no plugin config. The hooks in the project's hook-config file are picked up automatically when you run the agent in the Plankton directory.
...
## Pairing with Other Quality Tooling

### Complementary, Not Overlapping

| Concern | Baseline tooling | Plankton |
|---------|-----|----------|
...
| Model routing | Manual (operator selects model) | Automatic (violation complexity → tier) |

### Recommended Combination

1. Set up your baseline agent tooling (agents, skills, commands, rules)
2. Add Plankton hooks for write-time quality enforcement
3. Use AgentShield for security audits
4. Use a verification-loop skill as a final gate before PRs

### Avoiding Hook Conflicts

If running both baseline hooks and Plankton hooks:
- A Prettier hook and Plankton's biome formatter may conflict on JS/TS files
- Resolution: disable the baseline Prettier PostToolUse hook when using Plankton (Plankton's biome is more comprehensive)
- Both can coexist on different file types (the baseline hooks handle what Plankton doesn't cover)

## Configuration Reference

Plankton's hooks `config.json` controls all behavior:
...
## Additional Hook Profile Options
...
```bash
export QUALITY_GATE_HOOK_PROFILE=strict
export QUALITY_GATE_FIX=true
