...
Run via ``./evals/hybrid/eval.sh prompts`` (or ``python3.11 -m evals.hybrid.dump_prompts``
...
slims the framed request), into:
    ...
    specs/006-hybrid-eval-suite/prompt-dumps/
...
OUT_DIR = (
    _BACKEND.parent
    / "specs"
    / "006-hybrid-eval-suite"
    / "prompt-dumps"
)
FIXTURES = (
    Path(__file__).resolve().parent
    / "workflow" / "prototype" / "revision" / "fixtures"
)
