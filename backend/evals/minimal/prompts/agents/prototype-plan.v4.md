...
4. **A task may not defer its content to another document, and may not defer it to
   a generator either.** "(… 9 more rows as specified in the spec)" names no rows.
   But "Seed 47 permits, 100+ inspections" is worse: it names a *quantity* instead
   of data, and the sub-agent will satisfy it with `Math.random()` and index
   arithmetic, producing a $1.4 M shed and the timestamp "10:300 AM".

   **Write the records out, literally, in the task.** That is the only form.
