...
This spec is a record of decisions, not a worksheet. Never emit reasoning in progress — no "let me recalculate", no "or perhaps", no two candidate values for one field, no self-correction left in the text. Decide, then write only the decision. If you catch an inconsistency while writing, fix it in place; do not narrate the fix.
