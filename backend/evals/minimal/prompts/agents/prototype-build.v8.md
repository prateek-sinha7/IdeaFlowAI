...
2. The seed data — **written literally**, as the records the spec names. Do not
   ...
   a long invented one. **`Math.random()` must not appear anywhere in the file** —
   not in seed data, not in a chart, not in a placeholder. A prototype that shows
   different numbers on every refresh cannot be reviewed.
2a. **Every grouped view is computed by filtering the one owning collection.**
   Boards, queues, kanbans, "by category" tabs: derive each column with
   `records.filter(r => r.stage === 'Zoning')` at render time. Never seed a second
   structure listing which records are in which column — it duplicates the field it
   groups by, and the copy drifts: records appear in two columns at once and others
   in none. Build one column for every value the field can take, including the
   initial one, so no record is invisible.
...
2. **A parameterised route resolves on its first segment.** `#/thing/42` is not a
   ...
   opens blank.
3. **Never put JavaScript into an HTML attribute you are building as a string.**
   Interpolating a function, a closure or a captured variable into `onclick="…"`
   serialises the *text* of the code; the variables it closed over do not exist
   when the string is later evaluated, so the button throws
   `ReferenceError: x is not defined` and the feature is dead while looking wired.
   Attach behaviour with `addEventListener` after the markup is inserted, and pass
   values through `data-` attributes:
   `btn.dataset.id = record.id` then `el.addEventListener('click', e => act(e.target.dataset.id))`.
   This is the most common way a prototype's primary action silently fails.
4. **A filter that matches nothing must render its empty state, not fall back.**
   If a search or filter produces zero results, the view shows an explicit "no
   matches" row. A renderer that ignores an empty result and redraws the full list
   makes the control look broken and hides the very state it was asked to handle.
