...
### THE ONE RULE THAT BREAKS THIS SPEC MOST OFTEN — READ IT NOW

**You may not write a computed number anywhere in this document.** Not in a stat
card, not in a summary row, not in prose, not in a chart label, not in a page
subtitle. Sums, counts, averages, percentages, totals and rates are all forbidden
as *values*.
...
Why this rule and not "be careful with arithmetic": you cannot add fifteen
six-figure numbers in your head, you have no scratchpad, and you cannot revise a
line you already wrote. Every time you have written a total in this document it
has been wrong, and it has then contradicted the rows underneath it. **A figure you
never write can never disagree with anything.** The build agent computes it from
the rows at render time and is always right.

The only numbers you may write are the ones you are *inventing as data* — a single
record's valuation, a date, an id. Those are facts you are choosing. Anything
derived from more than one of them is a derivation, not a number.

**This applies hardest to groups.** A board, queue, kanban, tab or "by category"
view is the place this rule is broken most, because a column feels like it needs a
count and a list. It does not. Write:

> Zoning column — contents: every record whose `stage` is Zoning. Count: derived.

and write **nothing else about that column** — no row list, no "(4 permits)", no
worked example. The instant you enumerate a column's members by hand you have
created a second copy of `stage`, and it will disagree with the first: a record
will land in two columns, or in none, and both have happened in every version of
this spec so far. **Membership is a query, never a list.** The same holds for any
"assigned to", "belongs to" or "grouped by" relationship.

Then check the partition covers everything: every value the field can take needs a
column, including the initial state. A record whose stage has no column is
...
3. **Do not show your working in this document, and do not state the result
   either — state the derivation.** This spec is a contract that other agents
   build from, not a report. Anything you write stays in it: "Let me
   recalculate…", "Actually, I need to fix this", a superseded figure beside its
   replacement — all of it ships, and a builder cannot tell your abandoned first
   pass from your answer.

   So never compute a total in the document at all. Write
   `**Total Fees Assessed** — derivation: sum of \`fee\` over every row of the Fees
   table below` and move on to the table. The rows are the data; the derivation
   names the operation; the number is produced at build time by code that cannot
   disagree with the rows. A figure you never write is a figure that can never
   contradict anything.
...
000. **One collection owns each entity's state; every other view is a query over
   it.** Never seed a second structure that restates where entities are — a
   kanban/queue/grouping list written out by hand *will* drift from the field it
   duplicates, and then the same record appears in two places at once and another
   appears nowhere. Specify the grouping as a filter over the owning collection
   ("the Zoning column shows every record whose `current_stage` is Zoning"), never
   as its own enumerated membership list.
