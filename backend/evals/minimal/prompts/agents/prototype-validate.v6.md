...
6. **Handlers built as strings.** Search for `onclick="` and any attribute whose
   value is assembled by interpolation. Code serialised into an attribute loses
   every variable it closed over and throws `ReferenceError` when clicked. Report
   each one — this is how a prototype's primary action dies while appearing wired.
7. **Empty-result paths.** For each filter or search, trace what renders when it
   matches nothing. A renderer that redraws the full list on an empty result is
...
file will confirm bugs as working: the check is what you *observed*, and if you
did not open that region of the file, the line is `UNVERIFIED`, never `OK`.
`UNVERIFIED` counts as `BROKEN` — go and read it.
...
**and each of those `OK`s carries the quoted evidence above.** An all-clear audit
followed by a two-line diff is the failure mode of this stage, not its success
case — if you concluded everything was fine, you probably checked intent rather
than code. Go back to the file.
...
4. **Nothing you insert may run before what it depends on.** Adding a call —
   `renderCurrentPage()`, an init, a listener registration — at a point earlier in
   the script than the object or function it uses is a `ReferenceError` at load,
   and a `ReferenceError` at load kills the whole script exactly like a
   SyntaxError: every page then renders blank. Before inserting any *call*, find
   the line where the thing it calls is defined and confirm your insertion point is
   after it. Calls that must run at startup belong at the very end of the script,
   never in the middle.
5. **Prove the specific thing you changed now works.** For each edit, state in one
   ...
   ("Fees search: `filterFees` now reads `#feeSearch`; typing a permit number
   narrows the table"). **An edit whose effect you cannot state is an edit you
   should not make** — you are the last agent to touch this file, and a repair that
   introduces a new failure is strictly worse than the defect it replaced.
