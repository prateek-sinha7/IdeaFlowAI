#!/usr/bin/env bash
# aws-judge-1.sh — rejudge run 1 (fraud_review) with the bedrock judge.
#
#   ./aws-judge-1.sh
#
...
# 260803-155235-prototype_aws_1_fraud_review
# and are NOT re-dispatched. Its twin is mistral-judge-1.sh,
...
exec "$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)/rejudge.sh" \
  260803-155235-prototype_aws_1_fraud_review-awsjudge \
  bedrock "$@"
