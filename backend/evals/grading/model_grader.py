...
async def run_workflow(
    ...
    dataset_run_id = run_config.from_run or artifacts.new_dataset_run_id(dataset["dataset_id"])
    ...
    for stage in selected:
        ...
        upstreams = _resolve_upstreams(stage, run_dir=run_dir, produced=produced, from_run=run_config.from_run)
...
async def run_stage(
    ...
    *,
    ...
    if dry_run:
        inputs, note = _planned_inputs(stage, dataset, upstream_outputs, workflow_dir)
        return _stage_plan(stage, inputs, ids, run_config, dataset, rubric, note=note)
    inputs = stage_input.build_stage_inputs(
        stage, dataset=dataset, upstream_outputs=upstream_outputs, config_dir=workflow_dir
    )

    artifacts.guard_append_only(run_dir, ids["agent_token"], replace=replace)
    ...
    output = _stage_output(runs, dataset, ids, agent_id)
...
def _stage_output(runs: list[dict], dataset: dict, ids: dict, agent_id: str) -> dict:
    ...
    rows = []
    for run in runs:
        if run["errored"] or run["skip_reason"] or run["expect"] == "fail":
            continue
        rows.append(
            {
                "id": run["row_id"],
...
def _planned_inputs(stage: dict, dataset: dict, upstream_outputs, workflow_dir) -> tuple[list, str | None]:
