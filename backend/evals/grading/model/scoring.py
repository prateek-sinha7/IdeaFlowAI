...
def summarize_stage(runs: list[dict], grades: list[dict], rubric: dict) -> dict:
    ...
    summary = {
        ...
        "recurring_weaknesses": _cluster_texts(rows, "weaknesses"),
        "recurring_strengths": _cluster_texts(rows, "strengths"),
...
def _cluster_texts(rows: list[dict], key: str) -> list[dict]:
    """Cluster one judge narrative list across rows, ranked by row frequency.

    The same clustering serves both weaknesses (what to fix) and strengths
    (what a prompt edit must not lose) — two lists that would otherwise drift
    apart in threshold and shape.
    """
    clusters: list[dict] = []
    for row in rows:
        for text in row.get(key) or []:
            ...
            if cluster is None:
                clusters.append(
                    {"tokens": tokens, "row_ids": [row["row_id"]], "evidence": text}
                )
...
def _warnings(
    ...
    # `tokens_in` is the proof a dispatch happened: every real one bills input
    # tokens (the smallest observed here is 5,830), so a row with zero IN and zero
    # OUT was never dispatched in this folder at all — its response was seeded from
    # a committed fixture, or replayed. Nothing is under-reported there, and since
    # `rejudge` never dispatches, those zeros can never be filled in: keying only on
    # `tokens_out` made this fire on every stage of every rejudge of a seeded run,
    # calling rows "dispatched" that never were. Warn only for the real case —
    # input billed, output uncounted, which is what tool-using agents actually do.
    starved = [
        row["row_id"]
        for row in rows
        if not row["errored"]
        and not row["skip_reason"]
        and row["tokens_in"]
        and not row["tokens_out"]
    ]
    if starved:
        warnings.append(
...
def _threshold_failures(summary: dict, thresholds: dict) -> list[str]:
    ...
    # SAMPLE-SIZE GATED: two rows that legitimately tie are not a dead judge,
    # so the check arms only when the run judged at least twice the required
    # distinct count. An 11-row constant is still caught; a 1-2 row debug run
    # no longer fails its baseline over arithmetic that cannot be satisfied.
    judged = (summary.get("counts") or {}).get("judged") or 0
    if (
        minimum is not None and distinct is not None
        and judged >= 2 * minimum and distinct < minimum
    ):
        failures.append(
            f"distinct_score_count {distinct} < {minimum} over {judged} judged rows "
            "(the judge is returning too few distinct values to detect a regression)"
        )
