...
from evals.grading import hooks  # shared plumbing stays one level up
...
def build_stage_inputs(
    ...
    *,
    ...
    propagate_expected_fail: bool = False,
) -> list[StageInput]:
    ...
    propagate downstream — UNLESS `propagate_expected_fail` is set, the opt-in
    that lets you watch what the whole chain does with a bad brief: the row then
    flows through every stage (its own upstream precheck failure is by design,
    so the failure policy is overridden to run_anyway for it), but it is still
    never judged. Every skipped row carries `upstream_chain` so a later score
    ...
    for row in dataset.get("rows") or []:
        negative = str(row.get("expect", "pass")) == "fail"
        # A propagated negative row's upstream "failure" is the eval working;
        # holding it to the failure policy would re-strand it one stage later.
        row_policy = "run_anyway" if (propagate_expected_fail and negative) else policy
        texts, chain, upstream_reason = _gather_upstreams(
            str(row.get("id", "")), needed_agents, indexes, row_policy
        )
        ...
        expect_reason = (
            None if propagate_expected_fail else _expect_reason(row, is_root=is_root)
        )
        reason = expect_reason or upstream_reason or adapter_reason
        stage_inputs.append(
            StageInput(
