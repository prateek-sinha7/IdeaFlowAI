...
Run via ``./run-eval.sh <scenario-id>`` WITHOUT ``--live`` (that's the whole
point — this is what runs by default so a bare scenario name never
accidentally reaches a real model), or directly:
``python3.11 -m tests.evals.validate_scenario prototype_multi_issue_repair``
from ``backend/``.
...
def main() -> None:
    ...
    from tests.evals.common.scenario_discovery import discover_all_scenarios
    ...
    print(f"      ./run-eval.sh {scenario_id} --live")
