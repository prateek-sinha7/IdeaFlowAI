...
class TestRevisionDisposal:
    ...
    async def test_confirmed_revision_mints_and_drives(self, monkeypatch):
        ...
        def _fake_mint(db, *, user, parent_run_id, target_artifact_type, instruction,
                       **_extra):  # pipeline_run_id / extra_agent_count (run_commands.py:1516)
        ...
        # The Concierge revision path now runs the revision ANALYZER before minting
        # (run_commands.py:1464), which builds a real ChatBedrockConverse — the
        # ISS-102 conftest guard fails the test rather than let it bill a provider.
        # These cases assert the mint/drive wiring, so stub the analyzer at its
        # construction seam (guard option (a)).
        async def _fake_analyzer(**_k):
            return "small", "analyzer solution"

        # Imported INSIDE the function (run_commands.py:1439), so the SOURCE
        # module is the only seam that binds.
        import app.agents.revision_analyzer as _ra
        monkeypatch.setattr(_ra, "run_analyzer", _fake_analyzer)
        ...
        # The child id is PRE-generated by the caller now (run_commands.py:1459 —
        # "so run_analyzer events land on the correct queue before the DB row is
        # minted"), so it is a fresh uuid, not `_mint_revision_row`'s return value.
        _uuid_mod.UUID(res["revision_run_id"])  # raises if it is not a real uuid
...
class _FakeDb:
    """Enough of a Session for `_dispose_concierge_proposal`.

    The Concierge revision path sources the parent's existing HTML from the
    `WorkflowRun.output` column (run_commands.py:1447) before minting the child,
    so a `close`-only double now raises AttributeError before the seam under test
    is ever reached. `.first()` returning None is the honest shape here: these
    cases assert the mint/drive wiring, not the html carry-over.
    """
...
class TestConciergeEndpoint:
    ...
    def test_confirm_round_trip_executes_revision_seam(self, env, monkeypatch):
        ...
        # The revision path runs the ANALYZER before minting (run_commands.py:1464),
        # which builds a real ChatBedrockConverse — the ISS-102 guard fails the test
        # rather than let it bill a provider. This case asserts the confirm ROUND
        # TRIP, not the analyzer, so stub it at its source module (the import is
        # function-local, so patching `rc` would not bind).
        async def _fake_analyzer(**_k):
            return "small", "analyzer solution"
