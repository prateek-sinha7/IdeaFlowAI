...
Pure stdlib, cross-platform. Extracts the site heredoc out of ``infra/scripts/bootstrap-ec2.sh``
and asserts the SSE location exists, is NOT rate limited, and that the general ``/api/`` 
limiter survives. Non-vacuous: every assertion fails on the pre-fix tree.
"""
...
def _render_site() -> str:
    """Extract the `sites-available/velocityai` heredoc from bootstrap-ec2.sh.
...
def _render_limits() -> str:
    """Extract the `velocityai-limits.conf` heredoc from bootstrap-ec2.sh."""
    text = _bootstrap_text()
    lines = text.splitlines()
    start = next(
        i for i, l in enumerate(lines)
