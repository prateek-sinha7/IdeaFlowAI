"""Characterization: `ppt` deliverable byte-snapshot (SAFE-01 / INV-3).

The 003 characterization baseline for the `ppt` (OpenDesign PPT deck) pipeline.
Its three agents (ppt-brief-analyst / ppt-composer / ppt-validator) are
...
The ppt agents declare ``injects=[template, design_system]``; ``_drive`` seeds an
``od_context`` for ppt so ``_compose_injection`` does not raise
...
@pytest.mark.asyncio
async def test_ppt_deliverable_byte_snapshot() -> None:
    events = await _drive("ppt")
    assert events, "ppt produced no events"

    deliverable = extract_final_output(events).encode("utf-8")
    assert_deliverable_snapshot("ppt.html", deliverable)


@pytest.mark.asyncio
async def test_ppt_event_snapshot() -> None:
    """Lock the normalized semantic event stream for ``ppt`` (SAFE-02 / SAFE-03)."""
    events = await _drive("ppt")
    assert events, "ppt produced no events"

    seen = {e.get("type") for e in events}
    unknown = seen - _DOCUMENTED_EVENT_TYPES
    assert not unknown, f"ppt emitted UNDOCUMENTED event type(s): {sorted(unknown)}"
    ...
    for ev in events:
        ...
        assert not missing, (
            f"ppt event '{ev.get('type')}' is MISSING required data keys "
            f"{sorted(missing)}"
        )
    ...
    if SNAPSHOT_UPDATE:
        write_events_golden("ppt.events.json", normalized)
        return

    golden = load_events_golden("ppt.events.json")
    assert golden is not None, (
        "ppt.events.json golden missing — run once with SNAPSHOT_UPDATE=1 and "
        "COMMIT golden/ppt.events.json."
    )
    assert normalized == golden, (
        "ppt normalized event stream diverged from the committed golden "
