...
# scripts/deploy.sh
#
# End-to-end VelocityAI deploy. Takes a fresh AWS account from "nothing" to
# "https://<fqdn>/health responding 200" in one invocation. Idempotent —
# re-running redeploys (rebuilds images + restarts velocityai-app.service).
#
...
#   VELOCITYAI_ALERT_EMAIL — SNS subscription target for alarms
#
# Optional env overrides:
#   VELOCITYAI_ENVIRONMENT      default: prod
#   VELOCITYAI_OWNER            default: parsed from SSO ARN, fallback "velocityai"
#   VELOCITYAI_COST_CENTER      default: velocityai
#   VELOCITYAI_USE_NIP_IO       default: true
#   VELOCITYAI_ROUTE53_ZONE     required when VELOCITYAI_USE_NIP_IO=false
#   VELOCITYAI_APP_SUBDOMAIN    default: velocityai (only used with route53)
#   VELOCITYAI_AUTO_APPROVE     skip the confirmation prompt (set to 1)
#
# Skip flags (resume a partial run):
#   VELOCITYAI_SKIP_BOOTSTRAP_TF=1     skip the bootstrap-stack apply
#   VELOCITYAI_SKIP_PROD_TF=1          skip the prod-stack apply
#   VELOCITYAI_SKIP_IMAGES=1           skip docker build + push
#   VELOCITYAI_SKIP_BOOTSTRAP_SSM=1    skip the EC2 SSM RunCommand
#
...
# whitespace or weirder than `[A-Za-z0-9_.@-]` falls back to "velocityai" rather
...
if [[ -z "$DETECTED_OWNER" || ! "$DETECTED_OWNER" =~ ^[A-Za-z0-9_.@+=-]+$ ]]; then
    DETECTED_OWNER="velocityai"
fi

ENVIRONMENT="${VELOCITYAI_ENVIRONMENT:-prod}"
OWNER="${VELOCITYAI_OWNER:-$DETECTED_OWNER}"
COST_CENTER="${VELOCITYAI_COST_CENTER:-velocityai}"
STATE_BUCKET="velocityai-tfstate-${ACCOUNT_ID}-${REGION}"
BACKUP_BUCKET="velocityai-${ENVIRONMENT}-pg-dumps-${ACCOUNT_ID}"
...
LOCK_TABLE="velocityai-tfstate-locks"
USE_NIP_IO="${VELOCITYAI_USE_NIP_IO:-true}"

if [[ -z "${VELOCITYAI_ALERT_EMAIL:-}" ]]; then
    if [[ -t 0 ]]; then
        read -rp "Alert email (where SNS sends paging notifications): " VELOCITYAI_ALERT_EMAIL
    fi
    [[ -n "${VELOCITYAI_ALERT_EMAIL:-}" ]] \
        || die "VELOCITYAI_ALERT_EMAIL is required (set it as an env var or run interactively)."
fi
...
cat <<EOF
  ...
  alert email:   $VELOCITYAI_ALERT_EMAIL
  DNS:           $([[ "$USE_NIP_IO" == "true" ]] && echo "nip.io (zero DNS config)" || echo "Route 53 (zone=$VELOCITYAI_ROUTE53_ZONE)")
EOF
bold "──────────────────────────────────────────────"

if [[ "$USE_NIP_IO" == "false" ]]; then
    [[ -n "${VELOCITYAI_ROUTE53_ZONE:-}" ]] \
        || die "VELOCITYAI_ROUTE53_ZONE is required when VELOCITYAI_USE_NIP_IO=false"
fi

if [[ "${VELOCITYAI_AUTO_APPROVE:-}" != "1" ]]; then
    if [[ -t 0 ]]; then
        read -rp "Proceed? [y/N] " CONFIRM
        [[ "$CONFIRM" =~ ^[Yy]$ ]] || { yellow "Aborted."; exit 1; }
    else
        die "Refusing to apply non-interactively without VELOCITYAI_AUTO_APPROVE=1"
    fi
fi

# ── 3. Bootstrap stack: state bucket + lock + CMK ─────────────────────
if [[ -z "${VELOCITYAI_SKIP_BOOTSTRAP_TF:-}" ]]; then
    ...
    yellow "==> [1/5] SKIPPED (VELOCITYAI_SKIP_BOOTSTRAP_TF=1)"
fi

# ── 4. Prod stack: VPC, EC2, ECR, KMS, ... ────────────────────────────
if [[ -z "${VELOCITYAI_SKIP_PROD_TF:-}" ]]; then
    ...
    PROD_TF_VARS=(
        ...
        -var "owner=$OWNER"
        -var "cost_center=$COST_CENTER"
        -var "alert_email=$VELOCITYAI_ALERT_EMAIL"
    ...
    if [[ "$USE_NIP_IO" == "false" ]]; then
        PROD_TF_VARS+=(
            -var "route53_zone_name=$VELOCITYAI_ROUTE53_ZONE"
            -var "app_subdomain=${VELOCITYAI_APP_SUBDOMAIN:-velocityai}"
        )
    fi
    ...
    yellow "==> [2/5] SKIPPED (VELOCITYAI_SKIP_PROD_TF=1)"
fi
...
tf_out() {
    ...
    val=$(terraform -chdir=infra/envs/prod output -raw "$1" 2>/dev/null) \
        || die "terraform output '$1' is missing. Did the prod apply succeed? (If you set VELOCITYAI_SKIP_PROD_TF=1, the state must already contain this output.)"
...
# can override via VELOCITYAI_IMAGE_TAG (mainly for VELOCITYAI_SKIP_IMAGES=1 mode,
...
TAG="${VELOCITYAI_IMAGE_TAG:-$TAG}"

# ── 5. Build + push initial images ────────────────────────────────────
if [[ -z "${VELOCITYAI_SKIP_IMAGES:-}" ]]; then
    ...
    # exec() fails with "exec format error" — velocityai-app.service then loops
    ...
    yellow "==> [3/5] SKIPPED (VELOCITYAI_SKIP_IMAGES=1)"
    # The box will try `docker compose pull` on this tag. If it doesn't
    # exist in ECR yet, velocityai-app.service crash-loops with manifest-unknown
    # and the only signal is buried in `journalctl -u velocityai-app`. Fail fast
    ...
    for repo in "$BACKEND_REPO" "$FRONTEND_REPO"; do
        ...
        if ! aws ecr describe-images \
            ...
            die "VELOCITYAI_SKIP_IMAGES=1 set, but $repo_name:$TAG isn't in ECR. Either unset VELOCITYAI_SKIP_IMAGES, or set VELOCITYAI_IMAGE_TAG to a tag that's been pushed."
        fi
    done
...
if [[ -z "${VELOCITYAI_SKIP_BOOTSTRAP_SSM:-}" ]]; then
    bold "==> [4/5] EC2 bootstrap via SSM RunCommand (~6 min)"

    # `terraform apply` returns when the instance is `running`, NOT when
    # cloud-init has finished writing /etc/velocityai/bootstrap.env or when the
    ...
    # us under. The terminator __VELOCITYAI_BOOTSTRAP_EOF__ is unique vs. any
    ...
    #
    # VELOCITYAI_IMAGE_TAG is set via `export` inside the bash heredoc so
    # bootstrap-ec2.sh's IMAGE_TAG="${VELOCITYAI_IMAGE_TAG:-latest}" picks up
    # the just-pushed git-sha tag.
    EOF_TAG="__VELOCITYAI_BOOTSTRAP_EOF__"
    SCRIPT_BODY="$(cat infra/scripts/bootstrap-ec2.sh)"
    # VELOCITYAI_ACME_EMAIL is exported here as a belt-and-suspenders for instances
    # whose /etc/velocityai/bootstrap.env was written before TF started populating
    # the key (modules/compute/user_data.sh.tpl writes it via user_data_extra_env
    # going forward). bootstrap-ec2.sh's `${VELOCITYAI_ACME_EMAIL:-fallback}` keeps
...
export VELOCITYAI_IMAGE_TAG=$(printf %q "$TAG")
export VELOCITYAI_ACME_EMAIL=$(printf %q "$VELOCITYAI_ALERT_EMAIL")
${SCRIPT_BODY}
${EOF_TAG}"
    ...
    yellow "==> [4/5] SKIPPED (VELOCITYAI_SKIP_BOOTSTRAP_SSM=1)"
fi
...
#   - velocityai-app.service ExecStartPre runs `docker compose pull`. Cold
...
for attempt in $(seq 1 10); do
    if curl -fsS --max-time 10 "https://$FQDN/health" >/dev/null 2>&1; then
        green ""
        green "  VelocityAI is live at https://$FQDN"
...
red "Likely causes:"
red "  - velocityai-app.service still pulling images (slow ECR / cold cache)"
red "  - cert not yet issued (DNS propagation if NOT using nip.io)"
red ""
red "Debug:"
red "  aws ssm start-session --region $REGION --target $INSTANCE_ID"
red "  # then on the box:"
red "  sudo systemctl status velocityai-app.service"
red "  sudo docker compose -f /opt/velocityai/docker-compose.yml logs --tail=200"
exit 1
