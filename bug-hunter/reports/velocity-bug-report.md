# Velocity — Autonomous Bug Hunt Report

**92 reproducible defects** across 55 pages · rounds 1-2 complete · 2026-08-28 10:56 UTC
...
| # | Severity | Route | Title |
|---|---|---|---|
| 1 | High | `/login?expired=true` | An already-authenticated user landing on /login is shown the sign-in form instead of being redirected to /dashboard |
| 2 | High | `/create` | "Jump back in" recent-runs cache is not scoped per user, leaking a previous account's run history into a different signed-in user's session |
| 3 | High | `/create/ppt` | Selecting the "Investment Banking Pitch Book" template introduces an unsatisfiable "Pick a design system" requirement that permanently blocks Continue |
| 4 | High | `/create/prototype` | Browser Forward navigation to the prototype wizard leaves a permanently blank page |
| 5 | High | `/workflows/ppt/canvas` | "Save as copy" of the PPT built-in silently drops the manifest, converting the copy from a PPTX-generating workflow into a generic streamed-text/markdown workflow |
| 6 | High | `/runs` | Run History silently caps at 50 runs and mislabels it as the full total, with no pagination path to the remaining 223 |
| 7 | High | `/runs/b9feac1c-ec21-4531-8ba7-bb391786993e/f` | Files tab never lists the run's real PPTX binary deliverable; instead shows a scratch tmp/ HTML file mislabeled as "Final output" |
| 8 | High | `/runs/b9feac1c-ec21-4531-8ba7-bb391786993e/a` | The Audit tab fabricates 96 "Governance/Gate" records from ordinary pipeline events; the backend's real gate-events endpoint reports zero for the same run |
| 9 | High | `/runs/{id}` | "Edit brief & run again" on a failed run opens a brand-new, completely blank composer instead of pre-filling the original brief |
| 10 | High | `/runs/{id} → /runs/{id}/stream` | "Run Again" on a cancelled run leaves the live /stream view permanently stuck at "Running · 0/4 agents · Starting…" after the backend has already failed the pipeline |
| 11 | High | `/runs/{id}` | "Start a new run" on a diverted run's detail page calls the resume endpoint, which the backend correctly rejects, with no visible error shown to the user |
| 12 | High | `/library?tab=hooks` | Hook cards on the Hooks tab are not keyboard-operable |
| 13 | High | `/handoff/settings` | An over-length GitHub PAT is echoed back verbatim in the 422 response body and rendered raw on-page, contradicting "never returned by any API" |
| 14 | High | `/create/ex_A4_human_divert` | "Save as my version" on the seeded "Ask a Human, Then Hand Off" example fails with a raw 422 backend manifest-validation error |
| 15 | High | `/create/ppt` | "Save as my version" on `/create/ppt` silently discards the entered brief and selected template, always saving the same static generic record |
| 16 | High | `/workflow/create?mode=../admin` | A path-traversal `mode` value escapes the `/create/<mode>` namespace and navigates to an unrelated real route, including `/admin` |
| 17 | High | `/workflows/{id}/run` | A saved workflow's launch panel loses its override entirely and renders the generic base-type wizard, even though the override fetch succeeds |
| 18 | High | `/analytics` | Pipeline filter narrows the breakdown list but leaves every KPI tile, chart, and Success Rate on unfiltered all-pipelines data |
| 19 | High | `/runs/77f74563-fa19-4f0b-84fd-d0224f89a54a/v` | Switching tabs while pinned to a non-root version silently drops the pin: URL, API calls, and displayed data all revert to v1 |
| 20 | High | `/library/skills/<skillId>` | Skill detail route (`/library/skills/<id>`) now falls back to the Library listing for EVERY skill id, hyphenated or not — a full regression beyond the previously filed no-hyphen-only scope |
| 21 | High | `/workflows/<nonexistent-id>/canvas` | "Run once" on the dead-link `/canvas` composer launches a real 9-agent pipeline run, ignoring the visibly-empty 0-agent workflow shown on screen |
| 22 | Medium | `/library, /library?tab=skills, /library?tab=` | Library search with no matches shows a blank page, no empty state |
| 23 | Medium | `/settings/profile` | Password-mismatch error does not clear when the fields are corrected to match |
| 24 | Medium | `/login` | Sign in button is not disabled during the login request, allowing duplicate submissions |
| 25 | Medium | `/register/anything, /register/abc123xyz` | For an authenticated user, unmatched `/register/<sub-path>` URLs silently render the Dashboard instead of a 404 or a redirect |
| 26 | Medium | `/` | Browser Back to `/` after the corrupt-token auth bounce leaves the app permanently blank instead of redirecting to `/login` |
| 27 | Medium | `/create/app` | Checking an agent in the composer's "Review gates" checklist does not set that agent's Gate in the Advanced modal's per-agent Config panel |
| 28 | Medium | `/create/user-stories` | The Advanced modal's Workflow-level settings (Smart planning, Confirm requirements first, Deliverable strategy, Internet access) are permanently disabled with no explanation |
| 29 | Medium | `/create/ex_A2_branch` | Advanced modal's "Cancel" button does not discard edits to a conditional-gate outcome's route |
| 30 | Medium | `/workflow/create` | `/workflow/create` never redirects when `mode` is absent or empty, leaving the legacy URL live and uncanonicalized |
| 31 | Medium | `/workflow/create?mode=nonsense` | Invalid `mode` value redirects into a permanently dead-end 0-agent composer with the underlying 404 silently swallowed |
| 32 | Medium | `/workflows/new` | Leaving the new-workflow composer via its own "Back" control silently discards all unsaved work with no confirmation |
| 33 | Medium | `/workflows/656ca387-e69c-474d-b7ff-5fd9eb017` | Simple view's per-agent "Configure →" panel has no Tools section at all, hiding an existing tool-grant override (e.g. Write files disabled) with no visibility or edit path |
| 34 | Medium | `/workflows/<nonexistent-uuid>/run, /workflow` | Loading a saved-workflow launch panel with a nonexistent or malformed workflow id silently renders the Dashboard under the unchanged bogus URL |
| 35 | Medium | `/runs/b9feac1c-ec21-4531-8ba7-bb391786993e` | Run detail header's relative timestamp is stuck on "just now" for a run that finished 11+ hours ago, disagreeing with the version picker's own age label |
| 36 | Medium | `/runs/b9feac1c-ec21-4531-8ba7-bb391786993e/s` | Steps tab footer's "input" token figure is ~20x too low and its own breakdown does not sum to the stated total |
| 37 | Medium | `/runs/b9feac1c-ec21-4531-8ba7-bb391786993e/s` | Deep-linking a specific agent's step URL never opens that agent's detail pane; it renders the plain unfiltered lane list instead |
| 38 | Medium | `/runs/b9feac1c-ec21-4531-8ba7-bb391786993e/w` | Workspace tab's "Code" file viewer is a live, freely-editable text editor with no read-only indication for a completed run |
| 39 | Medium | `/runs/{id}/preview/full` | For a failed run with no Preview tab, `/preview/full` silently renders the Audit trail instead of an honest "no deliverable" state |
| 40 | Medium | `/runs/{id}/stream` | Diverted run's Preview tab shows the "waiting for live output" placeholder even though the run is permanently terminal |
| 41 | Medium | `/runs/77f74563-fa19-4f0b-84fd-d0224f89a54a/v` | Version picker label is stuck on "Version v1" while viewing a real, distinct v2 |
| 42 | Medium | `/library/agents/<agentId>` | Library agent detail's "Save agent" Config override silently discards the change with no API call |
| 43 | Medium | `/library/skills/<skillId>` | Skill IDs with no hyphen (single-word slug) silently fall back to the Library listing instead of rendering the skill detail page |
| 44 | Medium | `/library/hooks/<hookId>` | Hook detail's "Compatible agents" list references nonexistent agent ids not found in the Agents library |
| 45 | Medium | `/settings/ai-model` | Pipeline model selector enforces no tier restriction; a basic-tier account can select and persist the most expensive "powerful" models |
| 46 | Medium | `/settings/usage` | "Manage plan" button on Usage & Limits has no click handler at all; a basic-tier account has no way to upgrade |
| 47 | Medium | `/settings/constitution` | Constitution's "X / 4000 chars" limit is display-only; content well over 4000 chars saves and persists with no truncation or rejection |
| 48 | Medium | `/settings/security` | Security tab tells break-glass admin their credentials are "managed outside the application" when this app manages them itself |
| 49 | Medium | `/admin` | Create-User dialog accepts and persists a malformed email with no @ or domain |
| 50 | Medium | `/workflows/<nonexistent-or-malformed-id>/can` | `/workflows/<bad-id>/canvas` silently opens an empty "copy" composer instead of 404ing, letting a user save a brand-new persisted workflow from a dead link |
| 51 | Medium | `/preview-fullscreen?error=quota` | `?error=quota` always shows the "too large" dead-end even when a fully valid preview is sitting in sessionStorage |
| 52 | Medium | `/handoff/{token}` | A genuine network/fetch failure on the handoff page is misreported as "Handoff not found", even for a real, valid, owned token |
| 53 | Medium | `/library/agents/{id}` | "Coming Soon" agents are fully browsable and configurable via direct URL, bypassing the catalog's own disabled/unreachable gating |
| 54 | Medium | `/runs/a0693fcd-9451-4c19-a9e7-61474ea03516` | Run detail chat header shows a token total that does not match the run's own API `token_usage`, disagreeing with the correct figure shown on the Run History list card for the same run |
| 55 | Medium | `/workflows` | Delete-workflow confirmation dialog never identifies which workflow it will delete |
| 56 | Medium | `/create/app` | Oversized text attachment (.txt/.md/.json/.csv) is silently truncated with zero user-visible indication, unlike the equivalent PDF/DOCX/PPTX path |
| 57 | Medium | `/create/user-stories` | The "Add agent" library's own "User Stories" category tab always shows "No agents found", even for a bare text search |
| 58 | Medium | `/create/ex_A4_human_gate` | A step's seeded "before-human" gate is invisible in both the Gate combobox and the Review-gates checklist on fresh load |
| 59 | Medium | `/workflows/new` | "Save workflow" silently no-ops when the name field is empty, with zero visible feedback and no API request fired |
| 60 | Medium | `/runs/{id}/preview/full` | Preview toolbar's "Full Screen" button renders entirely outside a narrow (mobile) viewport with no scroll affordance to reach it |
| 61 | Medium | `/runs/{id}/stream` | Completed run's "Download the deliverable" toolbar button stays permanently disabled when the backend never populates `deliverable_filename`, even though the deliverable is fully rendered |
| 62 | Medium | `/runs/d6e425b6-df0d-4f54-9bba-f4a771e33812/s` | Steps panel keeps showing "Run failed" while a reopened run is genuinely running |
| 63 | Medium | `/runs/{id}, /runs` | A run whose "Run Again" pipeline actually failed keeps reporting `status: "cancelled"` everywhere, permanently misrepresenting its real outcome |
| 64 | Medium | `/library/agents/<agentId>` | Library agent detail's Skills tab "Add" gives success feedback (checkmark + badge count) but persists nothing — silently discarded on reload, no Save affordance exists at all |
| 65 | Medium | `/settings/ai-model` | Concurrent `PUT /api/settings/preferences` calls silently lose one write and return a false-success response with the WRONG persisted value |
| 66 | Medium | `/create/prototype` | /create/<type> composer has no tier gating at all; a basic-tier account reaches a fully configured, submit-ready wizard for a deliverable its own Usage & Limits page says it cannot run |
| 67 | Medium | `/settings/constitution` | "Clear" on Constitution deletes the saved value immediately, with no confirmation and no relation to "Save" |
| 68 | Medium | `/login?expired=true` | Mid-session token expiry drops the original protected route; re-login always lands on /dashboard instead of using the working `?redirect=` mechanism |
| 69 | Medium | `/admin` | Create-User dialog silently swallows a duplicate-email 409, giving the admin zero feedback that submission failed |
| 70 | Medium | `/preview-fullscreen` | File explorer/download parses a markdown subheading in the deliverable as a phantom "file" with no real name or extension |
| 71 | Medium | `/runs/b9feac1c-ec21-4531-8ba7-bb391786993e/a` | Audit tab's CSV and JSON export strip the Security/Governance category distinction and never populate the promised "severity" column |
| 72 | Low | `/dashboard` | Escape does not close a catalog card's "Inspect" details modal |
| 73 | Low | `/workflows` | Escape does not close a saved-workflow card's "Workflow actions" dropdown menu |
| 74 | Low | `/workflows/{id}` | Saved-workflow detail view lists agents by raw internal slug instead of the friendly display name used everywhere else in the app |
| 75 | Low | `/workflow` | "Run Workflow" enables with zero agents attached and silently no-ops on click |
| 76 | Low | `/library?tab=skills` | Skill with an empty `category` field renders a blank category label and is unreachable through any category pill |
| 77 | Low | `/library/hooks/<hook-id>` | Hook detail page's "Copy" button gives zero feedback that a copy occurred |
| 78 | Low | `/preview-fullscreen` | File explorer search leaves orphaned empty folders visible and the footer file count never reflects the active filter |
| 79 | Low | `/login?expired=1` | Failed-login error message renders with no ARIA live region or alert role, so screen readers never announce it |
| 80 | Low | `/settings` | Bare `/settings` redirects correctly on a cold/full load but renders the honest 404 page when reached via client-side (SPA) navigation |
| 81 | Low | `/settings/profile` | Confirm new password field has no show/hide toggle, unlike its sibling fields |
| 82 | Low | `/create/prototype` | "Upload custom template → From URL" leaks a raw Python errno/socket message to the user on an unreachable domain |
| 83 | Low | `/workflows/ppt/canvas` | Simple and Canvas views disagree on the PPT built-in's deliverable type ("Custom" vs "Streamed text") |
| 84 | Low | `/workflow` | Removing a file attachment leaves a stale "[Attached: ...]" marker in the brief textarea, keeping Run Workflow enabled for a phantom attachment |
| 85 | Low | `/runs/b9feac1c-ec21-4531-8ba7-bb391786993e/s` | Tool-call summary row renders "[object Object]" instead of a readable preview for array/object arguments |
| 86 | Low | `/runs/b9feac1c-ec21-4531-8ba7-bb391786993e/f` | Files tab file sizes are character counts, not byte counts, understating every non-ASCII file |
| 87 | Low | `/runs/{id}/workspace` | Diverted run's Workspace tab falsely claims "the deliverable is still on the Preview and Files tabs" when no deliverable was ever produced |
| 88 | Low | `/workflows/new` | Composer's per-agent Skills "View details" modal renders raw markdown syntax instead of formatted content |
| 89 | Low | `/library/hooks/<id> → close` | Closing a hook detail view resets the Hooks tab's search text and category filter |
| 90 | Low | `/runs/b9feac1c-ec21-4531-8ba7-bb391786993e` | Chat message timestamps in the Preview transcript silently reset to the current time on every page reload, never reflecting when the message was actually sent |
| 91 | Low | `/analytics` | Daily Activity chart tooltip shows raw unformatted token counts, breaking the page's own number-formatting convention |
| 92 | Low | `/handoff/settings` | Whitespace-only API key name bypasses the "Default" fallback, creating a permanently blank/unidentifiable key entry |

---

## Full detail

### 1. [High] An already-authenticated user landing on /login is shown the sign-in form instead of being redirected to /dashboard

- **ID:** `BUG-20260827-223300-login-expired-true`
...
### 2. [High] "Jump back in" recent-runs cache is not scoped per user, leaking a previous account's run history into a different signed-in user's session

- **ID:** `BUG-20260827-234030-create`
...
### 3. [High] Selecting the "Investment Banking Pitch Book" template introduces an unsatisfiable "Pick a design system" requirement that permanently blocks Continue

- **ID:** `BUG-20260827-234600-create-ppt`
- **Route:** `/create/ppt`
...
### 4. [High] Browser Forward navigation to the prototype wizard leaves a permanently blank page

- **ID:** `BUG-20260827-235402-create-prototype`
...
### 5. [High] "Save as copy" of the PPT built-in silently drops the manifest, converting the copy from a PPTX-generating workflow into a generic streamed-text/markdown workflow

- **ID:** `BUG-20260828-005700-workflows-ppt-canvas`
...
### 6. [High] Run History silently caps at 50 runs and mislabels it as the full total, with no pagination path to the remaining 223

- **ID:** `BUG-20260828-011500-runs`
- **Route:** `/runs`
...
### 7. [High] Files tab never lists the run's real PPTX binary deliverable; instead shows a scratch tmp/ HTML file mislabeled as "Final output"

- **ID:** `BUG-20260828-012900-runs-id-files`
...
### 8. [High] The Audit tab fabricates 96 "Governance/Gate" records from ordinary pipeline events; the backend's real gate-events endpoint reports zero for the same run

- **ID:** `BUG-20260828-013900-runs-id-audit`
...
### 9. [High] "Edit brief & run again" on a failed run opens a brand-new, completely blank composer instead of pre-filling the original brief

- **ID:** `BUG-20260828-015500-runs-failed`
...
### 10. [High] "Run Again" on a cancelled run leaves the live /stream view permanently stuck at "Running · 0/4 agents · Starting…" after the backend has already failed the pipeline

- **ID:** `BUG-20260828-015930-runs-cancelled`
...
### 11. [High] "Start a new run" on a diverted run's detail page calls the resume endpoint, which the backend correctly rejects, with no visible error shown to the user

- **ID:** `BUG-20260828-020430-runs-diverted`
...
### 12. [High] Hook cards on the Hooks tab are not keyboard-operable

- **ID:** `BUG-20260828-025521-library-hooks`
...
### 13. [High] An over-length GitHub PAT is echoed back verbatim in the 422 response body and rendered raw on-page, contradicting "never returned by any API"

- **ID:** `BUG-20260828-034200-handoff-settings`
...
### 14. [High] "Save as my version" on the seeded "Ask a Human, Then Hand Off" example fails with a raw 422 backend manifest-validation error

- **ID:** `BUG-20260828-051530-create`
...
### 15. [High] "Save as my version" on `/create/ppt` silently discards the entered brief and selected template, always saving the same static generic record

- **ID:** `BUG-20260828-052800-create-ppt-r2`
- **Route:** `/create/ppt`
...
### 16. [High] A path-traversal `mode` value escapes the `/create/<mode>` namespace and navigates to an unrelated real route, including `/admin`

- **ID:** `BUG-20260828-060000-workflow-create-prototype`
...
### 17. [High] A saved workflow's launch panel loses its override entirely and renders the generic base-type wizard, even though the override fetch succeeds

- **ID:** `BUG-20260828-073900-workflows-id-run-r2`
...
### 18. [High] Pipeline filter narrows the breakdown list but leaves every KPI tile, chart, and Success Rate on unfiltered all-pipelines data

- **ID:** `BUG-20260828-081200-analytics`
- **Route:** `/analytics`
...
### 19. [High] Switching tabs while pinned to a non-root version silently drops the pin: URL, API calls, and displayed data all revert to v1

- **ID:** `BUG-20260828-090732-runs-id-versions`
...
### 20. [High] Skill detail route (`/library/skills/<id>`) now falls back to the Library listing for EVERY skill id, hyphenated or not — a full regression beyond the previously filed no-hyphen-only scope

- **ID:** `BUG-20260828-091300-library-skills-r2`
...
### 21. [High] "Run once" on the dead-link `/canvas` composer launches a real 9-agent pipeline run, ignoring the visibly-empty 0-agent workflow shown on screen

- **ID:** `BUG-20260828-104200-workflows-nonexistent-r2`
...
### 22. [Medium] Library search with no matches shows a blank page, no empty state

- **ID:** `BUG-20260827-221730-library`
...
### 23. [Medium] Password-mismatch error does not clear when the fields are corrected to match

- **ID:** `BUG-20260827-222300-settings-profile`
...
### 24. [Medium] Sign in button is not disabled during the login request, allowing duplicate submissions

- **ID:** `BUG-20260827-222750-login`
- **Route:** `/login`
...
### 25. [Medium] For an authenticated user, unmatched `/register/<sub-path>` URLs silently render the Dashboard instead of a 404 or a redirect

- **ID:** `BUG-20260827-231407-register`
...
### 27. [Medium] Checking an agent in the composer's "Review gates" checklist does not set that agent's Gate in the Advanced modal's per-agent Config panel

- **ID:** `BUG-20260828-000100-create-app`
- **Route:** `/create/app`
...
### 28. [Medium] The Advanced modal's Workflow-level settings (Smart planning, Confirm requirements first, Deliverable strategy, Internet access) are permanently disabled with no explanation

- **ID:** `BUG-20260828-000600-create-user-stories`
...
### 29. [Medium] Advanced modal's "Cancel" button does not discard edits to a conditional-gate outcome's route

- **ID:** `BUG-20260828-001100-create-ex-a2-branch`
...
### 30. [Medium] `/workflow/create` never redirects when `mode` is absent or empty, leaving the legacy URL live and uncanonicalized

- **ID:** `BUG-20260828-001420-workflow-create-ppt`
...
### 31. [Medium] Invalid `mode` value redirects into a permanently dead-end 0-agent composer with the underlying 404 silently swallowed

- **ID:** `BUG-20260828-001720-workflow-create-prototype`
...
### 32. [Medium] Leaving the new-workflow composer via its own "Back" control silently discards all unsaved work with no confirmation

- **ID:** `BUG-20260828-003800-workflows-new`
- **Route:** `/workflows/new`
...
### 33. [Medium] Simple view's per-agent "Configure →" panel has no Tools section at all, hiding an existing tool-grant override (e.g. Write files disabled) with no visibility or edit path

- **ID:** `BUG-20260828-005500-workflows-id-edit`
...
### 34. [Medium] Loading a saved-workflow launch panel with a nonexistent or malformed workflow id silently renders the Dashboard under the unchanged bogus URL

- **ID:** `BUG-20260828-010600-workflows-id-run`
...
### 35. [Medium] Run detail header's relative timestamp is stuck on "just now" for a run that finished 11+ hours ago, disagreeing with the version picker's own age label

- **ID:** `BUG-20260828-011700-runs-id`
...
### 36. [Medium] Steps tab footer's "input" token figure is ~20x too low and its own breakdown does not sum to the stated total

- **ID:** `BUG-20260828-012500-runs-id-steps`
...
### 37. [Medium] Deep-linking a specific agent's step URL never opens that agent's detail pane; it renders the plain unfiltered lane list instead

- **ID:** `BUG-20260828-012700-runs-id-steps-agent`
...
### 38. [Medium] Workspace tab's "Code" file viewer is a live, freely-editable text editor with no read-only indication for a completed run

- **ID:** `BUG-20260828-013500-runs-id-workspace`
...
### 39. [Medium] For a failed run with no Preview tab, `/preview/full` silently renders the Audit trail instead of an honest "no deliverable" state

- **ID:** `BUG-20260828-014416-runs-id-preview-full`
...
### 40. [Medium] Diverted run's Preview tab shows the "waiting for live output" placeholder even though the run is permanently terminal

- **ID:** `BUG-20260828-014937-runs-id-stream`
...
### 41. [Medium] Version picker label is stuck on "Version v1" while viewing a real, distinct v2

- **ID:** `BUG-20260828-020900-runs-id-versions`
...
### 42. [Medium] Library agent detail's "Save agent" Config override silently discards the change with no API call

- **ID:** `BUG-20260828-025950-library-agents-id`
...
### 43. [Medium] Skill IDs with no hyphen (single-word slug) silently fall back to the Library listing instead of rendering the skill detail page

- **ID:** `BUG-20260828-030430-library-skills-id`
...
### 44. [Medium] Hook detail's "Compatible agents" list references nonexistent agent ids not found in the Agents library

- **ID:** `BUG-20260828-030700-library-hooks-id`
...
### 45. [Medium] Pipeline model selector enforces no tier restriction; a basic-tier account can select and persist the most expensive "powerful" models

- **ID:** `BUG-20260828-031200-settings-ai-model`
...
### 46. [Medium] "Manage plan" button on Usage & Limits has no click handler at all; a basic-tier account has no way to upgrade

- **ID:** `BUG-20260828-031600-settings-usage`
...
### 47. [Medium] Constitution's "X / 4000 chars" limit is display-only; content well over 4000 chars saves and persists with no truncation or rejection

- **ID:** `BUG-20260828-031900-settings-constitution`
...
### 48. [Medium] Security tab tells break-glass admin their credentials are "managed outside the application" when this app manages them itself

- **ID:** `BUG-20260828-032424-settings-security`
...
### 49. [Medium] Create-User dialog accepts and persists a malformed email with no @ or domain

- **ID:** `BUG-20260828-033400-admin`
- **Route:** `/admin`
...
### 50. [Medium] `/workflows/<bad-id>/canvas` silently opens an empty "copy" composer instead of 404ing, letting a user save a brand-new persisted workflow from a dead link

- **ID:** `BUG-20260828-040400-workflows-nonexistent`
...
### 51. [Medium] `?error=quota` always shows the "too large" dead-end even when a fully valid preview is sitting in sessionStorage

- **ID:** `BUG-20260828-040915-preview-fullscreen-quota`
...
### 52. [Medium] A genuine network/fetch failure on the handoff page is misreported as "Handoff not found", even for a real, valid, owned token

- **ID:** `BUG-20260828-042900-handoff-invalid`
- **Route:** `/handoff/{token}`
...
### 53. [Medium] "Coming Soon" agents are fully browsable and configurable via direct URL, bypassing the catalog's own disabled/unreachable gating

- **ID:** `BUG-20260828-050115-library`
...
### 54. [Medium] Run detail chat header shows a token total that does not match the run's own API `token_usage`, disagreeing with the correct figure shown on the Run History list card for the same run

- **ID:** `BUG-20260828-051900-runs`
...
### 55. [Medium] Delete-workflow confirmation dialog never identifies which workflow it will delete

- **ID:** `BUG-20260828-052400-workflows`
- **Route:** `/workflows`
...
### 56. [Medium] Oversized text attachment (.txt/.md/.json/.csv) is silently truncated with zero user-visible indication, unlike the equivalent PDF/DOCX/PPTX path

- **ID:** `BUG-20260828-053745-create-app-r2`
- **Route:** `/create/app`
...
### 57. [Medium] The "Add agent" library's own "User Stories" category tab always shows "No agents found", even for a bare text search

- **ID:** `BUG-20260828-054500-create-user-stories`
- **Route:** `/create/user-stories`
...
### 58. [Medium] A step's seeded "before-human" gate is invisible in both the Gate combobox and the Review-gates checklist on fresh load

- **ID:** `BUG-20260828-055300-create-ex-a2-branch`
...
### 59. [Medium] "Save workflow" silently no-ops when the name field is empty, with zero visible feedback and no API request fired

- **ID:** `BUG-20260828-060700-workflows-new-r2`
- **Route:** `/workflows/new`
...
### 60. [Medium] Preview toolbar's "Full Screen" button renders entirely outside a narrow (mobile) viewport with no scroll affordance to reach it

- **ID:** `BUG-20260828-084621-runs-id-preview-full`
- **Route:** `/runs/{id}/preview/full`
...
### 61. [Medium] Completed run's "Download the deliverable" toolbar button stays permanently disabled when the backend never populates `deliverable_filename`, even though the deliverable is fully rendered

- **ID:** `BUG-20260828-085047-runs-id-stream`
- **Route:** `/runs/{id}/stream`
...
### 62. [Medium] Steps panel keeps showing "Run failed" while a reopened run is genuinely running

- **ID:** `BUG-20260828-085400-runs-failed`
...
### 63. [Medium] A run whose "Run Again" pipeline actually failed keeps reporting `status: "cancelled"` everywhere, permanently misrepresenting its real outcome

- **ID:** `BUG-20260828-085830-runs-cancelled`
- **Route:** `/runs/{id}, /runs`
...
### 64. [Medium] Library agent detail's Skills tab "Add" gives success feedback (checkmark + badge count) but persists nothing — silently discarded on reload, no Save affordance exists at all

- **ID:** `BUG-20260828-092630-library-agents-id`
- **Route:** `/library/agents/<agentId> (e.g. /library/agents/material-analyzer)`
...
### 65. [Medium] Concurrent `PUT /api/settings/preferences` calls silently lose one write and return a false-success response with the WRONG persisted value

- **ID:** `BUG-20260828-094120-settings-ai-model-r2`
- **Route:** `/settings/ai-model`
...
### 66. [Medium] /create/<type> composer has no tier gating at all; a basic-tier account reaches a fully configured, submit-ready wizard for a deliverable its own Usage & Limits page says it cannot run

- **ID:** `BUG-20260828-094937-settings-usage-r2`
...
### 67. [Medium] "Clear" on Constitution deletes the saved value immediately, with no confirmation and no relation to "Save"

- **ID:** `BUG-20260828-095800-settings-constitution-r2`
- **Route:** `/settings/constitution`
...
### 68. [Medium] Mid-session token expiry drops the original protected route; re-login always lands on /dashboard instead of using the working `?redirect=` mechanism

- **ID:** `BUG-20260828-101500-login-expired-true-r2`
...
### 69. [Medium] Create-User dialog silently swallows a duplicate-email 409, giving the admin zero feedback that submission failed

- **ID:** `BUG-20260828-103434-admin`
- **Route:** `/admin`
...
### 70. [Medium] File explorer/download parses a markdown subheading in the deliverable as a phantom "file" with no real name or extension

- **ID:** `BUG-20260828-105300-preview-fullscreen-r2`
...
### 71. [Medium] Audit tab's CSV and JSON export strip the Security/Governance category distinction and never populate the promised "severity" column

- **ID:** `BUG-20260828-audit-export-r2`
- **Route:** `/runs/b9feac1c-ec21-4531-8ba7-bb391786993e/audit`
...
### 72. [Low] Escape does not close a catalog card's "Inspect" details modal

- **ID:** `BUG-20260827-221400-dashboard`
- **Route:** `/dashboard`
...
### 73. [Low] Escape does not close a saved-workflow card's "Workflow actions" dropdown menu

- **ID:** `BUG-20260828-003300-workflows`
- **Route:** `/workflows`
...
### 74. [Low] Saved-workflow detail view lists agents by raw internal slug instead of the friendly display name used everywhere else in the app

- **ID:** `BUG-20260828-004300-workflows-id`
- **Route:** `/workflows/{id}`
...
### 75. [Low] "Run Workflow" enables with zero agents attached and silently no-ops on click

- **ID:** `BUG-20260828-011000-workflow`
- **Route:** `/workflow`
...
### 76. [Low] Skill with an empty `category` field renders a blank category label and is unreachable through any category pill

- **ID:** `BUG-20260828-021300-library-skills`
...
### 77. [Low] Hook detail page's "Copy" button gives zero feedback that a copy occurred

- **ID:** `BUG-20260828-025500-library-hooks`
...
### 78. [Low] File explorer search leaves orphaned empty folders visible and the footer file count never reflects the active filter

- **ID:** `BUG-20260828-041300-preview-fullscreen`
...
### 79. [Low] Failed-login error message renders with no ARIA live region or alert role, so screen readers never announce it

- **ID:** `BUG-20260828-041815-login-expired-1`
...
### 80. [Low] Bare `/settings` redirects correctly on a cold/full load but renders the honest 404 page when reached via client-side (SPA) navigation

- **ID:** `BUG-20260828-042311-settings`
- **Route:** `/settings`
...
### 81. [Low] Confirm new password field has no show/hide toggle, unlike its sibling fields

- **ID:** `BUG-20260828-050900-settings-profile`
- **Route:** `/settings/profile`
...
### 82. [Low] "Upload custom template → From URL" leaks a raw Python errno/socket message to the user on an unreachable domain

- **ID:** `BUG-20260828-053300-create-prototype-r2`
- **Route:** `/create/prototype`
...
### 83. [Low] Simple and Canvas views disagree on the PPT built-in's deliverable type ("Custom" vs "Streamed text")

- **ID:** `BUG-20260828-073350-workflows-ppt-canvas`
...
### 84. [Low] Removing a file attachment leaves a stale "[Attached: ...]" marker in the brief textarea, keeping Run Workflow enabled for a phantom attachment

- **ID:** `BUG-20260828-074900-workflow-r2`
- **Route:** `/workflow`
...
### 85. [Low] Tool-call summary row renders "[object Object]" instead of a readable preview for array/object arguments

- **ID:** `BUG-20260828-080047-runs-id-steps`
- **Route:** `/runs/b9feac1c-ec21-4531-8ba7-bb391786993e/steps`
...
### 86. [Low] Files tab file sizes are character counts, not byte counts, understating every non-ASCII file

- **ID:** `BUG-20260828-082706-runs-id-files`
- **Route:** `/runs/b9feac1c-ec21-4531-8ba7-bb391786993e/files`
...
### 87. [Low] Diverted run's Workspace tab falsely claims "the deliverable is still on the Preview and Files tabs" when no deliverable was ever produced

- **ID:** `BUG-20260828-090219-runs-diverted`
...
### 88. [Low] Composer's per-agent Skills "View details" modal renders raw markdown syntax instead of formatted content

- **ID:** `BUG-20260828-093400-library-skills-id-r2`
...
### 89. [Low] Closing a hook detail view resets the Hooks tab's search text and category filter

- **ID:** `BUG-20260828-093757-library-hooks-id`
...
### 90. [Low] Chat message timestamps in the Preview transcript silently reset to the current time on every page reload, never reflecting when the message was actually sent

- **ID:** `BUG-20260828-095700-runs-id-chat-timestamp`
...
### 91. [Low] Daily Activity chart tooltip shows raw unformatted token counts, breaking the page's own number-formatting convention

- **ID:** `BUG-20260828-102900-analytics-r2`
- **Route:** `/analytics`
...
### 92. [Low] Whitespace-only API key name bypasses the "Default" fallback, creating a permanently blank/unidentifiable key entry

- **ID:** `BUG-20260828-103900-handoff-settings-r2`
- **Route:** `/handoff/settings`
