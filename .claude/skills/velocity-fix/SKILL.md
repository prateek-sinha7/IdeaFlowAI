...
1. `.planning/FIX-REGISTER.md` — has this exact bug (or its root cause) already been fixed? Grep
   for keywords from the bug description first: `grep -in "<keyword>" .planning/FIX-REGISTER.md`
...
3. The complete Implementation Register. It contains the full history of all phases, every file
   touched, every decision locked, and every piece of code deliberately deleted. This is
   mandatory — it tells you what was intentional vs. accidental, what exists vs. what was
   removed, and what constraints bind every fix.
...
If quick task, generate the task ID and create the folder:

```bash
DATE=$(date +%y%m%d)
ID=$(python3 -c "import random,string;print(''.join(random.choices(string.ascii_lowercase+string.digits,k=3)))")
SLUG="<short-kebab-slug-of-the-bug>"   # e.g. "router-guardrail-data-page-link-conflict"
DIR=".planning/quick/${DATE}-${ID}-${SLUG}"
mkdir -p "$DIR"
echo "$DIR"
```

Never invent a quick-task ID that collides with an existing one — the random suffix plus grepping
`.planning/quick/` first is enough to avoid this in practice.
...
- "<fact confirmed in Steps 1-3>, confirmed by <grep/read>"
</facts_verified_during_planning>
```
...
- **Backend restart needed?** — if backend files changed and a dev server is running, restart it

Identify and run the narrowest real test suite that exercises the change, e.g.:

```bash
cd backend
python3.11 -m pytest tests/agents/ -v          # agent-runtime changes
python3.11 -m pytest tests/unit/ -v            # engine/API/registry changes
python3.11 -m pytest tests/agents/test_guardrails.py -v   # guardrail-file changes
```
...
Find the next Fix ID:

```bash
grep -oE 'FIX-[0-9]+' .planning/FIX-REGISTER.md | sort -t- -k2 -n | tail -1
```
...
- **Root Cause**: the Step 3 finding, stated precisely (this is what future greps will match on).
...
- Never invent a Fix ID or quick-task ID that collides with an existing one — always grep first.
