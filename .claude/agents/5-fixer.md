...
- **Your result's `statusSet` field is that bare word and nothing else** — `FIXED`, never
  `FIXED — written to the ledger (row 22)`. The scheduler matches `statusSet` against the next
  ...
  Legal values for this phase: `FIXED | ESCALATED`.
