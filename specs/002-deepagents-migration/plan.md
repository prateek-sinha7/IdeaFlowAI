...
`live_contract.py` (content-agnostic validator + cost watcher), `test_phase8_live.py` (per-pipeline LIVE
suite + HITL on/off + offline proof), `test_phase8_resume.py`+`_resume_child.py` (**cross-process
resume-after-kill proven against real docker Postgres — incl. a SIGKILL crash + an InMemory-fails
contrast**), self-tests `test_live_harness.py` (14) + `test_live_contract.py` (29), and `PHASE8_VERIFY.md`.
Offline: `tests/agents` **238 passed / 17 skipped / 0 failed**, ruff clean.

**FIRST LIVE SMOKE PASSED (`72de9e1`).** `user_stories` end-to-end on real Bedrock Haiku (default
profile): **`CONTRACT: PASS`**, clarify auto-answered, 6 agents, ~$0.04 / 77k tokens / ~170s. The live
run surfaced **two harness gaps offline couldn't** (the faked planner never clarifies), both fixed +
regression-tested: (1) the REAL planner returned `CLARIFY_REQUIRED` → `ClarifyEngine` blocked on a human
questionnaire answer that never comes headlessly (hung ~1h52m) → `drive_engine_pipeline` now
