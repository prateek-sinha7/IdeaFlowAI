# Integration Test Report — `sample_conditional_previous_step`

**Test file:** `tools/api/runs/run.sample_conditional_previous_step.http`
**Run method:** `./run-http.sh sample_conditional_previous_step` (httpyac 6.16.7, `-e local --all`)
...
**Log transcript:** `tools/api/runs/logs/sample_conditional_previous_step/20260821-153804.log`
...
`sample_conditional_branch_new`: the `greet` step ignored its literal `write_file` instruction
...
Workflow `sample_conditional_previous_step` (spec 014, example A1 — conditional gate looping
back to an earlier step). Steps per
`backend/agents/workflows/sample_conditional_previous_step/workflow.yaml` **as it existed at
...
`sample_conditional_branch_new` report: `write_file` calls used titles derived from the run's
...
`backend/agents/workflows/sample_conditional_previous_step/workflow.yaml` was modified during/
...
and the `sample_conditional_branch_new` report both document. **This report's findings apply
...
1. **Loop-count assertion (step 4, informational) — confirmed BUGGY, same class of bug as
   `sample_conditional_branch_new`.** The script does `events.filter(e => e.type ===
...
`payload_json` field-name bug documented in the `sample_conditional_branch_new` report; the
...
`loop_max_iterations: 3` was never tested. Root cause is identical to the `sample_conditional_branch_new`
...
field-name bug already documented for `sample_conditional_branch_new`; the other two assertions
