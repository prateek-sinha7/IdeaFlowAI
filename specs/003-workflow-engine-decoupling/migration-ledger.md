...
| FANOUT-PERSIST | fan-out child audit trail — additive `subagent_runs` table (alembic `0019`, down_revision `0018`) + `SubagentRun` ORM + `ScopedStore.record_subagent_run`/`update_subagent_run`/`read_subagent_runs` + `KernelServices.record_subagent_run` (Phase 11 / FANOUT-10) | the owner-scoped per-child audit layer the single kernel `run_fanout` spawn path writes (one row per child, cross-owner read = ∅, T-11-01-03) | 11 | CHECK: the 0018→0019 chain stays a single head and is reversible offline — `tests/agents/test_subagent_runs.py::test_0019_reversible_offline` (upgrade head → downgrade -1 → upgrade head); NO `sa.Enum` | ☑ | |
| WAVE-PERSIST | per-wave audit trail — additive `wave_runs` table (alembic `0020`, down_revision `0019`) + `WaveRun` ORM + `ScopedStore.record_wave_run`/`update_wave_run`/`read_wave_runs` + `KernelServices.record_wave_run` (Phase 12 / WAVE-02) | the owner-scoped per-wave audit layer the `wave_scheduler` strategy writes through the single kernel `run_fanout` spawn path (one row per executed wave, cross-owner read = ∅, T-12-01-IDOR); the durable substrate the 12-03 mid-wave resume reads | 12 | CHECK: the 0019→0020 chain stays a single head and is reversible offline — `tests/agents/test_wave_runs.py::test_0020_reversible_offline` (upgrade head → downgrade -1 → upgrade head); NO `sa.Enum` | ☑ | |
