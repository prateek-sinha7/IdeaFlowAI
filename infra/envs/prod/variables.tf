...
variable "owner" {
  description = "Tag value for Owner. Drives AWS Cost Explorer / Billing reports filtered by tag — finance can answer 'how much is team X costing us?' when this is set. Default 'flowin' is fine for single-team / single-project accounts; override for multi-team accounts where per-team cost allocation matters."
  type        = string
  default     = "flowin"
}
