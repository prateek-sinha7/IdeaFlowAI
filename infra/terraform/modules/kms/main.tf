...
resource "aws_kms_key" "this" {
  ...
  policy = jsonencode({
    ...
    Statement = concat(
      ...
      var.allow_logs_service ? [
        {
          ...
          Condition = {
            ArnLike = {
              "kms:EncryptionContext:aws:logs:arn" = "arn:${local.partition}:logs:${var.region}:${var.account_id}:log-group:/velocityai/*"
            }
          }
        }
      ] : [],
      ...
      # events for SSM Get/Put on /velocityai/${env}/* + KMS Decrypt on the
