...
resource "terraform_data" "guard" {
  ...
  lifecycle {
    precondition {
      ...
      error_message = "Account mismatch: provider authenticated to ${data.aws_caller_identity.current.account_id}, expected ${var.expected_account_id}. Refusing to apply VelocityAI Terraform."
    }

    precondition {
      condition     = data.aws_region.current.name == var.expected_region
      error_message = "Region mismatch: provider region is ${data.aws_region.current.name}, expected ${var.expected_region}. Refusing to apply VelocityAI Terraform."
    }
  }
}
