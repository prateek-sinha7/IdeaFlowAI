...
description: Cream blueprint architecture deck — cream paper #F0EAE0 background + single rust-red #B5392A accent, 48px blueprint grid mask, 2px black-border hard cards, pipeline step boxes (one raised), right-side rust-red insight callout, large Playfair serif type, SVG dashed feedback loop. Zero gradients, zero soft shadows — serious and print-friendly.
triggers:
  - "architecture"
  - "blueprint"
  - "system design"
...
od:
  ...
  speaker_notes: true
  animations: true
  example_prompt: "Use the html-ppt-knowledge-arch-blueprint template to make a system architecture PPT. Cream paper background + rust-red accent + blueprint grid + one raised pipeline step + large serif type. Tell me the system name + 5-7 core modules + data flow direction first, then write 8-10 slides."
---
