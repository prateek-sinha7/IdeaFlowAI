...
> 🌏 **Versão em chinês: [README.md](./README.md)**
...
```
magazine-web-ppt/
├── SKILL.md              ← arquivo principal da skill: workflow, princípios, erros comuns
├── README.md             ← README em chinês
├── assets/
│   └── template.html     ← HTML semente totalmente funcional (CSS + WebGL + JS de swipe já configurados)
└── references/
    ├── components.md     ← manual de componentes (tipografia, cor, grid, ícones, callout, stat, pipeline)
    ├── layouts.md        ← 10 esqueletos de layout de página (prontos para colar)
    ├── themes.md         ← 5 paletas predefinidas (só escolher, sem personalizar)
...
- Série de PPTs de palestras presenciais de Guizang
