...
description: Dark-terminal honest-review deck — #0a0c10 black background + 56px cyber grid + CRT vignette + scanlines, traffic-light window chrome, `$ prompt` command-line titles, large mint-green #7ed3a4 text, JetBrains Mono, stroke-only bar charts, blinking cursor, amber/green/red three-tier tags, dark code blocks. Suited for CLI / agent / dev tool reviews (including trace, diff, benchmark).
triggers:
  - "terminal review"
  - "cli review"
  - "agent review"
...
od:
  ...
  speaker_notes: true
  animations: true
  example_prompt: "Use the html-ppt-hermes-cyber-terminal template to build a CLI / agent evaluation deck. Dark terminal style + scanlines + command-line titles + benchmark bar charts. First confirm: the thing being evaluated, 3-5 comparison dimensions, and benchmark data."
---
