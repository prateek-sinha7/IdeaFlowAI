...
description: HTML PPT Studio — author professional static HTML presentations in many styles, layouts, and animations, all driven by templates. Use when the user asks for a presentation, PPT, slides, keynote, deck, slideshow, "slides", "talk script", "make a PPT", "make some slides", a reveal-style HTML deck, a Xiaohongshu (RED) style image-and-text post, or any kind of multi-slide pitch/report/sharing document that should look tasteful and be usable with keyboard navigation. Triggers include keywords like "presentation", "ppt", "slides", "deck", "keynote", "reveal", "slideshow", "talk script", "sharing deck", "Xiaohongshu post", "talk slides", "pitch deck", "tech sharing", "technical presentation".
triggers:
  - "ppt"
  - "deck"
  - "slides"
...
od:
  ...
  speaker_notes: true
  animations: true
  example_prompt: "Use html-ppt to build a 12-slide HTML deck. First confirm three things with me: content/page count/audience, theme (recommend 2-3 from the 36 available), and a starting full-deck template (pick one of pitch-deck / tech-sharing / weekly-report / xhs-post / presenter-mode-reveal) — then start writing slides once we're aligned."
---
...
### 🎤 Presenter Mode (speaker view + full script)

If the user mentions any of: **presenting / sharing / talk script / full script / speaker notes / presenter view / teleprompter**, or says things like "I need to present xxx to the team", "I'm doing a tech-sharing talk", "I'm worried I'll stumble while presenting", "I want a deck with a full script" — **use the `presenter-mode-reveal` full-deck template** and write 150–300 words of speaker script in each slide's `<aside class="notes">`.
...
1. **It's not a script to read verbatim, it's a set of cue signals** — bold the key terms and give transition sentences their own paragraph
2. **150–300 words per slide** — paced for 2–3 minutes per slide
3. **Write like you speak, not like you write** — favor conversational phrasing over formal/written phrasing
...
- 🟠 **SPEAKER SCRIPT** — large-font full script (scrollable)
...
Only `presenter-mode-reveal` is designed from the ground up around the feature with a proper example full script on every slide.
...
1. **Content & audience.** What's the deck about, how many slides, who's
   watching (engineers / execs / Xiaohongshu readers / students / VCs)?
2. **Style / theme.** Which of the 36 themes fits? If unsure, recommend 2-3
   ...
   - Xiaohongshu (RED) social-post style → `xiaohongshu-white`, `soft-pastel`, `rainbow-gradient`,
     `magazine-bold`
...
3. **Starting point.** One of the 14 full-deck templates, or scratch? Point
   ...
   user's content suggests something obvious (e.g. "I need a product launch deck" →
   `product-launch`), propose it confidently instead of asking blindly.

A good opening message looks like:

> I can put this deck together for you! Let's confirm three things first:
> 1. Roughly what's the content / how many slides / who's the audience?
> 2. Any style preference? I'd suggest picking one of these three themes: `tokyo-night` (a solid default for tech-sharing talks), `xiaohongshu-white` (Xiaohongshu/RED style), `corporate-clean` (formal report style).
> 3. Want to start from my ready-made `tech-sharing` full-deck template?
...
- **NEVER put presenter-only text on the slide itself.** Descriptive text like
  "This slide shows…" or "Speaker: you can add more detail here…" or small explanatory captions
...
- [references/presenter-mode.md](references/presenter-mode.md) — **presenter mode + full-script writing guide (a must-read for tech-sharing talks/presentations)**.
