...
def slide_pipeline(prs, *, kicker, headline, intro, label, steps):
    ...
    add_footer(slide, "Page 08 · Coach On-Day Actions", "Witness, don't intervene", theme="light")
```
...
def slide_two_col(prs, *, kicker, headline, intro, left, right):
    slide = prs.slides.add_slide(blank_layout)
    paint_bg(slide, COLOR_INK)
    add_chrome(slide, "First-Time Caveats · Things to Watch", "05 / 14", theme="dark")
    ...
    add_footer(slide, "Page 05 · Special Notes for First-Timers", "A ceremony is not a leadership day", theme="dark")
```
...
def slide_obs_grid(prs, *, kicker, headline, intro, cards):
    ...
    slide = prs.slides.add_slide(blank_layout)
    paint_bg(slide, COLOR_PAPER)
    add_chrome(slide, "Observation · Field Notes", "09 / 14", theme="light")
    ...
    add_footer(slide, "Page 09 · Six Observation Metrics", "For the record only · not scored on the spot", theme="light")
```
