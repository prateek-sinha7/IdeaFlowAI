...
they never spell out a `python tools/...` line of their own. One command,
...
| name | command |
|---|---|
| **the status command** | `python tools/knowledge/status.py` |
| **the status command (JSON)** | `python tools/knowledge/status.py --json` |
| **the full rebuild** | `python tools/knowledge/rebuild_knowledge.py` |
| **the cards-only rebuild** | `python tools/knowledge/rebuild_knowledge.py --skip-architecture` |
| **the rebuild dry-run** | `python tools/knowledge/rebuild_knowledge.py --check` |
| **the sync-point stamp** | `python tools/knowledge/rebuild_knowledge.py --skip-architecture --set-sync-point` — rebuilds cards-only AND moves `state.yaml`'s `last_sync_commit`/`last_sync_date` to HEAD. Only `sync` may run this, and only after actually reconciling the commit delta (steps 2-7 of `sync.md`) — running it bare re-baselines the watermark with no review, silently dropping whatever came before from every future sync window. |
| **the ID linter** | `python tools/knowledge/normalize_card_ids.py --check` |
| **the ID migration** | `python tools/knowledge/normalize_card_ids.py` |
| **the architecture rebuild** | `python tools/knowledge/build_architecture.py` (`--only MOD-<id>` to scope) |
| **the summary applier** | `python tools/knowledge/apply_summaries.py` (`--check` to preview) |
| **the link validator** | `python tools/knowledge/validate_links.py` |
...
```sh
python tools/knowledge/status.py          # ~0.5s, read-only, ~15 lines
python tools/knowledge/status.py --json   # same data, machine-readable
```
...
```sh
python tools/knowledge/rebuild_knowledge.py
```
...
|---|---|
| `status.md` | `python tools/knowledge/status.py` — read-only, never regenerates |
| `sync.md` | `python tools/knowledge/rebuild_knowledge.py` (source may have moved), then **the sync-point stamp** as its true final action |
...
| `diagrams.md` | `python tools/knowledge/build_architecture.py --affected` / `--stale-prose` to find work, one Haiku subagent per affected card to re-author it, then `build_architecture.py` + `--stamp` per card |
| `book-keeping.md` | after writing a card: `python tools/knowledge/rebuild_knowledge.py --skip-architecture` |
