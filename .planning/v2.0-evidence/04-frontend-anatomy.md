...
## CORRECTIONS (2026-07-07 post-merge verification — the report above describes the PRE-merge tree)

Proof of staleness: report-time `ReviewGatePanel.tsx` = 454 ln vs current 508+ ln; `retainAgentEdit`/`pendingGateEditRef` = 0 hits then, present now. Verified corrections:

1. **§4 ReviewGatePanel:** reject is no longer one-action — a `showRejectConfirm` two-step dialog (`ReviewGatePanel.tsx:449-485`) + `handleRejectReview` navigates home & resets (`DashboardLayout.tsx:1211-1226`) (KAN-95). Analyze gates render a second **"Update the Specs"** button (`:428`, gated `agentId === "prototype-analyze"` — an FE SC-001 name-literal leak to generalize in Phase 32) and relabel approve to "Accept & continue to build" (KAN-101). Edit state resets on output/gateKey change; the approved edit is applied client-side via the new `retainAgentEdit` reducer because **the backend persists gate edits but never echoes them over WS** (KAN-98).
2. **§3 handler inventory:** add `retainAgentEdit` (`useWorkflow.ts:157-168`) and the `specRevisionCount` bump on `prototype-specify` re-start (KAN-101, reconciled with FIX-039's unconditional reset — the reset subsumes KAN-101's conditional clearing).
3. **§3 outbound WS:** `approve_review` now also sends `action:"update_specs"` + `analysis_report` (`page.tsx:1393-1405`); FE clears `reviewGateData` on `pipeline_cancelled/failed` and guards the panel with `isPipelineRunning` (KAN-100).
4. **§2 reopen path:** `WorkflowHistory.handleSelectRun` (`:175-198`) now branches — active run → live execution view (`onViewRunningPipeline`), terminal → detail (KAN-96). `PlanningOverlay` shows in the post-questionnaire pre-`pipeline_start` window (KAN-97).
5. **§5 uploads "images 100% greenfield" — now FALSE:** image picker + `readAsDataURL` + preview chips exist on IdeaInputPage + both wizard pages, threaded to `run_pipeline` (`IdeaInputPage.tsx:543-598`, image waves). Paste/drag-drop/resize remain absent.
6. **§2 anatomy:** `PrototypePipelineView` gained the revision-cycle badge (`specRevisionCount`) and the KAN-99 task-checklist N-1 cap (`:425-436` — final `task_progress` precedes the fix-loop; N/N only when the build truly finishes). Line counts drifted (DashboardLayout 1591→1630, page.tsx 1363→1418, useWorkflow 798→831).
