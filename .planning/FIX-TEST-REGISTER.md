...
| TEST-003 | FIX-251 (renumbered from FIX-217; quick-260811-mxg) | 2026-08-11 | `backend/agents/execution_engine/engine.py`, `backend/agents/execution_engine/context.py` | 11 | 11 | 0 | ✅ Pass |
| TEST-004 | FIX-252 (renumbered from FIX-218; quick-260811-si4) | 2026-08-11 | `backend/agents/execution_engine/engine.py`, `backend/agents/execution_engine/context.py` | 8 | 8 | 0 | ✅ Pass |
| TEST-005 | FIX-253 (renumbered from FIX-219; quick-260812-12t) | 2026-08-12 | `backend/agents/execution_engine/engine.py`, `backend/agents/capabilities/gates/human.py`, `backend/agents/capabilities/gates/approval.py`, `backend/app/api/run_engine.py`, `backend/app/api/run_commands.py` | 18 | 18 | 0 | ✅ Pass |
| TEST-006 | FIX-254 (renumbered from FIX-220; quick-260812-1nz) | 2026-08-12 | `backend/agents/execution_engine/engine.py`, `backend/tests/agents/characterization/_normalize.py`, `frontend/src/components/chat/InlineGateActions.tsx`, `frontend/src/components/layout/DashboardLayout.tsx`, `frontend/src/components/results/StepsOverviewSpine.tsx` | 8 | 8 | 0 | ✅ Pass |
...
| TEST-034 | FIX-255 (2026-08-13 dev merge) | 2026-08-13 | `backend/tests/unit/test_rest_run_launch.py` | 3 (empty brief, whitespace-only brief, non-empty positive control) | 3 | 0 | All seen RED on `785e40c1` first — the empty brief returned **200 OK and minted a run**. The positive control exists so a guard that rejected everything could not pass. |
| TEST-035 | FIX-256 (2026-08-13 dev merge) | 2026-08-13 | `frontend/src/app/dashboard/bridgeTerminality.source.test.ts` | 11 (4 bridge source-locks, 2 deletion locks, 5 semantic incl. an over-blocking control) | 11 | 0 | Seen RED on `785e40c1`: 6 failed / 5 passed (the 5 semantic ones are self-contained, so only the 6 locks discriminate). Source-lock rather than render test — page.tsx is impractical to render; mirrors `terminalReopenReconcile.source.test.ts`. The over-blocking control matters: a fresh store entry also carries `isRunning:false`, so a guard keyed on that alone would freeze every real run at launch. |
