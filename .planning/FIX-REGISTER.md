...
|--------|------|-------------|------------|---------------|---------------|------------|--------|
| FIX-129 | 2026-07-29 | Workflow reports done while the validation + build agents keep running in a loop; the completion notification fires once and can never fire again | Foreign-run frame bleed. `RunConnectionProvider` attaches ONE SSE stream per non-terminal run (`AUTO_STREAM_STATUSES`) and fans EVERY frame from EVERY attached run into the single `handleWebSocketMessage` subscriber. The agent-scoped payloads (`agent_start`/`agent_chunk`/`agent_complete`/`tool_*`/`task_*`) carry NO `pipeline_run_id`, and `RunStreamMessage` was not tagged with the run it arrived on, so a concurrently-running run's frames were indistinguishable from the viewed run's — and agent ids are NOT unique across runs (two prototype runs both stream `prototype-build`/`prototype-validate`). After the viewed run's `pipeline_complete` swept its agents to `done` and the completion notification fired (consuming `currentPipelineNotifId`), the OTHER run's build/validate frames flipped those same agents back to `running` and kept them cycling through its task loop. Only `pipeline_start` (BUG-005) and `pipeline_complete` (BUG-011) had ad-hoc foreign-run guards; every agent-state frame was applied blind. | `frontend/src/hooks/useRunStream.ts`, `frontend/src/providers/RunConnectionProvider.tsx`, `frontend/src/lib/wsReplayState.ts`, `frontend/src/app/dashboard/page.tsx`, `frontend/src/hooks/useRunStream.test.ts` (tests), `frontend/src/lib/__tests__/wsRunScope.test.ts` (tests) | Phase 29/44 (SSE transport + provider fan-out), Phase 12 (RESUME-03 dedup/cursor), Phase 38 (notification feed) | INV-1/3/12/SC-001 ✅ | Done |
| FIX-128 | 2026-07-28 | Chat-lane narrator result cards: "Open in Steps" / "Open in Preview" never switched the right-hand PreviewPanel tab (both buttons inert, running + completed run screens) | Two compounding defects on the same deep-link seam. (1) `ResultCard.CARD_SPECS` used `defaultTab: "steps"` for clarify/gate/pipeline/spec_revision, but PreviewPanel's tab id for the "Steps" tab is `"thinking"` — `"steps"` is not in `PANEL_TAB_IDS`. (2) `useRunChat.parseDeepLink` aliased the frame's `deep_link.target` onto `deepLink.tab`; `target` is the narrator's milestone ANCHOR (`run:<uuid>` / `deliverable:<file>` / `clarify:<id>` / `gate_key`), never a tab id — and because the stored `tab` won over the kind's `defaultTab`, every live card emitted an anchor as its tab. Either way PreviewPanel's `PANEL_TAB_IDS.includes(tab)` guard silently dropped the request, so `setActiveTab` never ran. | `frontend/src/components/chat/ResultCard.tsx`, `frontend/src/components/chat/ResultCard.test.tsx`, `frontend/src/hooks/useRunChat.ts`, `frontend/src/hooks/useRunChat.test.ts`, `frontend/src/types/index.ts` | Phase 31 (CHATUI-01 result cards + plan-03 `useTabDeepLink` seam), Phase 32 (plan-07 "Thinking"→"Steps" relabel that kept the internal id) | INV-1/3/12/SC-001 ✅ | Done |
