...
## ⚠ ID RENUMBER — 2026-08-13 `dev` merge (read before citing FIX-217..220)

`dev` and `bugfix/spec-revision-context-loss` **independently allocated the same four Fix IDs to
different fixes**. The four-source allocation rule did not catch it because `dev` registered its
ids (commits `8acf9b4a`, `54857fc3`, `ca724e16`, `c6c47403`) *after* this branch had allocated
them. On merge, **`dev` keeps 217–220** (it is the shared integration branch) and this branch's
four rows were renumbered:

| was (this branch) | is now | `dev`'s fix now holding the old id |
|---|---|---|
| FIX-217 — `update_specs` regenerated the spec from scratch | **FIX-251** | Chain context missing when chaining from prototype/ppt revisions |
| FIX-218 — second "Update the Specs" at the re-opened gate was a silent no-op | **FIX-252** | Chat file attachment injection (KAN-170) |
| FIX-219 — `update_specs_eligible` advertised but never enforced | **FIX-253** | `od_ppt` revision routed to `ppt_revision` |
| FIX-220 — per-firing revision discriminator on the gate | **FIX-254** | `Maximum update depth exceeded` — rAF-coalesced pipelineState bridge |

**Commit messages on this branch still say FIX-217..220 and were NOT rewritten** — history is a
record, not a register. The same applies to the dated `.planning/quick/*/PLAN|SUMMARY|BASELINE`
documents. When an old document or commit on **this** branch says FIX-217..220, resolve it
through the table above. Live lookup tables (FIX-TEST-REGISTER, ISSUES-REGISTER,
IMPLEMENTATION-REGISTER) and the `.knowledge/` cards **were** updated.

Also note: **FIX-214 and FIX-215 appear twice** in the log. Those are benign — the same fix
registered on both sides (this branch reconstructed them as ISS-069 backfills from commit
subjects; `dev` registered them properly). The backfill rows are annotated as superseded.
...
| FIX-159 | 2026-07-30 | C1: Extract nginx + CloudWatch config from bootstrap into reusable reconcile script — config changes never reach live hosts | bootstrap-ec2.sh §13â€“§14 embedded in once-per-instance script; repo edits to nginx/agent config have no delivery path to live hosts without instance destruction or destructive manual re-run. Fix: extract §13 (nginx, excluding cert) + §14 (agent config) into standalone reconcile-host-config.sh; bootstrap fetches and runs it on first boot; CI fetches and runs it on every deploy | `infra/scripts/reconcile-host-config.sh` (new, faithful extraction), `infra/scripts/bootstrap-ec2.sh` (lines 693â€“1208 replaced with S3 fetch + cert block + reconcile call; see 260730-bootstrap-replacement.sh for mechanical application), `infra/terraform/app/main.tf` (added aws_s3_object.reconcile_script, extended depends_on), `infra/buildspec.yml` (added reconcile fetch-and-run after chown, before ECR login) | quick-260730-c1 (infrastructure extraction, unblocks A1/D3/E1/E3) | INV-1/3/12/SC-001 ✅ (infra-only, no engine edits; no app byte changes; one impl; zero engine) | Done (Step 2a blocked by token limit; mechanical replacement script provided) |
| FIX-158 | 2026-07-30 | KAN-142: CloudWatch agent duplicate log stream configuration — audit.log and unattended-upgrades.log collide on /velocityai/*/system:{instance_id} | Two entries in the collect_list both targeted identical (logGroupName, logStreamName) pair (/velocityai/${ENVIRONMENT}/system, {instance_id}), violating CloudWatch's unique stream constraint per CreateLogStream API. Root: config materialized verbatim from documentation in 2026-07-01 prefix-rename pass (commit e1a3a495); uniqueness never validated. Collision latent due to B1 (agent crash-loop on missing logfile perms, FIX-149). Fix: apply uniform stream-naming scheme across all 8 entries — every entry now uses {instance_id}/<source-slug> for unique, self-documenting stream names (nginx-access, nginx-error, postgres, audit, auth, unattended-upgrades, letsencrypt, docker). Added durability comment documenting the naming rule and INV-12 contract. | `infra/scripts/bootstrap-ec2.sh` (collect_list entries lines 1066â€“1073 renamed, durability comment added lines 1030â€“1041) | quick-260730-k8x (infrastructure, no schema/migration, infra-only) | INV-1/3/12/SC-001 ✅ (infra-only, no engine edits; characterization goldens not affected; INV-12 ✅ uniform pattern established and documented) | Done (merge 2026-08-17: this slot's other claimant recorded only "exact FIX-158 scope is NOT recoverable", so the recovered detail below keeps the id) |
| FIX-224 | 2026-08-14 | Agent progress circles and L2 agent view flicker rapidly during active run — setViewedState fires on every SSE frame | `handleFrame` calls `project({ ...entry })` after every pipeline frame. The `project()` bail-out `prev.pipelineState === state.pipelineState` never fired because `fakeSetState` always produces a new pipelineState object reference, and `{ ...entry }` always creates a new outer object. Fix: add `lastProjectedPipelineStateRef` — skip `project()` when `entry.pipelineState` reference hasn't changed (idempotent re-delivery). Reset on `switchViewTo`. | `frontend/src/hooks/useRunStateStore.ts` | Phase 44 (FIX-201 store bridge), FIX-220 | INV-1/3/12/SC-001 ✅ | Done | | `_AGENT_KIND_MAP` did not include `prototype-analyze`, so `_artifact_kind_for` fell back to `"summary"` for it. `_UPDATE_SPECS_ELIGIBLE_KINDS = {"summary"}` then made ANY unmapped gated agent eligible. Fix: add `"prototype-analyze": "analysis"` to the map and change eligible set to `{"analysis"}` — only prototype-analyze produces this kind, all unmapped agents still fall back to `"summary"` which is NOT eligible. | `backend/agents/execution_engine/engine.py` | Phase 27 (KAN-101 spec revision), SC-001 | INV-1/3/12/SC-001 ✅ | Done | | `pipeline_start` case in `handlePipelineMessage` carried `agentStartEventIds` whenever `isSameRunReannounce=true` (same run_id). "Run Again" resumes the same run_id → `isSameRunReannounce=true` → ids carried from cancelled run → first `agent_start` appends a second identity → `deriveSpecRevisionCount` returns 1 → spurious "Spec Revision Cycle 1" banner. Fix: only carry ids when `prev.isRunning=true` (live SSE reconnect), not when `prev.isRunning=false` (resume from terminal state). | `frontend/src/hooks/useWorkflow.ts`, `frontend/src/hooks/useWorkflow.fix222.test.ts` | Phase 42 (KAN-101 spec revision cycle), ISS-063 | INV-1/3/12/SC-001 ✅ tests: 4 frontend ✅ | Done | | (1) Concierge stores `target="od_prototype_output"` in proposal params → `_handle_revision` derives `od_prototype_revision` → no agents → ValueError → silent fail. (2) `_mint_revision_row` created child `WorkflowRun` without `workspace_id` → `run_events.workspace_id NOT NULL` constraint fails on first event write. Fix: (1) `_OD_TARGET_REMAP` applied after reading `target`; (2) inherit `workspace_id` from parent run. | `backend/app/api/run_commands.py` | Phase 14 (revision), FIX-219, FIX-216b | INV-1/3/12/SC-001 ✅ | Done |
| FIX-220 | 2026-08-13 | Maximum update depth exceeded — useWorkflow.pipelineState sync effect fires 26× per SSE frame causing infinite re-render loop | `useEffect([pipelineState])` at page.tsx:1678 calls `runStore.updatePipelineState` → `project` → `setViewedState` → re-render → `pipelineState` is a new object (every SSE frame creates one) → effect fires again → loop. Fix: coalesce via `requestAnimationFrame` — each effect schedules an rAF, cancelling the previous; only the last pipelineState snapshot per animation frame (~16ms) actually calls `updatePipelineState`. Also added `syncPipelineStateOnly` to store (no-project variant) for future use. | `frontend/src/app/dashboard/page.tsx`, `frontend/src/hooks/useRunStateStore.ts` | Phase 44 (FIX-201 sync bridge), FIX-201 | INV-1/3/12/SC-001 ✅ | Done |
| FIX-219 | 2026-08-13 | od_ppt Concierge revision routes to `ppt_revision` (2 agents) instead of `od_ppt_revision` (1 agent) — FIX-216b regression; stale proposal correction added | FIX-216b's `_OD_BASE_MAP` unconditionally mapped `od_ppt → ppt` for ALL tiers, overriding `od_ppt_revision` with `ppt_revision`. Additionally stale durable `concierge_proposal` rows stored `target=ppt_output` for od_ppt runs. Fix: (1) remove `od_ppt` from the fallback map entirely — `od_ppt_revision` is entitled on all tiers that can launch od_ppt; (2) add stale-proposal correction: at confirm time remap `target=ppt_output → od_ppt_output` when `wr_type` is `od_ppt`. | `backend/app/api/run_commands.py` | Phase 14 (revision), FIX-216b | INV-1/3/12/SC-001 ✅ | Done  · **ROW RECONSTRUCTED 2026-08-13** — this row arrived on `dev` fused into the FIX-220 line with its id/date cells lost, so `check_ids.py` saw the id in use with no row. Split back out unchanged; the id and date are dev's own, from its `.knowledge/surface/INDEX.md`. |
| FIX-218 | 2026-08-13 | revision agents receive file as structured reference; filename shown on history replay; chip cleared after send; copy bar fixed; professional file bubble; filename in Context Received panel | (1) `_persist_chat_message` stripped `name` — blank on replay. (2) `RunAttachmentChips` showed mid-chat file chips. (3) Concierge prompt passive. (4) Send button disabled via stale ref. (5) Revision instruction had no framing. (6) `apply_steering` no-op on completed runs. (7) Copy bar disappeared via AnimatePresence gap. | `backend/app/api/run_commands.py`, `backend/app/agents/chat/concierge.py`, `backend/app/core/entitlements.py`, `frontend/src/hooks/useChatAttachments.ts`, `frontend/src/hooks/useRunChat.ts`, `frontend/src/components/chat/ChatAttachments.tsx`, `frontend/src/components/chat/RunChatLane.tsx`, `frontend/src/components/chat/MessageBubble.tsx`, `frontend/src/components/results/AgentDetailPanel.tsx`, `backend/tests/unit/test_run_commands_fix218.py` | Phase 30/31/33, KAN-170 | INV-1/3/12/SC-001 ✅ tests: 24 unit ✅ | Done  · **ROW RECONSTRUCTED 2026-08-13** — this row arrived on `dev` fused into the FIX-220 line with its id/date cells lost, so `check_ids.py` saw the id in use with no row. Split back out unchanged; the id and date are dev's own, from its `.knowledge/surface/INDEX.md`. |
| FIX-217 | 2026-08-12 | Chain context missing when chaining from prototype/ppt revisions; stale chain context; empty brief on wizard-chained launches | `get_chain_context` walked only ONE level up for `*_revision` runs — revision-of-revision stopped at v1 with no spec output. Fix: loop up ancestor chain until first non-revision base run (capped 20 hops). Also: clear stale `chain.context_block`; write `chain.brief` after awaiting `getChainContext`; fall back to `ctx.brief`; `setMainView('execution')` synchronously. FIX-216c: remove `!draft.brief` guard from od_prototype/od_ppt pending-run staging. | `backend/app/api/runs.py`, `frontend/src/app/dashboard/page.tsx`, `frontend/src/components/layout/DashboardLayout.tsx` | Phase 25, Phase 43 | INV-1/3/12/SC-001 ✅ | Done |
| FIX-216b | 2026-08-11 | od_prototype/od_ppt revision maps to wrong pipeline type — `od_prototype_revision` has no agents + excluded from hexaware tier | `_dispose_concierge_proposal` used `wr_type` verbatim → `od_prototype_output` → `od_prototype_revision` → no agents + not entitled. Fix: `_OD_BASE_MAP = {"od_prototype": "prototype", "od_ppt": "ppt"}` applied before constructing fallback target. | `backend/app/api/run_commands.py` | Phase 14, KAN-156 | INV-1/3/12/SC-001 ✅ | Done |
| FIX-215 | 2026-08-11 | Notification click always opened latest run instead of clicked run | `markCompleted` passed `undefined` as `workflowRunId`; background completions never stamped `workflowRunId`; `onViewResults` fell through to type-based lookup. Fix: `markCompleted` passes `pipelineState.pipelineRunId`; background runs stamp on `pipeline_complete`; `onViewResults` fetches by id via `getWorkflow`; storage key bumped v2→v3. | `frontend/src/components/layout/DashboardLayout.tsx`, `frontend/src/hooks/useNotifications.ts` | Phase 44, FIX-202..205 | INV-1/3/12/SC-001 ✅ | Done |
| FIX-214 | 2026-08-11 | Clarify action button labels updated to be action-neutral | Labels "Submit answers & start the build" / "Skip questions & start the build" implied build always follows clarify — wrong for all workflow types. New labels: "Submit answers & proceed" / "Proceed with auto recommendation". | `frontend/src/components/chat/InlineClarifyActions.tsx` | Phase 42 | INV-1/3/12/SC-001 ✅ | Done |
| FIX-213 | 2026-08-11 | Concierge system prompt overhaul — emojis, internals leakage, verbose answers, no proactive next-step, wrong run state label | Complete rewrite of `_compose_system_prompt` base blocks: RESPONSE RULES (no emojis, no internals, short answers); INTENT ROUTING (decisive, no interrogation); `run_summary` label guarded by `run_is_complete`. | `backend/app/agents/chat/concierge.py`, `backend/app/api/run_commands.py` | Phase 33, KAN-165 | INV-1/3/12/SC-001 ✅ | Done |
| FIX-212 | 2026-08-11 | "Start this workflow" chip persisted after clicking; revision `pipeline_not_entitled`; blank bubble on confirm; confirm button no feedback | Chain chip dismissed via `onSuggestion` but never removed from `proposals`. Confirm turn had no `message_id` → 404. Confirm text="" → blank bubble. No loading state on confirm button. | `backend/app/api/run_commands.py`, `frontend/src/components/chat/RunChatLane.tsx`, `frontend/src/hooks/useRunChat.ts` | Phase 33, Phase 43 | INV-1/3/12/SC-001 ✅ | Done |
| FIX-211 | 2026-08-11 | chat_router auto-approved gate / auto-submitted clarify on plain chat text | `PHASE_GATE_PAUSED`: any plain free-text with no `turn.action` defaulted to `action="approve"`. `PHASE_CLARIFY_WAITING`: plain text with no `responses` routed to `CHANNEL_ANSWERS`. Fix: add plain-text guard in both states → route to `CHANNEL_CONCIERGE` instead. | `backend/app/api/chat_router.py` | Phase 29 | INV-1/3/12/SC-001 ✅ | Done |
| FIX-210 | 2026-08-11 | Concierge proposal chips not appearing; revision confirm posting wrong run; revision UI not switching to new run | `chain_hints` had no `target_id`. Confirm POSTed to viewed run not proposal run. `revision_run_id` not wired to `onRevisionLaunched`. `_load_pending_proposal` needed original message id not optimistic id. | `backend/app/agents/chat/concierge.py`, `backend/app/api/run_commands.py`, `frontend/src/hooks/useRunChat.ts`, `frontend/src/components/layout/DashboardLayout.tsx`, `frontend/src/app/dashboard/page.tsx`, `frontend/src/providers/RunConnectionProvider.tsx` | Phase 33, Phase 43 | INV-1/3/12/SC-001 ✅ | Done |
| FIX-209 | 2026-08-11 | useSmoothText re-render loop; useMeasuredVirtualWindow scroll handler infinite loop | `useSmoothText` tick called `setState` every animation frame. `useMeasuredVirtualWindow` scroll handler triggered re-render loop. Fix: tick moved to `requestAnimationFrame`; scroll handler debounced with `useRef` guard. | `frontend/src/hooks/useSmoothText.ts`, `frontend/src/components/chat/runtime/useMeasuredVirtualWindow.ts` | Phase 31 | INV-3/SC-001 ✅ | Done |
| FIX-208 | 2026-08-07 | Notification panel shows 0/N agent progress for background concurrent runs | `AppHeader` received agent counts only from the viewed `pipelineState`. Background runs' progress was in `runStore` but never threaded to header. Fix: compute `runAgentsCompletedMap` from `runStore.get()` for all live runs; thread through `page.tsx → DashboardLayout → AppHeader`. | `frontend/src/app/dashboard/page.tsx`, `frontend/src/components/layout/DashboardLayout.tsx`, `frontend/src/components/layout/AppHeader.tsx` | Phase 44, FIX-201 series | INV-1/3/12/SC-001 ✅ | Done |
| FIX-207 | 2026-08-07 | useRunStream "Maximum update depth exceeded" — setCursor/setLastMessage on every SSE frame | Both declared as `useState` — every SSE frame called both setters causing a re-render storm. Fix: convert to `useRef`; only call `setState` when semantically meaningful. | `frontend/src/hooks/useRunStream.ts` | Phase 44 (SSE) | INV-1/3/12/SC-001 ✅ | Done |
| FIX-206 | 2026-08-07 | Concurrent background run agent frames contaminate Steps panel of clarify-paused run | Early-registration didn't check if previous viewed run had active `questionnaireData`. `StepsOverviewSpine` showed agent rows regardless of clarify state. Fix: skip `switchViewTo` during clarify; add `!hasClarify` guard in `StepsOverviewSpine`. | `frontend/src/app/dashboard/page.tsx`, `frontend/src/components/results/StepsOverviewSpine.tsx` | Phase 44, ISS-061 | INV-1/3/12/SC-001 ✅ | Done |
| FIX-205 | 2026-08-07 | Background run workflowRunId never stamped on notification — backgroundCompletedRunId lookup fails silently | `DashboardLayout` tracked `backgroundCompletedRunId` but never set `workflowRunId` on the matching notification. Fix: add `backgroundStartedRunId` on foreign `pipeline_start`; stamp `workflowRunId` on unbound notification; fallback lookup by `pipeline-{runId}` pattern. | `frontend/src/app/dashboard/page.tsx`, `frontend/src/components/layout/DashboardLayout.tsx` | Phase 44, FIX-202..204 | INV-1/3/12/SC-001 ✅ | Done |
| FIX-204 | 2026-08-07 | Duplicate notification entries — 2× Presentation, 2× User Stories when only 1 of each is running | Reactive `odProtoNotifCreated` effect and `handleRunPipeline` both called `addNotification` for the same run. Fix: set `odPptNotifId`/`odProtoNotifId` refs in handlers to pre-empt the reactive effect. | `frontend/src/components/layout/DashboardLayout.tsx`, `frontend/src/hooks/useNotifications.ts` | Phase 44, FIX-202..203 | INV-1/3/12/SC-001 ✅ | Done |
| FIX-203 | 2026-08-07 | Background run completion never marked on notification — stayed "running" after completing | Terminal effect fired only on viewed run's `pipelineState`. Foreign `pipeline_complete` frames processed by store but `markCompleted` never called. Fix: `page.tsx` sets `backgroundCompletedRunId` on foreign `pipeline_complete`; `DashboardLayout` calls `markCompleted` + toast. | `frontend/src/app/dashboard/page.tsx`, `frontend/src/components/layout/DashboardLayout.tsx` | Phase 44, FIX-202 | INV-1/3/12/SC-001 ✅ | Done |
| FIX-202 | 2026-08-07 | Notification panel refactor: localStorage persistence, per-item dismiss, toast run navigation, duplicate/stale entry elimination | No persistence, no per-item dismiss, toast nav went home not run, entries stacked indefinitely. Complete refactor: `useNotifications` hook with localStorage; `CompletionToast` navigates to run; `NotificationPanel` shows per-item dismiss; integrated into `DashboardLayout`/`AppHeader`. | `frontend/src/hooks/useNotifications.ts`, `frontend/src/components/ui/NotificationPanel.tsx`, `frontend/src/components/ui/CompletionToast.tsx`, `frontend/src/components/layout/DashboardLayout.tsx`, `frontend/src/components/layout/AppHeader.tsx`, `frontend/src/hooks/useNotifications.fix202.test.tsx` | Phase 44 | INV-1/3/12/SC-001 ✅ | Done | ISS-061 continued: concurrent run cross-contamination — agents/clarify questions/review gates from one run appearing in another; per-run state store (useRunStateStore viewport pattern); agent progress not updating after clarify submit/gate approval; infinite re-render loop (setCursor + setViewedState); duplicate toast keys; AppHeader LIVE_STATUSES missing "analyzing" | PRIMARY: shared `pipelineState` reducer in `useWorkflow` + shared `questionnaireData`/`reviewGateData` React state causes all concurrent runs to bleed into each other's UI. SECONDARY: `launchPendingRef` in `isForActiveRun` blocked agent frames; `pipeline_start` early-registration didn't set store viewport; `handleSelectWorkflowRun` set refs async not sync; replay skip used stale store state; `project()` created new ref on every frame; toast dedup missing; "analyzing" absent from LIVE_STATUSES. | `frontend/src/hooks/useRunStateStore.ts`, `frontend/src/app/dashboard/page.tsx`, `frontend/src/components/layout/AppHeader.tsx`, `frontend/src/components/layout/DashboardLayout.tsx`, `frontend/src/app/dashboard/liveRunSwitch.fix201.test.ts`, `frontend/src/app/dashboard/freshLaunchTranscriptReset.source.test.ts` | Phase 44 (DEF-44-12-4 concurrent run isolation), ISS-061, KAN-168 | INV-1 ✅ (no kernel edits) INV-3 ✅ (frontend-only) INV-12 ✅ (reuses handlePipelineMessage) SC-001 ✅ (run-id/status keyed, no workflow names) tests: 26 frontend ✅ | Done |
...
### FIX-263 — Revision Run Stream Page Missing Parent Messages and Concierge Responses

**Date:** 2026-08-27
**Triggered by:** `#velocity fix "please check while we triggered in revision pipeline, why the run chat is not showing previous message and concierge response at all on the stream page"`

#### Root Cause

T11's cold-mount fast-path (liveRunIds guard in `page.tsx`) fetched the revision run's durable events via `getRunEvents`, then branched on `hasChatFrames`:

```javascript
const hasChatFrames = durableFrames.some(f => f.type === "chat_message" || f.type === "chat_reply");
if (hasChatFrames) {
  seedRunChatTranscript(durableFrames, false);   // ← wipes parent messages
} else {
  // family seed — was unreachable once "Revision started" existed
}
```

`chat_narrator.py` emits a `chat_reply` "Revision started" card early for every `*_revision` pipeline. By the time T11's `getRunEvents` resolved, this card was already in `run_events` → `hasChatFrames = true` → `seedRunChatTranscript(revisionRunOnlyFrames)` wiped the parent run's messages and showed only "Revision started".

The `else` family-seed branch (which would have fetched and ordered all members) was therefore unreachable in practice.

Trace: revision triggered → `router.push(routes.runStream(revisionId))` → T11 fires → `getRunEvents(revisionId)` → narrator card in events → `hasChatFrames=true` → `seedRunChatTranscript(revisionOnly)` → parent messages gone.

#### Phase Context
- **Phase(s) involved:** Phase 44 (SSE transport / 015-frontend-routing / FIX-298b)
- **Deleted code verified (not resurrected):** No deleted code involved — the fix removes the hasChatFrames predicate and consolidates into a single always-family path
- **Locked decisions respected:** Family-ordered transcript (FIX-174 ordering) preserved; INV-12: reuses durableFrames for current run to avoid duplicate fetch

#### Fix Applied
| File | Change | Why |
|------|--------|-----|
| `frontend/src/app/[...view]/page.tsx` | Removed `hasChatFrames` branch; always call `getRunFamily(runId)` and seed from all members in order; reuse `durableFrames` for the current run's slot; fallback to revision-only on family-fetch failure | Parent messages must always be included; the "Revision started" card must appear at the end as the revision member, not as the sole content |

#### Invariants Verified
- **INV-1** (no pipeline_type branches): not affected — frontend only
- **INV-3** (golden parity): not affected — frontend chat seeding only
- **INV-12** (no duplication): reuses `durableFrames` for current run slot — no extra fetch
- **SC-001** (zero engine edits): not affected

#### Verification
- TypeScript diagnostics: no issues found.
- Logic trace: `getRunFamily(revisionId)` → backend walks UP to root via `_compute_root_ids`, BFS-down returns `[parentRun, revisionRun]` → seed collects parent's `chat_message`/`chat_reply` + revision's "Revision started" in order → full family transcript visible.
- Fallback: if `getRunFamily` fails or returns no members, falls back to revision-only frames (same as pre-fix hasChatFrames=true behaviour).

#### Notes
The `hasChatFrames` predicate was originally added (FIX-298b) to guard against wiping an existing transcript with an empty replacement for just-started runs. That concern is now addressed differently: the family fetch always includes the parent's messages, so there is always meaningful content to seed. Fresh non-revision launches have no family members other than themselves, so the path degrades correctly (memberIds = [currentRunId], falls back to current run's frames only).

---

### FIX-262 — Prototype/PPT Wizard Launch Flashes /dashboard Before Navigating to Run Stream

**Date:** 2026-08-26
**Triggered by:** `#velocity fix "found one issue when i triggered pipeline from prototype and ppt launch wizard. It first show /dashboard page for few seconds then goes to run /stream page. it should directly go to the stream page."`

#### Root Cause

`LaunchWizard.handleLaunch` (LaunchWizard.tsx:745) stages the run in sessionStorage (`prototype.pending` / `ppt.pending`) then calls `router.push(routes.home())` = `/dashboard`. `page.tsx` mounts at `/dashboard`, parses `screen = "home"`, and passes `initialMainView="home"` to `DashboardLayout` via `initialMainViewFor(parsedView)`.

`DashboardLayout`'s `mainView` useState initializer (DashboardLayout.tsx:494-510) checks `initialMainView` FIRST:
```ts
if (initialMainView) { return initialMainView; }  // "home" wins — guard below never reached
// sessionStorage check for prototype.pending / ppt.pending:
if (typeof window !== "undefined" && (sessionStorage.getItem("prototype.pending") || ...)) {
  return "execution";
}
```

Since `initialMainView = "home"` is truthy, the sessionStorage guard is bypassed. `mainView` starts as `"home"`, and the user sees the home screen for the ~1–3 seconds it takes for SSE to connect and the pending-run effects to call `setMainView("execution")`.

Trace: wizard submit → `router.push("/dashboard")` → `page.tsx` screen=`"home"` → `initialMainView="home"` → `DashboardLayout.useState` returns `"home"` (guard bypassed) → home screen renders → SSE connects → `setPendingOdProtoParams` → `setMainView("execution")` → `pipeline_start` → `router.push(routes.runStream(id))`

#### Phase Context
- **Phase(s) involved:** Phase 44 (SSE transport / 015-frontend-routing)
- **Relevant register section:** T9 `initialMainView` re-sync fix (015-frontend-routing)
- **Deleted code verified (not resurrected):** No deleted code involved — frontend routing only
- **Locked decisions respected:** initialMainView-takes-precedence contract preserved; fix is at the call site in page.tsx, not inside DashboardLayout

#### Fix Applied
| File | Change | Why |
|------|--------|-----|
| `frontend/src/app/[...view]/page.tsx` | `initialMainView` prop is now computed by an IIFE that checks `sessionStorage` for `prototype.pending` or `ppt.pending` when `initialMainViewFor(parsedView)` would return `"home"`, and returns `"execution"` instead | `initialMainView="execution"` bypasses the home screen; DashboardLayout's useState initializer already maps this to execution view immediately |

#### Invariants Verified
- **INV-1** (no pipeline_type branches): not affected — frontend routing only, no engine changes
- **INV-3** (golden parity): not affected — frontend only, no backend output changes
- **INV-12** (no duplication): fix reuses the same sessionStorage key names already used by the DashboardLayout guard and the wizard itself; no dual mechanism
- **SC-001** (zero engine edits for new workflows): not affected

#### Verification
- TypeScript diagnostics: `get_diagnostics` on `frontend/src/app/[...view]/page.tsx` → no diagnostics found.
- Logic trace: with `prototype.pending` in sessionStorage, `base === "home"` + `typeof window !== "undefined"` + `sessionStorage.getItem("prototype.pending")` truthy → returns `"execution"` → `DashboardLayout` mounts with `mainView = "execution"` immediately.
- Regression check: non-home routes (`"execution"`, `"history"`, etc.) — `base !== "home"`, guard not reached, unaffected. Normal home navigation with no pending run — `sessionStorage.getItem` returns null, `base = "home"` returned as before.

#### Notes
The DashboardLayout sessionStorage guard (lines 503-510) was added for exactly this problem but predates the T9 `initialMainView` prop and re-sync effect from 015-frontend-routing. The prop takes precedence over the guard by design (T6 URL-wins rule). The correct layering is: route-derived `initialMainView` wins, but page.tsx is the right place to inject the wizard-pending exception since it already owns all the URL→prop mapping logic.
...
| FIX-251 | 2026-08-11 | `update_specs` regenerated the spec from scratch instead of revising it, and every resumed run rebuilt its planning context from a 200-char stub — the degraded spec then drove the planner and the finished prototype, with no UI signal | Not data loss, a routing bug: spec v1, the planning_context row and the clarifications row were all in `ectx.artifacts` when the degraded prompt was composed. (D1) `_run_spec_revision_sub_pipeline` set only `ectx.spec_revision_context = analysis_report`; `prototype-specify` declares `consumes: []` + `tools: []`, so it had neither a routing nor a filesystem channel to the spec the prompt told it to "preserve unchanged sections" of. (D2) a resumed run took `skip_planner` -> `_default_planning_context` (`user_message[:200]`, every list empty): block collapsed 4,591 -> 291 chars across 16 of 18 dispatches. (D3) the revision reused the IDENTICAL checkpoint `thread_id`, so the only thing that had ever carried the prior spec was the LangGraph conversation — empty after a restart on sqlite's InMemorySaver. | `backend/agents/execution_engine/engine.py`, `backend/agents/execution_engine/context.py`, `backend/tests/agents/test_spec_revision_context.py`, `backend/tests/agents/test_restart_resume.py` | quick-260811-mxg | INV-1/3/12/SC-001 ✅ | Done · tests: 11 unit ✅ · LIVE-PROVEN on Bedrock (run 5ecb990f)  · **RENUMBERED from FIX-217** on the 2026-08-13 `dev` merge — dev had independently allocated FIX-217 to a different fix; commits on this branch still say FIX-217. |
| FIX-252 | 2026-08-11 | A second "Update the Specs" at the RE-OPENED analyze gate was a silent no-op — the gate closed, instantly re-opened on identical content, no revision ran, and the build proceeded from the unrevised spec (defect B); and a nested revision pass minted DUPLICATE `:rev1` checkpoint threads while zeroing the outer pass's counter and analysis report mid-flight (defect A) | (B) Of the three `_gate_update_specs` branches only two called the sub-pipeline; the third — the pending-re-open short-circuit — merely re-seeded `spec_revision_pending_output` and `break`ed. It was written to CLOSE a revision, not to start one, and the FE sends the analyzer's own output as the report (`InlineGateActions.tsx:163`), so the re-opened gate showed identical content and the no-op was invisible. (A) `spec_revision_attempt` is a per-`_run_agent` LOCAL, so the analyze re-run inside a pass — itself gated, and `_run_review_gate` acts on `action == "update_specs"` without ever consulting `update_specs_eligible` — re-entered the consumer with its own counter at 1; both levels published index 1 and minted the same `:rev{N}`. The sub-pipeline's `finally` then cleared to zero rather than restoring, stripping the outer pass while it was still on the stack. | `backend/agents/execution_engine/engine.py`, `backend/agents/execution_engine/context.py`, `backend/tests/agents/test_spec_revision_cycles.py`, `backend/tests/agents/test_redo_gate_safety.py` | quick-260811-si4 (successor to FIX-217 / quick-260811-mxg; closes its carry-forward ISS-064 + ISS-051) | INV-1/3/12/SC-001 ✅ (INV-12: two duplicate consumer copies DELETED for one shared `_consume_update_specs`, leaving exactly 1 `_run_spec_revision_sub_pipeline` call site, net -139 lines in engine.py; INV-3: goldens + lint-imports byte-identical to `bf51a170`) | Done · tests: 5 new unit + 3 revived guards ✅ · live Bedrock acceptance DEFERRED (recipe in `260811-si4-BASELINE.md`)  · **RENUMBERED from FIX-218** on the 2026-08-13 `dev` merge — dev had independently allocated FIX-218 to a different fix; commits on this branch still say FIX-218. |
| FIX-253 | 2026-08-12 | `update_specs_eligible` was advertised but never enforced — the review gate ran the spec-revision sub-pipeline at firings that had published the affordance as unavailable, and `gate_key` names a gate SLOT rather than a FIRING so REPLAYING a legitimate earlier click was enough (no crafting). Harm was corruption and cost, not redundancy: at the two DEFAULT-gated agents (`index < 2`) it revised nothing yet blanked the agent's output and re-opened the gate empty; at a build/validate gate it re-ran the build (~$3.68 vs ~$0.03, no rate limiting). Live-observed on run `d5dbc9f2` (gate advertised `eligible=0`, action accepted 6.05s later). The declared `gates:[human]`/`[approval]` path also had NO `_gate_update_specs` branch, so the signal leaked to the SSE wire AND fell through to GATE_PASS — a silent approve. | A modelling decision, not a mechanism: KAN-101 modelled `update_specs_eligible` on `redoable`, but `redoable` is genuinely advisory (a redo re-runs the same agent, always meaningful) while `update_specs` dispatches a sub-pipeline whose targets are chosen POSITIONALLY from the gated agent's index — so the same action means "re-run specify/plan/analyze" at one gate and "re-run plan/analyze/build" at the next. The flag that encodes WHICH gate is which was classified as cosmetic and discarded: it was a parameter of `_run_review_gate` (`engine.py:5450`), stamped onto the outbound event (`:5516`), then never read again — the branch at `:5576` keyed purely on the client-supplied `action` string. | `backend/agents/execution_engine/engine.py`, `backend/agents/capabilities/gates/human.py`, `backend/agents/capabilities/gates/approval.py`, `backend/app/api/run_engine.py`, `backend/app/api/run_commands.py`, `backend/tests/agents/test_update_specs_enforcement.py`, `backend/tests/unit/test_update_specs_ingress_fence.py`, `backend/tests/agents/test_spec_revision_cycles.py` | quick-260812-12t (closes ISS-053, reversing quick-260811-si4's T-si4-01 accept-as-designed on evidence; files ISS-070 + ISS-071) | INV-1/3/12/SC-001 ✅ (INV-3/INV-12: the eligibility RULE stays solely in `_update_specs_eligible` — the engine layer reads the parameter it produced, the ingress layer reads the value it published; neither restates it, so no third copy exists. SC-001: no workflow name or agent id introduced. INV-3: goldens + lint-imports identical to `d62bfe4d`) | Done · tests: 18 new unit (5 engine + 13 ingress) ✅ · all seen RED before the fix · live Bedrock acceptance DEFERRED (end-of-milestone rule)  · **RENUMBERED from FIX-219** on the 2026-08-13 `dev` merge — dev had independently allocated FIX-219 to a different fix; commits on this branch still say FIX-219. |
| FIX-254 | 2026-08-12 | One "Update the Specs" click opened the analyze gate THREE times and nothing on the wire told the firings apart. `gate_key` is `f"{run_id}:{agent_id}"` — a gate SLOT, not a FIRING — so the analyze re-run INSIDE the revision sub-pipeline and the gate re-opened after that pass returned arrived with the same key AND byte-identical output (live run `5ecb990f` seq 24652/24655: 11,974 chars, sha1 `0136795fc392`, 5 ms apart, zero events between) while carrying OPPOSITE affordances (`update_specs_eligible` False vs True). The user could not tell which gate they were on (84 s of hesitation on the duplicate, live), and the ambiguity produced a wrong `major` issue (ISS-062, closed as a misdiagnosis). Second, untested harm found by the investigation: `InlineGateActions` reset its one-action latch on `[output, gateKey]` and NEITHER changes between the two firings, so the second gate rendered with every action disabled. | A modelling gap, not a mechanism: the sub-pipeline was DESIGNED headless — its own comment says the review gate "is not expected during a revision cycle" (`engine.py:5310-5313`) — and the "re-open the gate afterwards" step (`:5440` → `:3463-3482`) exists precisely to give the user the review the pass would not. But the pass re-runs each agent through the full `_run_agent` path for behaviour reuse (INV-12), so every one of them DOES gate. The re-open therefore re-presents a review that already happened, and the event vocabulary had no field that could distinguish a gate FIRING from a gate SLOT. | `backend/agents/execution_engine/engine.py`, `backend/tests/agents/characterization/_normalize.py`, `backend/tests/agents/test_gate_revision_discriminator.py`, `backend/tests/agents/test_spec_revision_cycles.py`, `backend/tests/agents/test_restart_resume.py`, `frontend/src/types/index.ts`, `frontend/src/app/dashboard/page.tsx`, `frontend/src/hooks/useRunStateStore.ts`, `frontend/src/components/layout/DashboardLayout.tsx`, `frontend/src/components/chat/RunChatLane.tsx`, `frontend/src/components/results/StepsOverviewSpine.tsx`, `frontend/src/components/chat/InlineGateActions.tsx` | quick-260812-1nz (closes ISS-052's disambiguation half; carries ISS-062's valid substance; hands ISS-063 its durable signal; files ISS-072 for the deferred suppression) | INV-1/3/12/SC-001 ✅ (INV-12: ONE resolver `_revision_stamp` with three callers — no new counter, no new state, no migration; the values are the existing `revision_high_water` / `revision_attempt`. SC-001: no workflow name, no agent id, no `pipeline_type` branch on either side, guarded by a source grep. INV-3: goldens + lint-imports identical to `28c0111c`) | Done · tests: 6 new backend unit + 2 new frontend ✅ · all seen RED before the fix · **NOT a full close of ISS-052** — it removes the ambiguity, not the extra approval; the collapse half is ISS-072 · live Bedrock acceptance DEFERRED (end-of-milestone rule)  · **RENUMBERED from FIX-220** on the 2026-08-13 `dev` merge — dev had independently allocated FIX-220 to a different fix; commits on this branch still say FIX-220. |
| FIX-221 | 2026-08-12 | The violet "Spec Revision Cycle N" banner was **dead on every path** for `od_prototype`, not merely absent on reload as ISS-063 filed it. FIX-164's arming predicate (`page.tsx:1646`) required EVERY agent in the roster to be `"done"` while the run was still `isRunning`, but the update_specs gate sits on `prototype-analyze`, agent 2 of 5 — `prototype-build`/`prototype-validate` are still `"idle"`, and the only three sites that assign `status:"done"` to the rest also set `isRunning:false` in the same update. The arm was structurally unsatisfiable, so the counter never left 0 in ANY session. Two further sufficient mechanisms sat behind it: the detector read `pipelineAgentsRef`, refreshed only by a post-commit effect, so it was frozen for the whole synchronous REST replay loop (`page.tsx:2257-2269`); and the trailing resume `pipeline_start` (run `d5dbc9f2` seq 24791, same run id) landed LAST on every replay and zeroed the counter. Dead since `219702a4` (2026-08-03), on the flagship workflow, with zero test coverage. | A modelling decision, not a mechanism: FIX-163/164 modelled "a revision is happening" as **live-session state assembled by a detector** — an arm/consume state machine over refs — rather than as a **property derivable from the accumulated event history**. Once the answer depends on transient arming, correctness depends on scheduling (which frame arrives when, whether React committed in between), and every replay path is a different schedule. The three failures are one root cause seen from three angles. | `frontend/src/types/index.ts`, `frontend/src/hooks/useWorkflow.ts`, `frontend/src/hooks/useRunStateStore.ts`, `frontend/src/app/dashboard/page.tsx`, `frontend/src/hooks/useWorkflow.specRevisionCount.test.ts`, `frontend/src/app/dashboard/deadRevisionRefs.source.test.ts` | quick-260812-2ci (closes ISS-063; shipped together with FIX-222/ISS-065 as ONE change; files ISS-075 + ISS-076) | INV-1/3/12/SC-001 ✅ (INV-12: the arm/consume machinery is DELETED — `revisionCycleArmedRef`, `setSpecRevisionCountRef`, `pipelineAgentsRef` and the `page.tsx` `useState` are gone, and the value lands in the `specRevisionCount` field that already existed dead at `useRunStateStore.ts:81/170`; ONE writer, in `handleFrame`. SC-001/INV-1: keyed on restart counts alone — no agent id, no workflow name. INV-3: frontend-only; goldens + lint-imports identical to `b13d5c33`) | Done · tests: 15 new frontend ✅ · all 15 seen RED before the fix · **source of truth chosen deliberately**: derived from the event stream, NOT FIX-220's `revision_cycle` — that pair rides only on gate events, and the durable log shows a **94-second window** (seq 9285 → 13353) between the revision starting and the first gate frame, which is exactly the window the banner exists to cover · live Bedrock acceptance DEFERRED (end-of-milestone rule) |
| FIX-222 | 2026-08-12 | No UI surfaced PER-ARTIFACT versions inside a run. `artifact_refs` durably stores every version and `GET /api/runs/{id}/artifacts?kind=X&include=content` already returns each one WITH its body — verified live on `:8010` (run `d5dbc9f2`: spec v1/v2/v3 at 30081/36729/40052 chars, one call, HTTP 200) — but the only FE caller was `WorkflowHistory.tsx:303`, pinned to `kind="clarifications"`. Worse, the evidence is actively destroyed in FE state: `useWorkflow.ts` resets an agent's `output` to `""` on every `agent_start` (FIX-039), so the moment the spec agent re-runs during an update_specs cycle the only in-memory copy of v1 is gone and the panel shows v3 with no label and no way back. **This invisibility is why FIX-217 shipped undetected.** | A gap between two axes that were never distinguished on screen: the only version control users had is the run-header chip, which walks the run FAMILY (`parent_run_id`) per locked decision D2. Three in-run revision cycles move that chip not at all, so the product taught users that "Version" means something it does not, and the axis that DID change (`artifact_refs.version`, per `(run_id, kind)`) had no control at all. Data, API and read-scope all already existed; only the consumer was missing. | `frontend/src/components/results/ArtifactVersionPicker.tsx`, `frontend/src/components/results/AgentDetailPanel.tsx`, `frontend/src/components/results/AgentThinkingTab.tsx`, `frontend/src/components/results/ArtifactVersionPicker.test.tsx`, `frontend/src/components/results/AgentDetailPanel.artifactVersions.test.tsx` | quick-260812-2ci (closes ISS-065; shipped together with FIX-221/ISS-063 as ONE change) | INV-1/3/12/SC-001 ✅ (INV-12: reuses `getRunArtifacts` and the existing `ReadOnlyVersionBanner` — no second banner, no second artifact read path; the Concierge's `list_refs`/`get_ref` and the REST route both still bottom out on `ScopedStore`. SC-001/INV-1: grouping keys on the generic `producer_agent` field, labels come from the `kind` string as data. INV-3: zero backend files, zero new event types, alembic head stays 0030; goldens + lint-imports identical to `b13d5c33`) | Done · tests: 12 new frontend ✅ · all 12 seen RED before the fix · dedupe keys on `content_hash` because the typed-kind and `produces`-alias series do NOT stay in lockstep (`html_file` v23 vs `prototype-build` v11) · content fetched lazily + memoised (measured 2.89 MB for `html_file&include=content`) · `resp.artifacts ?? []` is load-bearing — `mockApi.ts:708` returns `json({})` for unmatched `/api/**` · live Bedrock acceptance DEFERRED (end-of-milestone rule) |
| FIX-223 | 2026-08-12 | **The 5 characterization event goldens — the project's INV-3 parity oracle — had been red on `dev` for weeks, so INV-3 could only be checked by diffing failure counts against a hand-captured baseline.** Root-caused to exactly 3 causes (proven exhaustive by counterfactual: undoing all 3 makes every one of the 5 files byte-identical), each adjudicated separately per the ISS-029 method. **C1 (all 5) — STALE NORMALIZER, not a stale golden:** `c71f3d9c` added an additive `pipeline_start.resume_offset`; it is 0 on every non-resumed run and the harness never resumes, but a key whose value is 0 is still a NEW key and the snapshot compares whole canonical-JSON dicts. **C2 (`prototype`, `od_prototype`) — a REAL REGRESSION; the golden was RIGHT:** `f608e4bc` (FIX-190) appended an `accessibility` guardrail to 4 prototype-family agents, and in `prototype-build/AGENT.md` the `+` line landed ON TOP OF `icon: "🏗️"` (the other 3 files are pure `1 +` insertions; this one is `2 +-`), so the agent fell through to the loader's `🤖` default — a user-visible UI regression in `pipeline_start.agents[]` + both `agent_start`s that config review, code review and CI all missed. **C3 (`prototype_revision`) — a genuinely STALE GOLDEN:** `fd2ad8f1` (FIX-185) deliberately set `planner: skip`, removing `planner_start`/`planner_complete`/`gate_status`. | C1: the author of `c71f3d9c` skipped the project's mandatory 3rd step for an additive parity-neutral key (6 prior precedents in `_VOLATILE_STRIP_KEYS`), and the emit-site comment asserts "0 on every normal run → byte/event-identical (INV-3)", which is FALSE as written and is what misled them. C2: a text-editor clobber, not a decision — sweep proved `prototype-build` was the ONLY 1 of 86 AGENT.md files missing an icon. C3: the manifest change was correct and tested; only the golden was never regenerated. | `backend/tests/agents/characterization/_normalize.py`, `backend/agents/prompts/prototype-build/AGENT.md`, `backend/tests/agents/test_loader.py`, `backend/tests/agents/characterization/golden/prototype_revision.events.json` | quick-260812-35u | INV-3 ✅ (restored, not weakened) · SC-001 ✅ (no workflow-name branch) · INV-12 ✅ | Done · tests: 1 new unit pin ✅ (seen RED before: "1 of 86 ... ['prototype-build']") · **goldens 5 failed/5 passed → 10 passed/0 failed** vs `f5f2f7c6` · C1+C2 touched **ZERO golden bytes**; only `prototype_revision.events.json` was regenerated, in its own revertable commit `a6b2097e`, strictly AFTER C1 so `resume_offset` could not be baked in · regeneration fully accounted: 33 deletions / **0 insertions**, 17→14 events, removed exactly {planner_start, planner_complete, gate_status}, the 14 survivors byte-identical and same-order · REJECTED regen-only (would have pinned the C2 UI regression as expected behaviour — the exact failure ISS-029 rejected) |
| FIX-224 | 2026-08-12 | **The `agents.capabilities must not import the execution kernel or the web layer` import-linter contract had been BROKEN since 2026-07-20**, so Ports & Adapters was at 3 kept / 1 broken against a documented intended state of 4/0. A single line — `agents/capabilities/strategies/task_loop.py:562`, `from agents.execution_engine.od_context import get_example_html` (the KAN-103 `template.html` revision seed) — broke the contract on BOTH forbidden roots at once, because `od_context.py:26` itself does `from app.services import od_loader`. A sweep confirmed it was the only violation in the entire capability tree. | Introduced by `26bcaacb` (KAN-102/103) as a **one-off bypass of a seam that already existed**: `KernelServices.template_example()` (`kernel_services.py:1163`) → `engine._load_template_example()` (`engine.py:9040`) → the same `get_example_html`, same `EXAMPLE_MAX_CHARS` cap, same swallow-and-return-`None` policy — and the sibling capability `context_providers/opendesign.py:128-135` was already consuming it correctly. It survived ~3 weeks because the contract lived ONLY in `lint-imports`, a separate binary that must be invoked from `backend/` (from the repo root it prints "Could not read any configuration" and exits, which reads as a pass). | `backend/agents/capabilities/strategies/task_loop.py`, `backend/tests/agents/test_banned_patterns.py` | quick-260812-35u | Ports & Adapters ✅ (4 kept / 0 broken restored) · INV-12 ✅ (used the existing port, did not copy the bytes) | Done · tests: 3 new unit pins ✅ (boundary scanner seen RED before, naming `task_loop.py:572`) · **lint-imports 3 kept/1 broken → 4 kept/0 broken** vs `f5f2f7c6` · REJECTED amending the contract to record the violation (pins the architecture break) and REJECTED threading `example_html` through the `od_context` dict (a second way to the same bytes, INV-12) · blast radius checked: `_write_reference_files` has one caller and no test fake sets a `template_id`, so the seed block is inert in tests |
| FIX-225 | 2026-08-12 | **Three defects on one reopen-replay path, two of them in the same expression — the "Spec Revision Cycle N" banner read 5 for a run with 2 revisions, and the reopened run's trace repainted as "0 / 5 agents" with every Steps row disabled.** (a) ISS-080: `agentStartCounts` was a bare `+= 1` with NO identity key, so it counted DELIVERIES; a history reopen replays the durable log once over REST and again over SSE, and the trailing same-run `pipeline_start` fired `resetReplayState` mid-replay (24 783 event ids discarded, measured), voiding the CR-05 dedup that was the counter's only protection — `{specify:6, plan:4, analyze:4}` against a durable truth of `{3,2,2}`. (b) ISS-081: `deriveSpecRevisionCount` took `max` over ALL agents while a per-task agent loop restarts ONE step once per task — run `6e38b9a7` (one revision, 11 `prototype-build` starts) yields 10, and a live prototype build would show a revision banner having performed none. (c) ISS-075: a `pipeline_start` re-announcing the run on screen REBUILT every agent as idle and zeroed `completedCount`, which also made the FIX-222 version picker unreachable (`StepsOverviewSpine` gates navigation on `status !== "idle"`). | One missing concept, three symptoms: the FE had no predicate for *"this `pipeline_start` re-announces the run I am already showing"*. FIX-221 introduced it for exactly ONE field (`useWorkflow.ts:306-309`) instead of promoting it, so the same trailing frame still wiped the dedup seen-set and still rebuilt the roster. Underneath that: two accumulators trusted an external guarantee instead of carrying their own identity — the restart history assumed "each frame arrives once", and the roster assumed the engine's `resume_offset` was a count of completed agents when it is the FIRST INCOMPLETE step (0 for a run parked at a gate on step 0). | `frontend/src/hooks/useWorkflow.ts`, `frontend/src/hooks/useRunStateStore.ts`, `frontend/src/app/dashboard/page.tsx`, `frontend/src/types/index.ts`, `frontend/src/hooks/useWorkflow.specRevisionCount.test.ts` | quick-260812-4ss | INV-3 ✅ (fresh-run path byte-identical) · INV-12 ✅ (`agentStartCounts` REPLACED, not shadowed; one predicate, not a second) · SC-001/INV-1 ✅ (the revision head is POSITIONAL — the first step of the compiled roster; no agent id, no workflow name, no artifact kind) | Done · tests: 12 new frontend cases (21/21 in file, 79/79 across the reducer+replay suites) ✅ · engine untouched, goldens 10/10 unmoved |
| FIX-226 | 2026-08-12 | **The ISS-065 artifact-version picker only moved the raw output — the prominent artifact CARD kept rendering the LATEST version, so the owner selected v1, saw v3's page grid, and reported "while changing the version there, i couldnt see the older version's content".** `AgentDetailPanel` computed `shownOutput = viewed?.content ?? agent.output` and passed it to `OutputPreviewSection` (`:869`) — correct — but handed the RAW `agent` to `SettledArtifactCards` (`:840`), which re-derived everything from `agent.output`: `parseSpecSections` (`:611`), `parseSpecOverview` (`:612`) and `<AnalysisPreview content={agent.output}/>` (`:687`). The card TYPE was picked the same way (`discriminateArtifact(agent.output)`, `:130`), so a mid-re-run agent — `agent.output` cleared by FIX-039 — rendered NO card at all even with v1 selected. | One screen, two answers to *"what am I looking at"*. The version selection lived in a local `shownOutput` that only one of four consumers read; every other consumer went back to the agent for the same information. Not a missing feature — a missing single source (INV-12), the same shape as FIX-225's "two accumulators, one truth". The card derivation additionally treated the artifact's TYPE as a property of the AGENT when it is a property of the ARTIFACT. | `frontend/src/components/results/AgentDetailPanel.tsx:608-616,687,745-753,770,856`, `frontend/src/components/results/AgentDetailPanel.artifactVersions.test.tsx` | quick-260812-77g | INV-12 ✅ (`shownOutput` derives FROM `displayed` — one value, not a second notion; `SettledArtifactCards` lost its `agent` prop so it CANNOT reach the latest output again) · INV-3 ✅ (frontend-only; backend tree byte-identical to `a48965a5`) · SC-001 ✅ (`discriminateArtifact` is a content sniff — no workflow/agent-name literal added) | Done · tests: 4 new frontend cases (TEST-010; 8/8 in file, 50/50 across the 5 results suites) ✅ · goldens 10/10 and lint-imports 4 kept / 0 broken, both IDENTICAL to `a48965a5` · live-proven read-only on run `5ecb990f` |
| FIX-227 | 2026-08-12 | **There was NO way for anyone — user or operator — to stop a running build once it had crossed a backend restart, and every un-stoppable `od_prototype` build costs 5–21M Bedrock tokens of the owner's money.** The whole escalation ladder was broken: UI Stop no-op; `POST /api/runs/{id}/cancel` → `HTTP 200 {"cancelled":true}` with the run still generating; SIGTERM ignored (local uvicorn has no `--timeout-graceful-shutdown`, and `SHUTDOWN_STOP_RUNS` is `False` anyway); and `kill -9` **finished the run**, because `restore_non_terminal_runs` re-adopts every non-terminal row on the next boot. Measured on run `d5dbc9f2`: **16,530,718 tokens total, of which 7,510,082 (45%) were billed AFTER the API answered `cancelled: true`**, with **zero** `pipeline_cancelled` events on the run — the entire dev DB contains exactly one such event ever, and its payload shows it came from a destructive `task.cancel()`, never a cooperative Stop. | Two defects, one upstream of every loop. (1) `ExecutionEngine._drive_resumed_stream` (`engine.py:8310-8332`) — the ONE funnel all four resume drivers reach the kernel through — passed `_execute_impl` **no `cancel_event` kwarg at all**, so it bound its `None` default (`engine.py:1096`) and all twelve cooperative guards, each written `if cancel_event and cancel_event.is_set()`, short-circuited. The guards were never wrong; the value was. Structural since `83cfff4c` (2026-06-11). (2) `run_engine._register_resume_task` minted an **orphan** `asyncio.Event` into `_CANCEL_EVENTS` for exactly those runs, on a premise its own comment asserted and that was false the day it was written — so `cancel_run` found one, set it, and reported registry mutation as cancellation. `849e536d`/KAN-88 (2026-06-30) had simultaneously **deleted the working destructive fallback**, and Phase 44's REST port dropped it entirely: after that there was no path, cooperative or destructive, by which a user could stop a resumed run. | `backend/agents/execution_engine/engine.py:65-78,889-896,8100-8116,8347-8364`, `backend/app/api/run_engine.py:503-521,524-541`, `backend/app/main.py:211-218`, `backend/app/api/run_commands.py:318-337,342-386`, `backend/app/api/run_shutdown.py:219-258,284,287-330`, `frontend/src/lib/api.ts:717-746` | quick-260812-7sk | INV-12 ✅ (`_resume_cancel_event` is the ONLY resume-path mint site — the orphan mint is superseded, not left alongside; `_drain_then_cancel` is the ONE escalation policy, shared with `stop_pipeline_drivers`, not a second one) · INV-3 ✅ (hook unset ⇒ `cancel_event=None` ⇒ byte/event-identical; goldens 10/10 unmoved) · Ports & Adapters ✅ (injected callback, no `app.api` import in the kernel; lint-imports 4 kept / 0 broken) · SC-001/INV-1 ✅ (keyed on `run_id` only — no workflow name, no agent id, no pipeline branch) | Done · tests: 9 new backend cases (TEST-011; 5 + 3 + 1 across three files, 99/99 across the changed area) ✅ · goldens **10 passed / 0 failed** and lint-imports **4 kept / 0 broken**, both IDENTICAL to `1ed94666`, no golden regenerated · `test_restart_resume.py` held at its 7 failed / 48 passed ISS-078 baseline · offline mutation proof reproduces the production symptom exactly |
| FIX-228 | 2026-08-12 | **"Request changes" (`redo`) at an agent's gate re-ran that agent with the user's instruction and WITHOUT the document the instruction was about, so the model wrote only the delta and the previous work was destroyed — the Task Planner asked to "add 2 more pages" came back with only 1 task, and every earlier task was lost.** Measured live on run `a0d17afa`: `task_list` v1 → v2 = 12,542 → 3,495 chars, headings 7 → 1, **0 of 7 kept**; the redo dispatch grew by exactly **106 bytes** — the instruction alone. Broken for EVERY agent (no branch at the publish site; a second live run shows it at two agents; the offline repro fails for three agents across two pipelines). | A **decision**, not a mechanism. P23 modelled a re-run as a REGENERATION and minted a fresh `:redo{N}` checkpoint thread specifically to stop the model seeing its rejected output — then the UI shipped that action as "Request changes" with an instruction box, so users write a *delta* and the engine gives the model no subject. FIX-217 hit the identical fork on the `update_specs` revision path and resolved it by pairing the fresh thread **with** an explicit prior-artifact injection; **P23 shipped only the fresh-thread half of that pair.** All three other channels are closed by construction: self-consumption is structurally impossible (`_filter_consumed_outputs` breaks on `upstream.id == spec.id`), the `:redo{N}` thread kills checkpoint replay, and gated authoring agents declare `tools: []`. | `backend/agents/execution_engine/engine.py` (new `_consume_redo`; the three `_gate_redo` call sites; the publish site in `_run_agent`'s redo loop; the `PRIOR ARTIFACT` block moved above `ADDITIONAL INSTRUCTIONS` in `_compose_context_message` + prose generalised), `backend/agents/execution_engine/context.py` (field doc: two publishers, one field) | quick-260812-8j4 | INV-1 ✅ / INV-3 ✅ (first non-redo dispatch byte-identical by sha256 `9f84b82e…` vs `742f0c6e`; goldens 10/10) / INV-12 ✅ (reuses FIX-217's field + renderer; **deletes** two consumer copies and one thin copy) / SC-001 ✅ (no agent id, no workflow name; proven generic across two pipelines) | Done · tests: 11 unit ✅ (8 observed RED first) · live Bedrock acceptance deferred to end-of-milestone |
| FIX-229 | 2026-08-12 | **A user who resumes a run parked at a review gate and then REJECTS at that gate had the run recorded in history as `completed`.** The rejection was silently discarded from the record and the deliverable appeared accepted. Reachable by anyone using Resume / Run Again on a `failed` or `cancelled` run that stopped at a gate — every workflow, not a family-specific defect. Found as red #7 of ISS-078's seven `test_restart_resume.py` failures; the other six were test defects (see TEST-013). | A broken assumption in the **reconcile**, not in the engine. `resume_supersedes` (`run_commands.py:602`, added by `bac1e556`/KAN-120) asked *"is there a `pipeline_complete` at a HIGHER SEQ than the last `pipeline_cancelled`?"* and read the answer as *"a later attempt succeeded"* — **two different questions.** On a rejection both events belong to ONE attempt: the engine emits `pipeline_cancelled`, `_run_agent` returns, and the dispatch loop falls through to the run's single `pipeline_complete` emitter. Durable tail, captured by probe: `…5:review_gate_ready \| 6:pipeline_cancelled \| 7:pipeline_complete` → `completed`. Before `bac1e556` the reconcile prioritised `cancelled` unconditionally and the trailing event was absorbed; **`bac1e556` did not create the contradictory tail, it removed the thing that was hiding it.** An attempt boundary is a `run_resuming`/`pipeline_start` row, so a completion only supersedes a cancellation when one lies BETWEEN them. | `backend/app/api/run_commands.py:592-621` (`_reconcile_terminal_status` — `resume_supersedes` tightened in place; one added clause, no second status decision) | quick-260812-97f | INV-1 ✅ (event types only — no workflow, agent or model name) / INV-3 ✅ (goldens 10/10, identical to `99fcf4a2`; no golden regenerated) / INV-12 ✅ (the predicate is tightened in place — no parallel status path) / Ports & Adapters ✅ (no import added; lint-imports 4 kept / 0 broken, identical to `99fcf4a2`) | Done · tests: 3 unit (the reconcile pin, both directions) + 1 end-to-end (`test_failed_run_with_open_gate_resumes_into_gate`) ✅, all seen RED first · offline-proven; **deliberately NOT live-proven — no run was launched, resumed or gate-approved** (one build is 5–21M tokens of the owner's money) |
| FIX-232 | 2026-08-12 | **Rejecting at a review gate did not stop the run — every remaining step still ran, the run reported `pipeline_complete`, and the fake `complete`/`completed` rows it left behind made a later resume skip every wave and produce NOTHING.** Fixed by giving the dispatch loop a terminal-event observation symmetric to its existing `agent_error` one, ending in `return`; plus defence-in-depth at the single fan-out cancel boundary. | Two parts. (a) The dispatch loop (`engine.py:2523`) observed ONLY `agent_error`. The inline gate's reject handler cancels the run, yields `pipeline_cancelled` and returns — but a generator `return` only ends THAT step, so the terminal event was forwarded and ignored and the outer per-step loop advanced to step i+1, falling through to the sole `pipeline_complete` emitter. (b) The only step-boundary termination check (`engine.py:2381`) reads `cancel_event`, which a gate rejection never sets — which is why FIX-227 was orthogonal. **The damage was never token spend** (every post-rejection worker short-circuits on `_run_agent`'s terminal guard at `:3329` and bills zero): it was the DURABLE RECORD. Each no-op worker was written `subagent_runs='complete'` and each wave `wave_runs='completed'`, and `wave_scheduler.py:251-256` skips completed waves on resume — so reject → restart → resume skipped both waves, produced zero files and recorded the run `completed`. Silent, permanent loss of a fan-out run's entire output. The fix is the exact sibling of the declared-gate handler WR-03 (`engine.py:2454-2479`), whose own comment describes this defect verbatim. `return`, not `break` — `break` falls through to the Step-5 terminal block which still emits `pipeline_complete`. Predates ISS-078/FIX-227/228/229; byte-identical at `3e80fcc4` (2026-07-19). | `backend/agents/execution_engine/engine.py`, `backend/agents/execution_engine/fanout.py`, `backend/agents/execution_engine/kernel_services.py`, `backend/tests/agents/test_restart_resume.py`, `backend/tests/agents/test_fanout_cancel.py`, `backend/tests/agents/test_fanout.py` | quick-260812-g1c | INV-1/3/12/SC-001 ✅ | Done · tests: 7 unit (4 dispatch-loop + 3 fan-out boundary) ✅ · 3 seen RED first with real output, the WR-03 guard proven by mutation · `test_restart_resume.py` 60 → 64 passed, `test_fanout_cancel.py` 6 → 9 passed · goldens 10 passed + 0 golden files modified and lint-imports 4 kept/0 broken, both identical to `c0bbb6e2`; the 11 pre-existing reds unmoved with identical ids. **Named limitation: the goldens compile `gate_agent_ids=[]` so no golden contains a gate event — they cannot detect this change and prove only that nothing else moved.** |
| FIX-231 | 2026-08-12 | **The live harness's review-gate wrapper could not accept the gate's own arguments — every gated harness drive raised TypeError and then reported the run COMPLETE without gating.** Fixed by forwarding the engine's kwargs through the wrapper, plus a static signature-drift guard over every `_run_review_gate` stub so the family ends. | `tests/agents/live_harness.py:672` pinned the four-parameter shape of 2026-06-11 and installed itself over `engine._run_review_gate` at `:683` unconditionally; the engine grew to TEN parameters and all FOUR call sites pass them by keyword (`engine.py:3397`/`:3504`/`:4440` + `kernel_services.py:1116`, the declared-gate delegate the ISS-074 row missed). The wrapper also forwarded only the same four positionally, so widening the def alone would silently DROP them. Broke at `f18de868` (2026-06-30, `redoable`). Invisible for 43 days because `.planning/TEST-REGISTER.md:85` excluded offline suites by FILENAME ("anything `*_live*`") and both affected files are offline-safe — coverage selection, not the credential gate the row claimed. | `backend/tests/agents/live_harness.py`, `backend/tests/agents/test_gates.py`, `backend/tests/agents/test_gate_stub_signature_drift.py` (new), `backend/tests/agents/test_live_harness.py`, `.planning/TEST-REGISTER.md` | quick-260812-fbk | INV-1/3/12/SC-001 ✅ | Done · tests: 4 unit (new guard) + 1 forwarding assertion ✅ · baseline 5 failed/3 passed → 8 passed/0 failed · goldens 10 passed + 0 golden files modified, lint-imports 4 kept/0 broken, 0 production modules changed, all identical to `fab9b646` |
| FIX-230 | 2026-08-12 | **Two model call-sites spent the owner's money without it appearing in the run's reported cost.** (1) The validation fix-loop re-invokes the build sub-agent on a `…:fixN` thread and drains its stream INTERNALLY; the drain was a bare `continue`, so **every `usage` event was discarded** — measured on the live run `fa66227a` that is ~929,000 input tokens, **6.9% of the run's input (+$0.176 of $2.550)**, and it recurs on every run whose validation fails. (2) `handoff/TestAgent.analyse` still called `llm.ainvoke` raw: **neither Bedrock-cached nor counted** — up to ~8k repo tree + N×10k test files per call. **This is a Phase 43-04 MISS, not a new regression:** that plan's own wording, "handoff Test(classifier)", mistook `classifier.py` (the coding-vs-test router) for `test_agent.py` (the real Test agent), so it routed the wrong module and closed. Part A of ISS-033 only — the Concierge/handoff *unbounded-input* half is ISS-092. | (1) `_run_validation_fix_loop` had no way to report usage: the loop is a non-yielding coroutine (it must re-emit nothing, so the UI shows ONE build per task), and the only accumulator reachable from it — `results` — is the wrong one, because `agents_completed = len(results)` would turn each fix attempt into a phantom agent. The right accumulator, `aux_token_usage` (43-04), was simply never threaded down. (2) 43-04 routed by plan wording rather than by call-site: a grep for the raw `ainvoke` would have caught it, a grep for "Test agent" did not. | `backend/agents/execution_engine/engine.py` (aux sink threaded into `_run_validation_fix_loop`; `usage` branch replaces the discard; `KernelServices(aux_usage_sink=aux_token_usage.append)`), `backend/agents/execution_engine/kernel_services.py` (ctor param → `run_validation_fix_loop`), `backend/app/agents/handoff/test_agent.py` (routed through the shared `cached_invoke`; the superseded local `_extract_text` DELETED), `backend/tests/agents/test_model_pricing.py` (dead INV-12 cost-site guard repaired) | quick-260812-9tq | INV-1 ✅ (keyed on `event["type"] == "usage"` and the KernelServices handle — no workflow, agent or model name) / SC-001 ✅ / INV-2 ✅ (the sink is a per-run constructor param, never engine-singleton state) / INV-3 ✅ (goldens 10/10, identical to `d24c576a`; **no golden regenerated** — and this was RE-RUN, not assumed: the prototype goldens DO fire the fix loop, 4 sub-agents, so their run totals genuinely move 220→460 input and stay golden-neutral only via `_VOLATILE_REQUIRED_KEYS`/`_VOLATILE_STRIP_KEYS`) / INV-12 ✅ (ONE fold into the EXISTING `aux_token_usage`; the discard is deleted, not paralleled; `TestAgent` reuses the existing `cached_invoke` seam and its shadowing `_extract_text` is removed) / INV-13 ✅ (no `create_deep_agent` outside the adapter) / Ports & Adapters ✅ (lint-imports 4 kept / 0 broken, identical to `d24c576a`) | Done · tests: 7 offline (5 new fix-loop + 2 new TestAgent) + 1 repaired INV-12 guard ✅, the RED-first ones seen failing first (`(220-220) == 240`, `TypeError: unexpected keyword argument 'usage_sink'`) · **offline-proven; deliberately NOT live-proven — no run was launched, resumed or gate-approved** (one build is 5–21M tokens of the owner's money). The dollar delta is computed read-only against the live `dev.db`. |
| FIX-020 | 2026-06-29 | Frontend UX polish batch — thinking tab, notification panel, scroll behaviour, and an XSS fix (KAN-71). **Exact FIX-020 scope within that batch is NOT recoverable**; the id survives only inside the commit body. | Not recorded. No card, no register prose, no in-code citation — commit body only. | not recoverable from the surviving evidence | commit `9bba1712` (KAN-71) | not recorded | **BACKFILLED 2026-08-12 by ISS-069 — evidence: commit body of `9bba1712` only.** Description reconstructed from the commit subject; the id's specific scope is unrecoverable and is deliberately NOT invented. |
| FIX-128 | 2026-08-05 | Chat-lane "Open in Steps" / "Open in Preview" never switched the right-hand tab. | Recorded in `.knowledge/cards/FIX-128.md`. | `frontend/src/hooks/useRunChat.test.ts` (cited at `:92`) | not recorded | not recorded | **BACKFILLED 2026-08-12 by ISS-069 — evidence: knowledge card + FIX-REGISTER prose mention + in-code citation.** Description quoted from the card summary. |
| FIX-129 | 2026-08-05 | Workflow reported "done" while the validation and build agents were still running in a loop. | Recorded in `.knowledge/cards/FIX-129.md`. | not recorded | not recorded | not recorded | **BACKFILLED 2026-08-12 by ISS-069 — evidence: knowledge card + FIX-REGISTER prose mention.** Description quoted from the card summary. |
| FIX-131 | 2026-07-xx | Safety net for run titles stored in the DB with `===` markers (with FIX-130, under KAN-116 Bug 3). | Titles persisted containing `===` separator markers leaked into the history list. | `frontend/src/components/history/WorkflowHistory.tsx:116` | KAN-116 (Bug 3) | not recorded | **BACKFILLED 2026-08-12 by ISS-069 — evidence: in-code citation only** (`WorkflowHistory.tsx:116`, "KAN-116 (Bug 3) + FIX-130 + FIX-131"). Date approximate; no commit or card carries this id. |
| FIX-261 | 2026-08-02 | EC2 host-config heredocs live in `reconcile-host-config.sh`, NOT `bootstrap-ec2.sh` — corrected, and pinned by a test. | The deploy heredocs had been assumed to live in the bootstrap script; editing there had no effect. | `backend/tests/unit/test_nginx_site_template.py:8` | commit `db668778` (KAN-134 SSE heartbeat batch) | not recorded | **BACKFILLED 2026-08-12 by ISS-069 — evidence: commit `db668778` + an explicit in-test citation.** The test docstring names the fix, which is how the scope was recovered. (merge 2026-08-17: re-homed from FIX-159, which the cognito-login side already used for the earlier fix that CREATED reconcile-host-config.sh; this row is the later correction to it) |
| FIX-183 | 2026-08-05 | One of three fixes shipped together: build-agent done badge, narrator crash, gate box in chat. **Which of the three is FIX-183 is NOT recoverable** — the commit writes them as the range `FIX-182/183/184`. | Not separately recorded. | not separately recorded | commit `adf60ff2` | not recorded | **BACKFILLED 2026-08-12 by ISS-069 — evidence: commit `adf60ff2` only.** Slash-list notation (`FIX-182/183/184`) is itself a drift source: the literal token `FIX-183` never appears, so a naive grep misses it. |
| FIX-184 | 2026-08-05 | One of three fixes shipped together: build-agent done badge, narrator crash, gate box in chat. **Which of the three is FIX-184 is NOT recoverable** — see FIX-183. | Not separately recorded. | not separately recorded | commit `adf60ff2` | not recorded | **BACKFILLED 2026-08-12 by ISS-069 — evidence: commit `adf60ff2` only.** Same slash-list caveat as FIX-183. |
| FIX-214 | 2026-08-11 | Clarify action button labels updated in the UI. | Not recorded beyond the commit subject. | frontend clarify action buttons (files not recorded) | commit `3309f6e8`; id also carried in `de42a7bf` | not recorded | **BACKFILLED 2026-08-12 by ISS-069 — evidence: commit subject `3309f6e8` ("ui: update clarify action button labels (FIX-214)").** This is one of the two ids the bookkeeping skill already cites as live-in-commits-but-absent-from-the-register.  · **SUPERSEDED by dev's canonical FIX-214 row** (this file, the dev-registered row) — this one is the ISS-069 backfill reconstructed from the commit subject; same fix, kept for backfill provenance. Merged 2026-08-13. |
| FIX-215 | 2026-08-11 | Notification click always opened the LATEST run instead of the clicked run. | `markCompleted` was not stamped with the originating run id, so the notification resolved to whichever run finished last. | `frontend/src/components/layout/DashboardLayout.tsx:469,618` | commit `c6a21e9f` | not recorded | **BACKFILLED 2026-08-12 by ISS-069 — evidence: commit subject `c6a21e9f` + two in-code citations.** Root cause recovered from the code comments ("pass backgroundCompletedRunId so markCompleted stamps workflowRunId"). Same family as ISS-060/061.  · **SUPERSEDED by dev's canonical FIX-215 row** (this file, the dev-registered row) — this one is the ISS-069 backfill reconstructed from the commit subject; same fix, kept for backfill provenance. Merged 2026-08-13. |
| FIX-233 | 2026-08-12 | **The Concierge was pre-fed the ENTIRE run through three unbounded read tools — one chat question re-fed 9,227,107 chars ≈ 2,306,776 tokens, on two runs 144% and 151% of everything the whole pipeline recorded — while its own model spend was discarded and therefore unmeasurable.** Replaced the read surface with seven bounded, on-demand tools and made the spend a durable number. **The issue row blamed the wrong event type, and that was load-bearing:** the payload is 18 `agent_input` rows (7,290,638 chars, largest 479,605), NOT `agent_chunk` (max row **169 chars**) — a chunk filter would have fixed almost nothing. Two further unbounded tools the row never mentioned: `list_refs()` projected artifact bodies inline (5,804,067 chars ≈ 1.45M tok in one call) and `get_ref()` returned bodies up to 389,651. Two security defects fixed alongside, each proven by a RED test: `exclude_builtin_tools` was never passed, so the model held `write_file`/`edit_file`/`read_file`/`ls`/`glob`/`grep`/`write_todos` — a filesystem WRITE surface on an app REST path, contradicting this module's own docstring; and `ScopedStore.get_ref` scopes by owner + visibility but never by run, so a prompt-injected `ref_id` could reach another run of the same owner. | **(1)** `concierge.py:365` drained the runner with a bare `if event["type"] != "chunk": continue`, discarding every `usage` event; `converse` runs in a background task spawned AFTER the run settles, outside `execute()`'s lifetime, so neither `aux_token_usage` nor `_apply_terminal_completion` could ever see it. **(2)** `concierge.py:434` called `scoped_store.read_events(run_id, 0)` — no LIMIT, no type filter — and the system prompt ORDERED the model to call it. **(3)** the runner defaults `exclude_builtin_tools=False`, excluding only the sub-agent tool. | `backend/app/agents/chat/concierge.py`, `backend/app/api/run_commands.py`, `backend/agents/authz.py` | quick-260812-gsf | INV-1/3/8/12/13 ✅ SC-001 ✅ | Done · tests: 22 unit (19 capability + 3 endpoint) ✅ |
| FIX-234 | 2026-08-12 | **The local backend could not be stopped by SIGTERM while any run was streaming, so the entire graceful-shutdown half never ran on a developer machine — and even when reached it did not stop the run.** Measured on a throwaway server: no flag → alive past **31.25s** with only `startup_complete` recorded, `kill -9` the only way out; with `--timeout-graceful-shutdown 5` → clean exit in **5.82s** with the shutdown body running to completion. Added the flag to the ten dev-facing documented commands (and both out-of-repo memory notes), dropped `--reload` (untreated it leaves **two** stuck processes and `watchfiles` is absent so it polls), kept port 8000 with evidence, deleted the double `close_checkpointer()`, corrected the under-counting teardown budget, and made `SHUTDOWN_STOP_RUNS` environment-differentiated (ON in `ENV=development`, OFF everywhere else). **Does NOT close the money hole: a production run still resumes and finishes after a restart — that is ISS-089.** | uvicorn's `timeout_graceful_shutdown` defaults to `None` (`uvicorn/config.py:215`) and `H11Protocol.shutdown()` does NOT close a connection whose response is still streaming — it only clears keep-alive (`h11_impl.py:328-337`). A live `EventSourceResponse` therefore never leaves `server_state.connections`, `_wait_tasks_to_complete()` spins forever (`server.py:294-310`) and `await self.lifespan.shutdown()` is never reached. Production had the flag since KAN-151 D8; no in-repo doc did. Second cause: `main.py:302-307` closed the checkpointer pool BEFORE `shutdown_run_infrastructure()`, whose step 4 closed it again where the docstring promises it runs LAST — idempotent, so silent, but the ordering guarantee was false and it blocked stop-runs. Third: the "under 25s" budget comment omitted both cancel-and-wait escalations and the pump drain. **Settled the blocking open question and REVERSED the prior hypothesis** — `asyncio.run`'s teardown does NOT already mark runs `cancelled` under SIGTERM, because `capture_signals()` restores the default disposition and re-raises the captured signal inside `serve()` (`server.py:326-330`), killing the process before `Runner.close()` → `_cancel_all_tasks`. Under SIGINT it DOES (KeyboardInterrupt unwinds normally), which is ISS-103. | `backend/app/main.py`, `backend/app/core/config.py`, `backend/app/api/run_shutdown.py`, `README.txt`, `docs/PRODUCTION_DEPLOYMENT_GUIDE.md`, `docs/FLOWIN_HANDOFF.md`, `frontend/e2e/README.md`, `frontend/e2e/tests/ts-v.per-workflow.live.spec.ts`, `frontend/e2e/tests/ts-w.model-override.live.spec.ts`, `.planning/TEST-REGISTER.md`, `.planning/RESUME-QA-TEST-SHEET.md` | quick-260812-hsx | INV-3/12 ✅ SC-001 ✅ | Done · tests: 10 unit (8 new reachability/budget/config + 2 new stop-runs branches) ✅ |
| FIX-235 | 2026-08-12 | **A re-delivered frame appended a second copy to every accumulating field in `handlePipelineMessage` — the reducer's correctness was a property of its CALLERS' dedup, the exact arrangement ISS-080 died of behind 15 green tests.** **The register row was right about the concern and wrong about three facts.** (1) It names **2** accumulating fields; there are **7**, six unprotected. (2) Its cited evidence command finds **neither field it names** — re-run verbatim at `07197b0e`, `grep -nE '\+= 1|\.push\(|\.concat\(' src/hooks/useWorkflow.ts` returns a **comment** (`:431`) and a **local array** (`:954`), because every real accumulator uses `[...prev.x, y]` or string `+`: a 3.5x under-report by a regex blind to the idiom the codebase uses. (3) `hook_run` was **never protected by anything** — not "protected only by the class-level fix". **Three layers.** **A1**: `emit_hook_event` stamps a uuid `event_id`, kept **TRANSIENT** — routing it through `execute()` would persist a `run_events` row and add a frame to the yield stream, moving the golden multiset (INV-3); verified `hook_run` appears in **0 of 10** golden files, and `emit_hook_event` is the single emitter so one stamp covers every producer. **A2a**: ONE gate, installed by **decorating the dispatcher** rather than open-coded six times, so a future accumulating case is protected by one set entry. `lastAppliedSeq` is an **O(1)** per-run monotonic cursor — chosen over a per-frame id set because `agent_chunk` is the highest-volume frame in the system (~24,791 events on run `d5dbc9f2`) and an immutably-copied set of that size is **O(n^2)** on replay; sound because `seq` comes from one monotonic per-run allocator (`engine.py:1028`), **both** transports carry it (`api.ts:585` merges the authoritative column over the payload) and both replay passes are seq-ordered. `appliedUnsequencedIds` catches the one frame the engine never stamps, so **A1 is a prerequisite, not an optional extra**. Both reset on a `pipeline_start` that is not a same-run re-announcement, on the SAME predicate `agentStartEventIds` already used (INV-2) — proved load-bearing by mutation (removing it turns the run-2 case red). **A3**, shipped WITH the gate and never after it (a run-level cursor would skip the second replay pass wholesale, so with the shadow in place the store would never receive the REST-replayed state -> regression on history reopen): `page.tsx` open-coded the frame's run id as `_sourceRunId ?? data.pipeline_run_id`, **SHADOWING its own `frameRunId` parameter**. **The blast radius was LARGER than the investigation modelled** — `page.tsx:1682` rebuilds the live message as `{type, data}` and passes the id as an ARGUMENT, so the shadow was `undefined` for per-agent frames on **BOTH** the live and the replay path, and the per-run store was never handed one. FIX-201's second, **UNDEDUPED** replay loop existed only to work around that and is **DELETED**; its `pipeline_start` skip is subsumed by ISS-075's roster MERGE. **INV-12**: FIX-225's per-field `.includes(startIdentity)` guard is **DELETED**; the `agentStartEventIds` **ARRAY SURVIVES** — it is what `deriveSpecRevisionCount` reads and what ISS-083 depends on. A stale source-guard locking the deleted bypass was **RECONCILED and made TIGHTER** (it now forbids the second pass instead of requiring it), never deleted or loosened, and proved non-vacuous by re-injecting the bypass. **12 tests seen RED first**, reproducing the investigation's observed values exactly. **A3's safety is MEASURED, not reasoned** — the run-id decision was extracted to `resolveFrameRunId` in `wsReplayState` *so that it could be unit-tested*; reverting it to the pre-fix behaviour turns exactly 3 red. **New finding, empirically verified: `hook_run`'s payload never reaches the reducer** (ISS-107), which **refutes** the investigation's claimed user-visible blast radius. | The reducer had no identity of its own: six of its cases grow a field out of `prev` (`[...prev.x, y]` / string `+`) with nothing keyed on the frame, so a partial redelivery (an SSE resume from `Last-Event-ID` with no intervening `agent_start`) appended a second copy. `hook_run` compounded it — `emit_hook_event` bypasses the engine's stamping chokepoint, so the frame reaches the browser with no `event_id` and `wsReplayState.ts:36` exempts unstamped frames **by design**; no frontend-only fix could touch it. And `page.tsx:855` shadowed the run-id parameter, which is why FIX-201 added a deliberately undeduped second replay pass that fed the reducer every durable frame twice. | `backend/agents/execution_engine/kernel_services.py`, `frontend/src/hooks/useWorkflow.ts`, `frontend/src/types/index.ts`, `frontend/src/lib/wsReplayState.ts`, `frontend/src/app/dashboard/page.tsx`, `frontend/src/hooks/__fixtures__/reducerHarness.ts` | quick-260812-iqh | INV-1/2/3/12/SC-001 OK | Done - tests: 30 frontend (12 RED first) + 6 wsReplayState + 1 reconciled guard; goldens 10 passed / 0 moved; lint-imports 4 kept / 0 broken; backend reds IDENTICAL ID set; vitest 147 failed | 824 passed -> 147 failed | 860 passed, ZERO newly-red; Playwright 33/43/108 byte-identical |
| FIX-236 | 2026-08-12 | **The `spec_revision` chat card held a SECOND, contradicting copy of the revision-cycle number — and the register row's proposed fix would have made it a THIRD.** ISS-083 asks for `ResultCard`'s dormant `cycle` prop to be wired from `runStore.viewed.specRevisionCount`. **REJECTED; nothing was wired.** The prop is not a gap, it is a duplicate: the header rendered `` `Revising spec — cycle ${cycle ?? 1}` `` while the body rendered the backend narrator's own text (`chat_narrator.py:157`, `f"Revising spec — cycle {attempt}"`), so had the card ever mounted it would have printed "cycle 1" directly above "cycle 2". Deleted, not shadowed (INV-12): prop doc+decl (`:96-100`), destructure (`:103` — **the row misses this line**), ternary (`:117-120`); the header falls back to the static `CARD_SPECS.spec_revision.title`. The row's cited `97-100,119` was stale at both ends. **The card KIND is deliberately KEPT** — cross-stack contract, parked on purpose; its unreachability is filed as ISS-111 rather than folded in. | **The FE keeps re-deriving a number the backend already computed as a string.** Three compounding facts, each re-verified here rather than inherited. (1) **The kind is unreachable end-to-end:** `chat_narrator._revision_index` (`:106-118`) keys on `spec_revision_attempt`/`revision_index`, and those two strings exist as quoted dict keys **only at `:112`/`:114` — the reader itself**; the engine holds `spec_revision_attempt` as a Python local and passes `revision_index=` as a kwarg, and neither ever enters an event payload (`agent_start`'s is fixed at `{agent_id,name,role,icon,index,total}`). `attempt` is therefore always 0 and the minting branch at `:151-159` never fires — confirmed over **232,023 durable `run_events` across 14 runs, including 52 rows proving spec revisions demonstrably happened**: cards are gate 52 / clarify 43 / pipeline 17 / deliverable 12 / null 6 and `spec_revision` is **ZERO**. (2) **The dormancy is deliberate and LOCKED:** `IMPLEMENTATION-REGISTER.md:4214` records FIX-220 naming its keys `revision_cycle`/`revision_in_flight` *specifically* to avoid waking this reader, which "would have silently turned every in-revision gate card into a 'Revising spec - cycle N' card", and forbids inventing a third counter. (3) **The store value is the wrong semantics anyway:** `specRevisionCount` is the run's CURRENT count while a chat card is a HISTORICAL transcript entry, so a transcript holding a cycle-1 and a cycle-2 card would paint both with the live number — while the correct per-card value is already on the wire twice (`text` and `deep_link.target`). This is the **fifth** dead counter in the family (ISS-039, FIX-164, FIX-163/ISS-063/FIX-221, FIX-225, this), not the third the row claims. | `frontend/src/components/chat/ResultCard.tsx`, `frontend/src/components/chat/ResultCard.test.tsx`, `frontend/e2e/tests/ts-chat-cards.spec.ts` | quick-260812-jn9 | INV-1/3/12/SC-001 OK | Done - tests: 3 frontend, ALL seen RED first and re-observed independently on a reverted source (4 failed / 9 passed, T1 receiving exactly 2 - the two copies caught in the act); ResultCard 1 failed / 10 passed -> 1 failed / 12 passed with the failing ID UNCHANGED; goldens 10 passed / 0 moved vs f12d99ae; lint-imports 4 kept / 0 broken vs f12d99ae; narrator 36 passed; reducer specs 63 passed (the affirmative evidence nothing was wired); tsc the same 2 pre-existing errors; ts-chat-cards e2e UNVERIFIABLE-GREEN (red before AND after on ISS-076's beforeEach), proven by before/after identity |
| FIX-237 | 2026-08-12 | **The settled "Task plan · N planned" card rendered the number COMPLETED, not the number planned — wrong at the latest version and frozen under the artifact version picker (ISS-087).** `deriveArtifactCardModel` derived the card's rows and count from `protoCompletedTasks`/`protoCompletedTaskCount` — the BUILD agent's `task_progress` stream (`useWorkflow.ts:966-995`) — while the label claimed "planned". **The ISS-087 row was wrong on two of its three decision-relevant claims and understated the defect** (see the corrected row). Measured read-only on `backend/dev.db`: run `d5dbc9f2`'s plan holds **7** tasks and the card rendered **"6 planned"** with **no picker interaction at all**; run `6e38b9a7` kept showing 11 after selecting a 10-task v1; `prototype-plan` is multi-version on **5 of 6** run-groups, so the picker is visible on most prototype runs — the opposite of the row's "one version on most runs". **Fix (owner decision D1):** the card parses the `<tasks>` body ON SCREEN. Because the panel already passes `displayed` (`AgentDetailPanel.tsx:753`, FIX-226's thread), the card follows the picker with **no second threading added**. `parseTasks` was **EXTRACTED verbatim** from `TasksPreview` into `artifactPreview.tsx:80-104` beside `parseSpecSections`/`parseSpecOverview`, mirroring how those were lifted from `SpecPreview` — **no third parser** (INV-12). `useWorkflow.ts` was deliberately **left untouched**: its regex is line-anchored and tolerates `\s*:`, is NOT equivalent, and feeds `ConstructionBlock`; filed as ISS-115 rather than silently unified. The dead `protoCompletedTasks`/`protoCompletedTaskCount` props were **deleted**, not left inert — the build's completed tasks still reach the ConstructionBlock via `construction.tasks` (`AgentThinkingTab.tsx:195-202`), where the label matches the data. `dagEdges` closed as NOT-a-defect (a DAG edge is constant within a run); the validation badge NOT version-threaded because it is dead (ISS-116). **3 pinned 42-09 tests reconciled — none deleted, skipped or loosened**; 2 encode owner-approved behaviour changes (the count is the plan's size; a `single_shot` `<tasks>` agent now shows its plan). | A card was fed a truthful count of the WRONG QUANTITY: "completed" rendered under a "planned" label. The version bug ISS-087 was filed for was a symptom — the rows never came from the artifact at all. | `frontend/src/components/results/artifactPreview.tsx`, `AgentDetailPanel.tsx`, `AgentThinkingTab.tsx` (+2 test files) | quick-260812-kpb | INV-12 ✅ (one parser, extracted not copied) · INV-3 ✅ (frontend-only; goldens byte-identical) · SC-001 ✅ (keys on the artifact's own wrapper tag, and drops a dependency on `task_progress` frames only the prototype build agent emits) | Done · tests: 4 new + 3 reconciled frontend cases, **all 7 seen RED first** ✅ |
| FIX-238 | 2026-08-12 | **The offline unit suite made REAL, billable AWS Bedrock calls, and nothing in the repo could have stopped it (ISS-102).** Two tests in `tests/unit/test_chat_messages_endpoint.py::TestRouting` did not monkeypatch `run_commands._resolve_concierge` (unlike all three sibling classes), so the endpoint resolved the REAL `ConciergeCapability`, whose ctx carries `model=None` → `build_model()` → `ChatBedrockConverse` → a live `ConverseStream` call. Verbatim before the fix: `DeepAgentRunner.astream_events failed: An error occurred (ExpiredTokenException) when calling the ConverseStream operation`. **Two parts.** (1) `TestRouting` gains a class-placed autouse fixture running the byte-identical sibling call `monkeypatch.setattr(rc_module, "_resolve_concierge", lambda: fake)` — class-wide, not per-test, because only 2 of the 8 methods reach the seam today and a patch on the other 6 would be dead code (INV-12). (2) The real deliverable: `backend/tests/conftest.py` gains an autouse guard that makes constructing a live provider client a loud failure naming the offending nodeid. **The chokepoint is the three provider CLASSES, not `build_model` — established, not assumed.** `agents/planner/smart_planner.py:173/:189/:196` builds its client WITHOUT `build_model`, and `deepagents.create_deep_agent(model="anthropic:...")` builds `ChatAnthropic` via langchain's `init_chat_model`; a `build_model` guard would have missed **6 of the 10** offending tests and been defeated anyway by the by-value `from app.agents.model_factory import build_model` imports at `deep_agent_runner.py:58` / `cached_invoke.py:44`. The guard patches `__init__` and **never the class object**, so `isinstance` (`deep_agent_runner._maybe_apply:234`, `cached_invoke:79`) and `object.__new__` (`test_bedrock_cache_and_thinking._bedrock_instance`) keep working. Its exception derives from **`BaseException`, not `Exception`** — `run_commands.py:1513` and `deep_agent_runner.py:586` are bare `except Exception`, which is exactly how the original `ExpiredTokenException` became a log line instead of a failure; an `Exception`-derived guard would have been swallowed on the very path it polices. Three allowances only: `RUN_LIVE_BEDROCK=1`, the `requires_api_key` marker, and exact-nodeid membership in `_CONSTRUCTS_BUT_NEVER_INVOKES` (8 verified construct-only tests, filed as ISS-118 rather than silently patched). **The two tests remain RED — deliberately** (ISS-119): they assert the superseded pre-safety routing contract, and reconciling that is an owner decision, not a side effect of a money fix. | Nothing anywhere asserted that an offline test may not reach a live model, and the suite has **no offline-by-default state**: `AWS_REGION`, `BEDROCK_INFERENCE_PROFILE_ID` and `BEDROCK_MODEL_ID` are hardcoded CODE defaults (`app/core/config.py:130/:123/:118`) with no `backend/.env`, so unsetting env vars cannot make `build_model()` offline. The only thing that ever stood between this suite and a bill was whether AWS credentials happened to resolve — and they now do. | `backend/tests/conftest.py`, `backend/tests/unit/test_chat_messages_endpoint.py`, `backend/tests/unit/test_iss102_live_model_guard.py` | quick-260812-lfv | INV-3 ✅ (test-infra only; zero production files; goldens byte-identical) · INV-12 ✅ (one guard, one allow-list, no per-test shadows) · SC-001 ✅ (no workflow name anywhere) | Done · tests: 6 new unit, **4 seen RED first** ✅ |
| FIX-239 | 2026-08-12 | **The product could not tell you whether prompt caching was saving money or costing it — and the obvious version of this feature would have printed a catastrophic wrong number on day one.** ISS-034 asked for "saved $Y (Z%)". **That framing is refuted by the repo's own data and was NOT built.** Re-derived read-only from `backend/dev.db` through the real shared `estimate_cost_usd` (0 reconciliation mismatches on all 11 rows): **6 of 11 persisted runs are strictly NEGATIVE** (-4.98%..-8.86%), 1 exactly zero, 4 positive; the +80.34% aggregate is carried by two long prototype runs. A UI that can only say "saved" would be wrong on the MAJORITY of runs in the only dataset that exists. Shipped instead (owner decision D2): `estimated_cost_full_usd` — the as-if-UNCACHED counterfactual on the SAME token base — at BOTH cost sites, and a **SIGNED** Analytics line that branches on the sign of the data (positive -> "saved", negative -> "cost more" in amber because it is a config finding not an error, zero -> renders NOTHING, since a "0%" would imply caching ran and broke even). **Every cited line number had drifted and was re-verified**: the analysis said `engine.py:2842-2855` / `run_commands.py:2100-2115` and D2 said `:2847-2854` / `:2107-2114`; actual at `c05906c0` was **`:2873-2880`** and **`:2171-2178`**. A THIRD `estimate_cost_usd` at `run_commands.py:1574` — unmentioned by the analysis — is the Concierge `chat_usage` per-message writer (FIX-233) and was correctly left alone. **The catastrophic-number trap was real and is closed:** `analytics.py:_num` coerces an absent field to `0.0` and **0 of 11 existing rows carry the new key**, so the naive `sum(_num(usage.get(...)))` was OBSERVED producing `spend_full = $0.00` against `spend = $15.00` -> "caching cost you the entire window more (-100%)". `r_cost_full = _num(...) or r_cost` makes a legacy row's delta **exactly zero — never a fabricated $0 baseline** — and `metered_runs` lets the UI state its coverage. **Golden-neutral PROVEN BOTH WAYS, not assumed**: key emitted AND stripped -> 10 passed with all 15 golden files byte-identical (SHA-256 compared); emitted and NOT stripped -> **5 event snapshots RED** (the non-vacuity control). Zero regeneration; `SNAPSHOT_UPDATE` appears nowhere in the diff. **D2's mandated footnote was stale and was corrected against the code**: the Concierge IS metered per message now (`concierge.py:578-609` -> `ctx.usage`; `run_commands.py:1559-1583` writes a `chat_usage` event) but `chat_usage` has **ZERO readers** and Analytics reads only `workflow_runs.token_usage`, so chat spend is *recorded and excluded*, not unmetered; handoff is genuinely unmetered — `app/services/handoff_pipeline.py` has zero token metering and writes no `WorkflowRun` rows (the analysis grepped `agents/execution_engine/handoff_pipeline.py`, **which does not exist**). No migration (`token_usage` is an untyped JSON blob; every reader a tolerant `json.loads`) and **no per-run badge** — that would reverse the deliberate KAN-83 removal, revive 3 quarantined e2e cases, and put a precise dollar beside a run whose own total is not displayed. The global-cache-flag half is split out as **ISS-120**, not folded in. | The two cost sites priced only what was ACTUALLY paid, with no counterfactual, so the effect of `BEDROCK_PROMPT_CACHE_ENABLED` was unobservable anywhere in the product — and the one surface that shows dollars (Analytics) had no second term to compare against. The discriminator turns out to be run LENGTH, not workflow type: a run that writes cache entries it never re-reads pays the 1.25x `cache_write_5m` premium for nothing. | `backend/agents/execution_engine/engine.py`, `backend/app/api/run_commands.py`, `backend/app/api/analytics.py`, `backend/tests/agents/characterization/_normalize.py`, `frontend/src/lib/api.ts`, `frontend/src/components/analytics/AnalyticsPage.tsx` (+3 test files) | quick-260812-mq5 | INV-3 ✅ (goldens 10 passed, all 15 files byte-identical; control proves non-vacuity) · INV-12 ✅ (one shared `estimate_cost_usd` at both sites, no rate literal, no FE per-model math — a subtraction of two backend dollars) · SC-001/INV-1 ✅ (unconditional on every `pipeline_complete` and every persisted row; the renderer keys on the numeric SIGN only, no workflow name) · Q3 N/A (no migration) | Done · tests: 8 new backend (`test_iss034_cost_full.py`, **4 seen RED first**) + 4 new analytics API (**legacy-row case seen RED under the naive sum, reproducing the -100% catastrophe**) + 5 new frontend (**signed-copy mutation proves teeth**) ✅; goldens 10 passed / 0 moved vs `c05906c0`; lint-imports 4 kept / 0 broken vs `c05906c0`; targeted suite 71 -> 83 passed; AnalyticsPage 4 -> 9 passed; `test_wire_parity.py` 4 F / 2 P — IDENTICAL ids, pre-existing; tsc the same 2 pre-existing errors |
| FIX-240 | 2026-08-12 | **Every chat turn during a live run silently destroyed one engine event from the durable log (ISS-121).** **The register row's own root cause was WRONG and is corrected in place.** It claimed `pipeline_cancelled` is NEVER WRITTEN to `run_events` and prescribed "make the reopen path consult the run record". Every MEASUREMENT in that row is right; the causal claim on top of them is not. `pipeline_cancelled` IS written by all eight engine terminal emitters — on run `808612bf` the INSERT was **attempted, rejected by `uq_run_events_scope_seq`, and silently swallowed**. Two writers shared ONE per-run seq space with TWO algorithms: `ScopedStore.append_event_next_seq` takes `max(seq)+1` and **retries past `IntegrityError`**, while `_RunEventSink.persist` used a fixed in-memory counter and **swallowed the collision** — it catches `SQLAlchemyError`, `IntegrityError` is one, so a production seq collision took the branch written for "the offline harness has no schema" and was indistinguishable in the logs. `authz.py:462-465` documented this as an accepted trade-off calling the collision **"rare"**; it fires on **every chat turn during a live run**, and the victim is whatever the engine emits next. At a gate the engine is idle, so the victim is **deterministically the terminal frame** — which is exactly why the symptom looked like "cancelled is never persisted". **The fingerprint:** the log does not end at `review_gate_ready` (278) — seq 281 is a narrator card reading **"Cancelled by you"**, projected from the very event whose row is missing; a milestone card whose preceding row is a `chat_message` is structurally impossible otherwise. Also corrected: the rejection came through **`POST /messages`** (the only one of the three gate-adjacent endpoints that writes a durable row), not `POST /gate`. **Fix:** `ScopedStore.append_event_at_or_after` re-appends past the racing writer and returns the seq the row actually landed on; both engine drive loops (`execute()` and the resume driver) **re-stamp `data["seq"]`** with it and advance the allocator past it. **The re-stamp is mandatory, not cosmetic** — `run_stream.py:198` renders the SSE `id:` cursor from `row.seq` on replay and `:264` from `data["seq"]` live, so a retry without a re-stamp would corrupt `Last-Event-ID` resumption: a worse bug than the one being fixed. **The obvious retry was REJECTED on cost**: calling `append_event_next_seq` per engine event materialises the whole log per write and the corpus holds a 36k-row run; `_max_event_seq` is a bounded index probe on `uq_run_events_scope_seq` instead. **The discriminator is the load-bearing part**: a rejection is retried ONLY when the tail actually reaches the attempted seq. It does not in the characterization harness — **measured, not assumed**: the harness's persist failure is a `sqlite3.IntegrityError` **FOREIGN KEY** violation (the `workflow_runs` parent row is absent), so a naive "retry on IntegrityError" would have spun 8 attempts per event across every golden **and** re-stamped `data["seq"]`, breaking `assert_seq_contiguous`, which pins seq DELTAS at exactly 1 on the RAW pre-normalize stream. **The claimed INV-3 hazard was refuted before the design was chosen** and replaced with the real one: goldens snapshot the YIELDED stream with `seq`/`event_id` stripped and no DB at all, and contain zero `review_gate_ready` / zero `pipeline_cancelled` (`gate_agent_ids=[]`) — so they are the **neutrality** gate, never the detection gate. **Part (b) is data repair only and deliberately small:** the D-14g SSE gate re-arm now consults the persisted `WorkflowRun.status` (reusing the exported `TERMINAL_STATUSES`, not a third copy). `gate_pendency` stays PURE — the import-linter contract forbids `agents.capabilities` importing `app.*`, so the status check lives at the app boundary. **No synthetic event is back-filled**: fabricating durable rows with no `event_id` provenance would make the log lie, and consulting the run record as a general substitute would install the second source of truth for terminal state that INV-12 exists to prevent. **Why it shipped:** `test_append_event_next_seq_retries_past_a_raced_seq` proved only that the chat writer WINS the race and never asked what happened to the loser — that missing half is TEST-024. **All 3 new tests assert the durable ROW, never the yielded frame** (the live stream is intact on every path, which is why three earlier offline probes for ISS-091 missed this entirely), were seen RED at `9e3dc9b0`, and fail on their OWN assertions under retry-disabled / guard-removed mutations. FIX-232 is untouched and its money-stopping behaviour is preserved. Files **ISS-123** (up to 29 already-destroyed events, not retroactively recoverable), **ISS-124** (two app-layer terminals that never persist at all), **ISS-125** (`_stamp_resume_marker`'s full-log scan), **ISS-126** (a fresh reopen still replays the dangling gate row). | Two `run_events` writers allocated from ONE per-run seq space with TWO algorithms; the engine's `_RunEventSink.persist` caught `SQLAlchemyError` (which `IntegrityError` subclasses) and degraded a real uniqueness collision through the branch meant for the schema-less offline harness, discarding the row. | `backend/agents/authz.py`, `backend/agents/execution_engine/engine.py`, `backend/app/api/run_stream.py`, `backend/tests/unit/test_run_events.py`, `backend/tests/unit/test_sse_stream.py` | quick-260812-ppu | INV-3 ✅ (goldens 10 passed, 0 files moved; the harness's FK rejection is not a seq collision so no re-stamp fires and `assert_seq_contiguous` is untouched) · INV-12 ✅ (reuses the `authz.py` retry idiom and the `engine.py:1078-1079` counter re-sync; adds no second terminal-state authority) · SC-001/INV-1 ✅ (keys only on generic seq/uniqueness mechanics — no workflow name, no agent-id literal, no event-type branch) · Ports & Adapters ✅ (`gate_pendency` stays pure; lint-imports 4/0) · Q3 N/A (no migration) | Done · tests: 3 new backend unit (**all 3 seen RED first at `9e3dc9b0`**; 2 in `test_run_events.py` 10 → 12, 1 in `test_sse_stream.py` 42 → 43) + 2 hand-rolled test fakes re-pointed at the new seam (assertions untouched) ✅; goldens 10 passed / **0 of the golden files moved** vs `9e3dc9b0`; lint-imports 4 kept / 0 broken vs `9e3dc9b0`; the 9-file pre-existing-red sweep **22 failed / 176 passed with an IDENTICAL ID set** vs `9e3dc9b0` (re-measured in a detached worktree, never a stash); `test_mechanical_router.py`'s 3 reds — absent from the briefed list — **verified pre-existing** at `9e3dc9b0` by the same method; frontend **0 files changed** |
| FIX-241 | 2026-08-12 | **A human-in-the-loop gate that fails OPEN: every near-miss of a DENIAL silently APPROVED (ISS-070).** `resolve_gate` dispatched on `action` with `else: # approve (default)` as the fallback (`run_commands.py:272`), an exact, case-sensitive, unstripped match. **Observed in-process against the real endpoint, not reasoned:** `'Reject'`, `'REJECT'`, `' reject'`, `'reject\n'`, `'rejct'`, `'no'`, `'deny'`, `'Redo'`, `'redo '`, `'update-specs'` → **all `HTTP200 approved=True`, gate resolved**. The user believes they stopped the run; the run resumes and spends the remainder of a 5–21M-token `od_prototype` budget. **Second, unfiled finding — the record was LAUNDERED:** the `else` never passed `action=`, so `store.set_review_response`'s own default (`store.py:123`) persisted `action="approve"` while `resolve_gate:278` echoed the raw garbage back to the caller — response and durable audit row disagree, which is very likely why this never showed up in a live log. On the **declared**-gate path it compounds: an unknown action emits none of `_gate_rejected`/`_gate_redo`/`_gate_update_specs`, so `human.py:104-141` resolves a formal **`GATE_PASS`** and writes an honest-looking audit row — the precise failure the in-file comment at `human.py:52` says the `_UPDATE_SPECS` branch exists to prevent. **The row's scope was OVER-GENERALISED and is corrected in place: only ONE of the three `set_review_response` ingresses is defective.** `/messages` `CHANNEL_GATE` (`:1322`) is fenced by `chat_router.py:234` — verified by reading BOTH `Dispatch(channel=CHANNEL_GATE)` construction sites (`chat_router.py:244`/`:255`), each requiring `turn.action in GATE_ACTIONS`; the Concierge confirm (`:1101`) is fenced by `concierge.py:350` normalisation plus H1 (params are server-written, never client body). Those two are latent hazards, not live defects. **Two layers, each independently sufficient — MUTATION-PROVED, not assumed.** **A (schema):** `GateCommand.action` → `Literal["approve","reject","redo","update_specs"]`, giving 422 at the wire and an `enum` in OpenAPI. **The DEFAULT IS RETAINED** — making the field required would break `test_attach_replay_matrix.py:551`'s bare `{"gate_key": ...}` POST *and* the published contract (`"default": "approve"`); verified after the change that OpenAPI still reads `required: ['gate_key']`. **B (dispatch):** `elif action == "approve"` + an `else` raising **400 `unknown_gate_action` / `recoverable: true`**. Mutation 1 (A reverted, B kept) → all ten variants **HTTP400**, nothing stored, gate armed; mutation 2 (B reverted, A kept) → all ten **HTTP422**, same. **It REFUSES and must never degrade to a reject:** FIX-232 makes a rejection terminal, so turning a typo into a denial would destroy the run — leaving the gate ARMED is the only degrade that preserves every legitimate option, mirroring the engine's own choice for an ineligible `update_specs` (`engine.py:5830-5838`). Layer C ships the same fail-closed shape at the two latent ingresses; **`chat_router.py` is deliberately NOT touched** — its `:234` guard IS the ISS-119 routing contract and reconciling that is ISS-119's decision. **The if/elif chain is NOT restructured** — only the `else` branches changed — because `test_rest_gate_commands.py:362` is a grep-ratchet asserting the literal source substring `action == "redo"`; a dict/`match` refactor turns it red. **Retag `api,auth` → `api,validation`:** verified NOT an authorization defect — `resolve_gate:222` → `run_engine.py:765` filters `WorkflowRun.user_id == user_id`, so a cross-owner caller 404s and nobody else can approve your gate; the affected principal is the run's own owner. Severity stays **major**: a safety gate fails open, invisibly, on the remainder of a multi-million-token budget. **INV-12:** `chat_router.GATE_ACTIONS` stays the ONE authority — `Literal` needs static values, so the two are pinned by a set-equality test rather than a fifth hand-written copy. Splits out **ISS-127** (`analysis_report` has no length cap on its path to the composed prompt) and **ISS-128** (a legitimate `reject` ALSO persists `action="approve"` — the reject branch shares the omission; inert because the engine keys on `approved`, but the durable row misdescribes the user's decision) rather than folding either in. | Nothing on the `POST /gate` path ever constrained what an action IS. `GateCommand.action` was a bare `str` with no enum, and the dispatch's terminal `else` treated "not one of the three strings I check for" as consent. Every other ingress had grown a validator in a different file (`chat_router.py:59`, `concierge.py:80`, the FE TS union at `api.ts:694`) — four allow-lists, none of them on this route — so the endpoint's safety depended entirely on clients volunteering to be well-behaved. The engine compounds it by keying its terminal decision on `approved`, not `action` (`engine.py:5852`/`:5859`), which makes the API layer the SOLE place that maps action → approved. | `backend/app/api/run_commands.py`, `backend/tests/unit/test_rest_gate_commands.py` | quick-260812-sgu | INV-1/SC-001 ✅ (branches only on the generic four-action discriminator already in `chat_router.py:59` — no `pipeline_type`, no `spec.id`, no agent-id literal) · INV-3 ✅ (all four valid actions byte-identical; goldens 10 passed / **0 of the 15 golden files modified**, and they are the NEUTRALITY gate only — `grep -c review_gate_ready` = 0 across all 15, so they could never have detected this) · INV-12 ✅ (one vocabulary authority, pinned by set-equality; one `_deny_unknown_gate_action` helper serving all three ingresses instead of three copies) · Ports & Adapters ✅ (app layer only; no kernel edit, no new import edge; lint-imports 4/0) · Q3 N/A (no migration) · WR-07 ✅ (this fix IS fail-closed compliance) | Done · tests: 13 new backend unit (**12 seen RED first at `e6b24ae5`** — ten of them `200 approved=True` on the case/whitespace/typo variants of a denial), `test_rest_gate_commands` 15 → 28 passed; both layers mutation-proved independently; goldens 10 passed / 0 files moved vs `e6b24ae5`; lint-imports 4 kept / 0 broken vs `e6b24ae5`; `test_update_specs_ingress_fence` 13 passed; the 14-id pre-existing-red set across the five gate-adjacent suites **byte-identical** to `e6b24ae5` (re-measured by swapping in `git show HEAD:...`, never a stash); `test_concierge_proposal_channels` + `test_attach_replay_matrix` 2 failed / 70 passed with the SAME two ids; ruff findings unchanged (3 pre-existing, none in a changed region); frontend **0 files changed** |
| FIX-242 | 2026-08-12 | **A gated fan-out step parked the run at a review gate nobody could see, discover or resolve (ISS-097).** `_should_gate` (`engine.py:4908-4925`) is a per-INVOCATION predicate on `spec.id`; `gate_agent_ids` is a per-STEP selection — the UI says "Checked agents pause the pipeline for your review after they finish" (`ReviewGatesSection.tsx:128-129`). Identical readings until fan-out made one step produce N invocations of one agent id (`fanout.py:95-96` resolves `agent: self` to `step.agent_id`). **The originally-filed prediction — "N simultaneous gates, each awaiting a human" — was wrong in the direction that matters.** Every worker's `review_gate_ready` is consumed and DISCARDED by `fanout.py:414-429`, whose loop reads only `agent_complete` for token accounting and forwards nothing. Zero frames reached the SSE stream, zero rows reached durable `run_events`, `derive_open_gate` returned `(None, None)`, and the run stopped dead with `workflow_runs.status` = `waiting_for_user` indefinitely; all N workers shared ONE gate key `f"{run_id}:{agent_id}"` (`engine.py:5705`), so they were N arms of one slot. **MEASURED, not reasoned** (`sample_wave`, single-wave plan, 4 workers, `gate_agent_ids=["sample-wave-worker"]` at `eb12bc7e`): `_run_review_gate` entered **4×**, **1** gate key, **0** frames on the wire, **HUNG** at the 20 s timeout; control with `[]` completes. **Fix: ONE keyword-only invocation-scope flag `invocation_gated`**, threaded from the two kernel sites that create non-step invocations — `run_worker` (`kernel_services.py`, the only non-test caller, grep-verified) and `run_merge_agent` (identical invisible-hang shape: its events are consumed with a bare `pass`) — through `run_agent` into `_run_agent`, reaching all THREE inline gate sites (restart re-entry `:3447`, revision re-open `:3563`, live post-stream `:4499`). **INV-12: the existing predicate is narrowed IN PLACE** (`return invocation_gated and selected`), both original branches preserved verbatim; no second gating rule was added, so nothing is superseded and nothing is left to delete. Default `True` ⇒ every step-shaped invocation byte-unchanged. **Three designs evaluated and REJECTED — recorded so they are not retried:** (a) making `_should_gate` read `step.gates` CANNOT work — `kernel_services.py:992-993` reuses the PARENT step object, gates included, whenever the worker is `agent: self` with no isolated workspace (exactly the offline and `shared_read` path); it is also a different mechanism (step-boundary `_evaluate_gates`), and wiring both to one source is the dual implementation INV-12 forbids. (b) Parking the flag on `ExecutionContext` RACES — N workers share ONE `ectx` under `asyncio.gather`, so a save/restore field could be cleared by worker A's `finally` while worker B is mid-flight (INV-2). (c) Renaming worker agent ids breaks `_spec_for`, the `allowed_workers` allow-list, fragment artifact provenance and the wave resume cursor. **Deliberate residual, filed not folded:** `engine.py:2470` still passes `inline_gated=self._should_gate(spec, ectx)` at the step boundary, so a fan-out step that BOTH declares `gates:[human]` AND has its agent ticked now dedupes its declared gate against an inline gate that no longer fires — **ISS-131**. Left byte-unchanged on purpose so the INV-3 dormancy argument stayed exact; correcting it needs the strategy to declare whether it runs its agent inline, which the kernel must not infer from a capability-name list. Also filed rather than folded: **ISS-129** (the `fanout.py:414-429` blanket worker-event swallow — the amplifier, still live for `agent_error`/`pipeline_cancelled`), **ISS-130** (shared-`ectx` scratch mutation under parallel fan-out — believed safe only because `_run_agent` consumes the scratch before its first `await`, believed but NOT verified), **ISS-132** (the task-loop sibling — N *visible* sequential gates; suppressing it is a product decision, not a defect with one answer). **Test-double repairs required by the signature change, and turned into coverage:** `test_fanout.py`'s `run_worker` double and `test_merge_conflict.py`'s new merge double now ASSERT `invocation_gated is False`; three `_run_agent` doubles in `test_pipeline_failure_semantics.py` pinned a stale positional arity and now absorb additive params. `test_gate_stub_signature_drift.py` — the AST guard that exists for exactly this defect family — caught a first attempt that patched `_run_review_gate` at CLASS level with an unbindable signature; the test was rewritten to the file's dominant instance-patch idiom with `*a, **kw`, and the guard is green.  | `gate_agent_ids` was designed as a per-STEP selection and implemented as a per-INVOCATION predicate on `spec.id`. Those two readings were the same thing for every workflow that existed when the selector shipped, because every step invoked its agent exactly once. Fan-out (Phase 11) and the task loop broke that equivalence and nothing re-derived the gate rule; the `gates=[]` on the worker step view (`kernel_services.py:1001`) reads as "this worker has no gates" and is a false reassurance — it suppresses only the DECLARED path, never the inline one, which never sees a step at all. | `backend/agents/execution_engine/engine.py`, `backend/agents/execution_engine/kernel_services.py`, `backend/tests/agents/test_restart_resume.py`, `backend/tests/agents/test_fanout.py`, `backend/tests/agents/test_merge_conflict.py`, `backend/tests/unit/test_pipeline_failure_semantics.py` | quick-260812-syf | INV-1/SC-001 ✅ (no agent-name literal, no workflow-name literal, no workflow-type branch — the flag is stamped by a structural call site exactly as the existing `redoable` / `update_specs_eligible` booleans are; `gate_agent_ids` remains the generic discriminator) · INV-3 ✅ (default `True` ⇒ dormant on every non-worker path; **argued from dormancy, NOT from goldens** — `tests/agents/_scripted_model.py:649` drives all 10 goldens with `gate_agent_ids=[]  # suppress all gates`, so zero gate events exist in any golden and they could never have detected this; they are the NEUTRALITY gate only) · INV-12 ✅ (one predicate, narrowed in place; no parallel gating rule) · INV-2 ✅ (an argument, not per-run state — see rejected design (b)) · Ports & Adapters ✅ (kernel-side only; `fanout_batch.py` / `wave_scheduler.py` untouched, no capability module touched, no new import edge; lint-imports 4/0) · Q3 N/A (no migration) · FIX-232 ✅ undisturbed (adds a parameter; cancellation, terminality and the `run_fanout` finally teardown unchanged) | Done · tests: 4 backend unit (2 new in `test_restart_resume.py`, 1 new in `test_merge_conflict.py`, 1 strengthened in `test_fanout.py`) + 3 stub-arity repairs. **RED seen first, then RE-PROVEN by mutation after the tests reached their final shape** (opt-outs mutated back out ⇒ 3 failed: the hang test reported "4 gate arm(s), 0 review_gate_ready frame(s) on the wire" and both seam pins reported `True is False`). **The un-shadow is load-bearing and was proved by experiment**: with `del engine._run_review_gate` removed, the identical test passes GREEN in 1.07 s against a live 4-arm hang. Baselines vs `eb12bc7e`: goldens **10 passed, 0 of the 15 golden files moved** (sha256-identical); lint-imports **4 kept / 0 broken**; `test_restart_resume` 64 → 66; `test_fanout_cancel` 9 → 9; `test_rest_gate_commands` 28 → 28; the 7 fan-out suites 108 → 109; 12 seam-adjacent suites 93 passed; `test_gate_stub_signature_drift` 4 passed. The 27-id pre-existing-red set is unchanged by ID, and the 5 reds in the seam-adjacent suites were re-measured **in a detached worktree at `eb12bc7e`** (5 failed / 38 passed, same ids, both sides — never a stash). Frontend **0 files changed**. Live verification deliberately NOT run — the offline harness reproduces the full defect path through the real kernel, and a live fan-out would spend real money to observe the same hang. |
| FIX-243 | 2026-08-12 | **The owner's Stop was thrown away whenever no in-process driver was live — and the next boot re-adopted the run and finished it at the owner's expense (ISS-089).** `POST /api/runs/{id}/cancel` wrote **no status at all** in the not-live branch (`run_commands.py:414-418` at `2df5324b`): it set an in-memory `asyncio.Event` and armed an in-process escalation task, both of which die with the process. So after a restart, a deploy, or any resumed run whose Event was an orphan, the row stayed `running`/`generating` — inside `restore_non_terminal_runs`' non-terminal filter — and the next startup drove it to completion. This is the second half of the `d5dbc9f2` incident: a 16,530,718-token run, **$3.58**, of which **≈$1.61 (~45%) was billed AFTER the API answered `cancelled: true`**. Measured ceiling on this corpus is **37.3M tokens / $7.07** for one `od_prototype` build; all 14 metered runs, all time, total **$17.75**. Cache reads dominate, so this is a **correctness-and-trust defect first and a cost defect second on a Haiku workload** — roughly an order of magnitude worse on an Opus-tier model, which is what the fix insures against. | `WorkflowRun.status` is a projection of an in-process asyncio task's lifetime, never a durable record of what the system was **asked** to do. Every terminal-status write in the codebase executes inside a task (launch/revision driver `CancelledError` handlers, `_reconcile_terminal_status`, the restore branches); the HTTP command layer wrote none. A truth no task was alive to record simply was not recorded. **The row's own proposed fix (Option B — a durable `cancel_requested` marker checked inside `restore_non_terminal_runs`) was REJECTED**: it needs a migration or a new event type, edits the resume tier's most delicate three-branch classifier, **misses branch (a)** (`waiting_for_user` spawns `_rearm_gate_run` without ever calling `_stamp_resume_marker`, so a marker checked before the stamp would miss every gate-parked run), and leaves the row reading `running` to history/analytics/`/resume`/the UI until the next boot — possibly forever. Option A writes the terminal status at cancel time: one function, no migration, no new vocabulary, and **no change to `restore_non_terminal_runs` at all**, because `cancelled` is already outside the non-terminal set and `POST /resume` already accepts it. The run moves from AUTOMATIC resume to EXPLICIT, owner-authenticated resume — which is what a Stop should mean. | `backend/app/api/run_commands.py` (`_record_cancellation_in_the_durable_tail`, `_cancel_run_without_a_live_driver`, `cancel_run`), `backend/agents/execution_engine/engine.py` (`NON_TERMINAL_RUN_STATUSES` extracted to module scope), `backend/scripts/cutover_legacy_runs.py` (imports it, second copy deleted), `backend/app/api/run_shutdown.py` (stale `engine.py:5263` citation), `backend/tests/unit/test_rest_answers_cancel.py` | quick-260812-tni | **INV-12 ✅ — load-bearing, twice.** (1) The non-terminal status tuple is **moved, not copied**: one module constant `agents.execution_engine.engine.NON_TERMINAL_RUN_STATUSES`, imported by the endpoint and by `cutover_legacy_runs.py`, which held the second copy. The value had been copied into three places and cited by **five** different `file:line` values in one week (`engine.py:5263`, `:5710-5713`, `:5838-5841`, `:5899-5902`, actual `:5921`) — copying it into a third module is how that keeps happening; a source guard now pins it. (2) The durable append **reuses `ScopedStore.append_event_at_or_after`** and the status write **reuses `_persist_resume_status`** — no second append idiom, no second terminal-write path. **INV-3 ✅** goldens 10 passed / **0 of the 15 golden files moved**, identical to `2df5324b`; the new durable row is written only on an explicit Stop of a non-live run, a path no golden exercises. **INV-8 ✅** the write sits behind the existing `_review_gate_owned_by` gate (404 for unknown AND unowned, no existence oracle) and rides `ScopedStore` (owner+workspace scoped); a cross-owner Stop is proven to write nothing. **SC-001 ✅** keyed on `run_id` only, no workflow/pipeline literal. **Ports & Adapters ✅** change is entirely in `app.api`; `app.api → agents.execution_engine` is the permitted direction (`run_commands.py` already imports lazily from that module in 8 places). **Additive-migrations ✅ — no migration at all**, alembic head stays `0030`. | Done · tests: **10 new backend unit** in `tests/unit/test_rest_answers_cancel.py` (10 → 20), **each seen RED first** in a detached worktree at `2df5324b` (never a stash) · **2 existing cases RECONCILED, not loosened** (both seeded `status="running"` and asserted `accepted:false`, which IS the defect; they now seed a TERMINAL run, preserving ISS-084's anti-lie invariant in the case where it still holds) · **SCOPE: single-process only** — `_is_run_live` is process-local and the deployment is single-process today (`exec uvicorn`, no `--workers`, no replicas), so "not live here" == "not live anywhere"; under the locked ECS Fargate to-be this stops holding → **ISS-134**. FE safe (both `postCancel` call sites are `void ….catch(console.error)`; the body is never read). Baselines vs `2df5324b`: goldens **10 passed / 0 files moved**, lint-imports **4 kept / 0 broken**, `test_run_shutdown` **9**, `test_shutdown_reachability` **8**, `test_restart_resume` **66**, `test_fanout_cancel` **9**, `test_rest_gate_commands` **28** — all unchanged; the 32-id pre-existing-red set is unchanged by ID. No live run launched, resumed or gate-approved; `dev.db` re-verified byte-identical (21 runs, same status sha256, 233,574 `run_events`); port 8010 never signalled (pid 1848, uptime predates the session). |
| FIX-244 | 2026-08-12 | **The two app-layer driver terminals wrote no durable row at all, so the owner's Stop was recorded as a FAILURE and fresh corrupted runs were still being minted (ISS-124).** `_drive_launch_to_queue` and `_drive_revision_to_queue` both catch `asyncio.CancelledError` — the branch `stop_run_driver` escalates to when a driver cannot observe the cooperative `cancel_event` in bounded time — push a `pipeline_cancelled` FRAME onto the queue, set `status="cancelled"`, and write NOTHING to `run_events`. **The register filed this as `minor`; it is not.** Two live consequences, both pinned by tests seen RED first. (1) The run's durable log ends on whatever preceded the cancel; when that is a `review_gate_ready`, a fresh reopen replays a dangling gate — i.e. **this path was still MINTING the corrupted runs ISS-126 has to render, which also disproves ISS-126's "affects only runs corrupted BEFORE FIX-240"**. (2) `stop_run_driver` then calls `_reconcile_terminal_status`, which decides from the durable tail ALONE: no `pipeline_cancelled` row → `cancelled=[]`/`completes=[]`/`failed=[]` → the D2 fail-safe `"failed"`, written through an **unconditional** `_persist_resume_status` that overwrites the `"cancelled"` the driver set moments earlier. Observed directly as `assert 'failed' == 'cancelled'`. **Fix:** both branches append through `_record_cancellation_in_the_durable_tail`, FIX-243's collision-safe writer (`ScopedStore.append_event_at_or_after` — FIX-240/ISS-121's failure mode), **EXTENDED with a `reason` parameter rather than copied** (INV-12: one durable-terminal writer, three callers); the default keeps FIX-243's no-live-driver payload byte-identical, the drivers pass `"driver_task_cancelled"`. **No duplicate-suppression guard, deliberately:** the `pipeline_cancelled` frame is already emitted unconditionally on both branches, so this only makes the row agree with the frame that already ships, and every verified consumer is `any()`/`max()`-shaped. **The discriminating test drives a REAL `task.cancel()`** because the append sits inside an `except asyncio.CancelledError` block and had to be OBSERVED completing under genuine cancellation rather than assumed — writing it also corrected a wrong assumption of mine: the driver deliberately CONVERTS the cancellation into a terminal instead of re-raising, so an earlier version of that test asserting a re-raise was wrong and was fixed against observed behaviour. | Two app-layer drivers owned a terminal transition but only ever wrote the transport frame and the status column, never the durable event — while `_reconcile_terminal_status`, which runs immediately afterwards on the same path, derives status exclusively from that missing durable event. | `backend/app/api/run_commands.py`, `backend/tests/unit/test_rest_answers_cancel.py` | quick-260812-wir | INV-3 ✅ (goldens 10 passed, **0 golden files moved**, SHA-256 compared vs `ce2db22e`) · INV-12 ✅ (extends the ONE durable-terminal writer; no second copy) · SC-001/INV-1 ✅ (no workflow name, no agent-id literal) · Ports & Adapters ✅ (app-layer only; lint-imports 4 kept / 0 broken) · Q3 N/A (no migration) | Done · tests: 4 new backend unit ✅ (**all 4 seen RED at `ce2db22e`**, each on its own assertion; `test_rest_answers_cancel.py` 20 → 24) |
| FIX-245 | 2026-08-12 | **A reopened TERMINAL run rendered as if it were still live — header "Awaiting approval", an armed Stop button, armed gate cards (ISS-126).** **The register's stated locus was WRONG and it changed the fix:** it put the symptom at the SSE durable replay, but **a terminal run never opens an SSE stream** (`page.tsx:2171` gates `attachRun` on non-terminal), so `run_stream.py` is never entered on this path — the screen is fed entirely by `GET /api/runs/{id}/events` replayed at `page.tsx:2234-2247`. **Both backend options previously rejected were therefore aimed at a code path the symptom does not traverse; their rejection stands, but neither would have worked.** **Fix (front-end, INV-12-REDUCING):** `terminalMarkers(status)` becomes the SINGLE definition of what a terminal status does to run state, and the three live terminal event cases (`pipeline_complete`/`pipeline_failed`/`pipeline_cancelled`) route through it — so the number of places deciding "what terminal looks like" goes **3 → 1**, and `applyTerminalStatus` is ONE-WAY (a non-terminal status returns `prev` unchanged, so a slow `getWorkflow` on a live run can never clear a legitimately open gate). The gate reuses the EXISTING `REOPEN_TERMINAL_STATUSES`, and a test pins the two sets equal so they cannot drift into the fifth divergent list INV-12 exists to prevent. **TWO of the three pieces were found ONLY by driving a real browser, after the unit tests were already green** — the live-verification lesson, paid again. (a) **Patching the store is TRANSIENT:** the FIX-201 bridge (`page.tsx:1663-1672`) copies the LEGACY `useWorkflow.pipelineState` into the store **wholesale** once the viewport switches, and that legacy state accumulated the same terminal-less replay, so it clobbered the store patch a few hundred ms later and the Stop button survived (filed as **ISS-138**). (b) **Resolving `isRunning` is not sufficient:** `laneClarifyOpen` is the ONE lane branch not AND-ed with `isRunning` — deliberately, since clarify precedes `pipeline_start` (on `808612bf`: `questionnaire_ready` @5, `pipeline_start` @11), so gating it would break a LIVE clarify — and the header merely fell from "Awaiting approval" to **"Clarifying"** with Stop still armed; on that run the lingering panel is itself ISS-123 damage, its `questionnaire_complete` row being one of the 15 destroyed events. **`reviewGateData` is deliberately NOT cleared** — it is already masked by the gate branch's `&& isPipelineRunning`, and clearing it alone is WORSE than nothing (`runLaneState` falls to `"building"` → header "Running", Stop still present). The fix is also **not** placed in the `case "pipeline_cancelled": case "pipeline_failed":` switch arm, which is **verified dead code** (ISS-139) and pinned as such by test. | Nothing in `frontend/src` derived the run-state terminal markers from the server's persisted `workflow_runs.status`; the run screen treated the terminal EVENT as the source of truth, inverting what every other consumer (boot restore scan, RunState fence, history badges, FIX-240's own SSE guard) already does. | `frontend/src/hooks/useWorkflow.ts`, `frontend/src/app/dashboard/page.tsx`, `frontend/src/hooks/__tests__/terminalStatusReconcile.test.ts`, `frontend/src/app/dashboard/terminalReopenReconcile.source.test.ts` | quick-260812-wir | INV-3 ✅ (frontend-only; goldens untouched, 0 files moved) · INV-12 ✅ (**reduces** terminal-marker deciders 3 → 1; the two status sets pinned EQUAL by test; no second source of truth added) · SC-001/INV-1 ✅ (status vocabulary only — no workflow name) · Ports & Adapters N/A | Done · tests: 23 new frontend ✅ (20 **seen RED at `ce2db22e`**; the 3 added after the browser findings pin strings **proven absent at `e3c38594`**) · full vitest 871 → **894 passed** with the failing set **147 → 147 and the failing test IDs BYTE-IDENTICAL** to a baseline measured in a **detached worktree** at `e3c38594` (never a stash) · `tsc` the same 2 pre-existing errors · **LIVE A/B on Bedrock dev data:** `808612bf` (corrupted) Stop-present + "Awaiting approval" → `lane-run-status` **"Cancelled"** (tone=neutral), no Stop; `1ea6d262` (clean post-FIX-240 control) "Cancelled", no Stop, **UNCHANGED** |
| FIX-246 | 2026-08-12 | **The historical event loss was recorded as an unmeasured range with a half-wrong justification (ISS-123); it is now a read-only, re-runnable audit.** Shipped `backend/scripts/audit_lost_run_events.py` — `mode=ro`, zero `INSERT`/`UPDATE`/`DELETE`/`ALTER`. It **simulates the engine seq allocator** rather than looking for holes, because a gap proves nothing: the stealing app writer OCCUPIES the seq, so a steal destroys an engine event only when the engine's counter actually WALKS onto it. Two loss signatures: `WALKED-ONTO`, and `ORPHAN-CARD` — a narrator `chat_reply:{uuid}` card whose source uuid has no row, written by the always-lands allocator so it outlives its own source and acts as a **tombstone that NAMES the destroyed event**. **Corrects the register: 15 destroyed events across 9 runs, not ~29 across 10** (`agent_chunk` 11, `questionnaire_complete` 2, `review_gate_approved` 1, `pipeline_cancelled` 1) — the 29 counted seq-stealing app ROWS, an upper bound, not the loss. **Also corrects the reason for refusing back-fill.** "A dropped row has no provenance to reconstruct from" is FALSE for 11 of the 15: they are `agent_chunk` fragments and `artifact_refs.content` is an independent authoritative record of the same text — the audit sizes each hole against it and reproduces the lost strings **exactly by length** (1 = the document's leading `#`, 4, 14, 26, 27). **The correct reason is narrower and stronger: the CONTENT is recoverable, the IDENTITY is not.** `event_id` is the key the whole replay contract rests on (`replayed_event_ids`, `append_event_next_seq` idempotency, the FE `seenRef`), so a back-fill must mint an `event_id` that never existed — making the log assert the engine emitted a frame it did not, with a de-dup key no live frame can match. The four structural losses are unrecoverable outright: `workflow_clarifications` is EMPTY corpus-wide and `gate_events` records NO approve outcome for any run. **Three self-corrections are documented IN the script** because each would silently inflate or misattribute the damage: post-FIX-240 runs show the identical walked-onto shape but lost nothing (the row was re-appended at max+1 — only creation time discriminates, hence `--fix240-utc`, and counting them gave 16/10); a destroyed `review_gate_approved` reads as "unknown" unless the scan looks back PAST the narrator card that always follows a `review_gate_ready`; and sizing a hole against the SUM of `artifact_refs` rows double-counts a re-run agent's versions, reporting a 1-character hole as 18,718. | ISS-123 was filed from a steal count rather than a loss simulation, and its recoverability claim was asserted rather than tested — so the damage was overstated by ~2x and the strongest argument against repair was never stated. | `backend/scripts/audit_lost_run_events.py` | quick-260812-wir | INV-3 ✅ (read-only script; goldens 10 passed, 0 files moved) · INV-12 ✅ (no repair path, no second writer) · Ports & Adapters ✅ (standalone script; lint-imports 4 kept / 0 broken) · Q3 N/A | Done · tests: N/A by design — a read-only forensic script whose output IS the evidence; verified by running it (15 across 9 runs, `--json` agreeing) and by `grep -c` proving zero write verbs |
| FIX-247 | 2026-08-13 | **Stop at the clarify gate cancelled the run, but the screen never repainted — the live terminal handler cleared only legacy React state, never the run store the lane actually reads (ISS-140), and the one piece of code that did it correctly sat in a switch arm that could never execute (ISS-139).** The lane stayed on "Clarifying" with Stop still armed and the clarify questionnaire still submittable, on a run the backend had genuinely already cancelled. **The live `pipeline_cancelled`/`pipeline_failed` handler is the `if (pipelineTypes.includes(msg.type))` block, NOT the `switch` below it** — both message types are members of `pipelineTypes`, whose branch always `return`s first, so the `case "pipeline_cancelled": case "pipeline_failed":` arm at the old `page.tsx:1527-1562` was 36 lines of **unreachable** code that nonetheless contained the mechanically correct `runStore.update(..., {questionnaireData: null})`. **Two source-lock tests had pinned that dead arm and were GREEN while pinning code that had never once executed** — a lock satisfied only by unreachable code proves nothing. What shipped: the reachable live block now does `if (frameRunId) runStore.update(frameRunId, { questionnaireData: null })` inside the existing run-scoping `if (!isForeignFrame)` guard (`page.tsx:1206`); the dead arm is DELETED; and the arm's two genuinely live-relevant effects — `setLastCancelledRunId` and `detachRunRef.current?.()` — are rescued onto the reachable path, **the first time either has ever executed on a live cancel**. Both source-lock files were reconciled, TIGHTER not relaxed: `liveRunSwitch.fix201.test.ts` (an unanticipated SECOND lock on the same dead string, found mid-execution) now also asserts the write stays inside `if (!isForeignFrame)`. **`reviewGateData` is deliberately NOT added to the store patch:** that half really IS masked, by the gate branch's own `&& isPipelineRunning`, and clearing it would erase the historical gate card from a reopened transcript — which `terminalReopenReconcile.source.test.ts` already locks against. Live-proven on Bedrock with two fresh cheap `user_stories` runs, each sampled at t+3s/8s/15s/25s after Stop: FIXPROBE (clarify armed, 15 chips / 4 questions) and GATEPROBE (review gate armed, the regression control this fix does not touch) both settle to "Cancelled" with no Stop, no clarify submit, no gate actions, a terminal-cancelled card and API status `cancelled`. | `laneClarifyOpen` (`DashboardLayout.tsx:1862`) is the ONE branch of the `runLaneState` ternary (`DashboardLayout.tsx:1864-1871`) NOT AND-ed with `isRunning` — deliberately, because clarify precedes `pipeline_start` — and it is evaluated BEFORE the terminal branch. So a lingering store `questionnaireData` OUTRANKED the terminal markers the same cancelled frame had already applied correctly. The lane reads the STORE (`page.tsx:2731`, `questionnaireData={runStore.viewed.questionnaireData}`); the three legacy setters the handler did call (`setReviewGateData`, `setQuestionnaireData`, `setActivePipelineRunId`) are invisible to it. **ISS-140's filed text was WRONG and is corrected by this fix:** it claimed the defect was "MASKED by the `isRunning` conjunct ... not user-visible today". `laneClarifyOpen` has no `isRunning` conjunct at all; the masking claim only ever held for the `reviewGateData` half of that row, never for the `questionnaireData` half, which was the live, user-visible cause all along. Disproven twice — once by source reading, once by live browser reproduction — hence that row's severity is raised minor -> major. | `frontend/src/app/dashboard/page.tsx`, `frontend/src/app/dashboard/terminalReopenReconcile.source.test.ts`, `frontend/src/app/dashboard/liveRunSwitch.fix201.test.ts`, `frontend/e2e/tests/ts-r.cancel.spec.ts` | quick-260813-1b1 | INV-1 ✅ (frontend-only; zero backend and zero kernel files) · INV-3 ✅ (goldens 10 passed / 0 failed, 0 golden files touched, IDENTICAL to the pre-change commit `d2d2da53`) · INV-12 ✅ (the superseded dead arm is DELETED, not left standing beside the live path — one terminal-clearing site, not two) · Ports & Adapters ✅ (lint-imports 4 kept / 0 broken, run from `backend/`) | Done · tests: 1 new mocked-e2e (TS-R-05, seen RED independently twice with an identical failure, then 4 passed / 1 pre-existing fixme skipped) + 2 reconciled source-lock files (35 vitest tests across 2 files, all green) + a PAIRED full-suite regression measurement (reverted 34 failed / 108 passed vs HEAD 31 failed / 111 passed) whose set-diff shows ZERO tests newly failing ✅ |
| FIX-249 | 2026-08-13 | **The Concierge told a user the product "does not expose billing or token usage metrics" and referred them to support, for a number rendered inches away in the same viewport (ISS-148).** Asked "how many tokens did this run use in total, and what did it cost?", it recited a false product limitation. Per the original investigation (run `5ecb990f-2c80-4752-a615-2bed3280387a` `chat_usage` rows — not independently re-observed this session, Bedrock unreachable): the question was answered in a SINGLE bare model turn, 4,816 input tokens, 0 cache-read, ZERO tool calls, versus 9,937-12,781 input tokens with cache reads for tool-using sibling questions in the same session. **Two independent, verified causes, both fixed.** `concierge.py:898` (pre-fix) forbade the model from exposing "token counts" in its RESPONSE RULES, paired with a `:893-895` denial template ("say they are not available") written for internal identifiers that the model over-applied. Separately, `_read_tools` (`:867-870`, pre-fix) returned exactly 7 tools; none touched `workflow_runs.token_usage`, already-populated JSON (`app/models/workflow.py:36`, written at `app/api/run_commands.py:2332-2354`). **A second instance of the same failure mode fired in the same investigation and was initially graded correct** — asked what FILE the run produced, the Concierge gave the document TITLE, never `workflow_runs.deliverable_filename`, because `ArtifactRef` (`app/models/artifact_ref.py`) has no filename column for `list_artifacts` (`concierge.py:792-801`) to project; filed separately (ISS-149), deliberately not fixed here. **Fix, both halves, inside `concierge.py` only.** Half 1: a new no-parameter `get_token_usage` tool added after `read_gate_history`, reading the run via the EXISTING `_safe_run(scoped_store, run_id)` helper (no new ScopedStore method, no raw ORM; the tolerant `app/api/analytics.py:66 _parse_token_usage` was deliberately NOT reused — importing it would create an `app.agents.chat -> app.api` layering edge, so the in-file `_agent_output_entries` guarded-`json.loads` idiom is mirrored instead). Returns EXACTLY `available/total_tokens/input_tokens/output_tokens/estimated_cost_usd` — an EXACT key-set, not a subset, asserted by test — so cache read/write, `model_id`, and the uncached-counterfactual cost (all present in the same source blob) cannot become answerable by accident; deliberately deferred (ISS-149), not incidentally included. Degrades to `{"available": false, "error": "..."}` — never zeros — when the column is `None` (a legitimate state: the writer only persists it when `total_input + total_output > 0`) or the blob fails to parse; the `error` key is LOAD-BEARING, since the standing cross-owner security guard `test_every_read_tool_denies_cross_owner` accepts a denied/degraded result only if it is falsy, `{}`, carries a truthy `error`, or has `agents_started == 0` — a bare `{"available": false}` would have turned that guard red. Half 2 (LOAD-BEARING — a tool alone would have fixed one question and left the failure MODE live for every other gap with no tool): removed `token counts, ` from the forbidden list; added a new honesty rule — if no tool can answer, say so plainly ("I can't see that from here") and stop, never claim what the product does or does not support, never invent a reason, never redirect to billing/support; added one TOOLS routing line. `model names` deliberately left forbidden — a genuine open product question, surfaced not decided (ISS-149). | `concierge.py:898`'s RESPONSE RULES literally forbade exposing "token counts", paired with a `:893-895` denial template the model over-applied from the neighboring internal-identifiers rule — and no read tool existed that could have answered honestly even without that rule (`_read_tools` returned exactly 7 tools, none touching `workflow_runs.token_usage`). | `backend/app/agents/chat/concierge.py`, `backend/tests/agents/test_concierge_capability.py` | quick-260813-5qr | INV-1 ✅ (no workflow-name literal; a generic run-scoped tool) · INV-3 ✅ (goldens 10 passed / 0 failed, 0 golden files touched) · INV-12 ✅ (no dual implementation — the tolerant `app.api.analytics._parse_token_usage` was deliberately NOT reused/duplicated, avoiding an `app.agents.chat -> app.api` layering edge; the in-file guarded-`json.loads` idiom is mirrored instead) · INV-13 ✅ (the new tool constructs no model/agent/runner — a plain data read shaped exactly like its 7 siblings) · Ports & Adapters ✅ (lint-imports 4 kept / 0 broken, run from `backend/`; entirely inside the already-registered `chat:concierge` capability — no kernel, registry, migration, or manifest edit) | Done · tests: 2 existing UPDATED (never weakened) + 4 new backend unit ✅ (`test_concierge_capability.py` isolated: baseline 34 collected/33 passed/1 failed [1 pre-existing, unrelated — ISS-151] → RED 38/31/7 → GREEN 38/37/1 [same pre-existing, unchanged], independently re-measured by the orchestrating agent, not just the executor's report) · goldens 10 passed/0 touched · lint-imports 4/0 · extra gates run for completeness: `test_sc001_lane_router_concierge.py` (9 passed) + `test_banned_patterns.py` (14 passed) = 23 passed · **LIVE-PROVEN 2026-08-13 by the orchestrator, after SSO was restored** — re-asked verbatim on the EXISTING COMPLETED run `5ecb990f` (launches nothing, ~1c/question) via `scratchpad/25-concierge-proof.mjs`. **THE FIX:** *"This run used **22,376,019 total tokens** (22.19M input, 184K output) and cost approximately **$4.49 USD**."* — exactly `workflow_runs.token_usage` (`total_tokens=22376019`, `estimated_cost_usd=4.485229`), where the SAME question previously returned *"Token counts and cost data are not available … contact support"*. **REGRESSION CONTROL** (unchanged, still exact): *"5 agents ran in this build. The Task Planner Agent took the longest at 220.93 seconds."* **NEGATIVE CONTROL** (prompt-honesty half, asked about prompt-cache savings — a genuine gap): *"I don't have visibility into prompt caching metrics from the run assistant tools. Token usage shows total tokens consumed, but the breakdown between cached and non-cached tokens is not available through this interface."* — the three worst pre-fix behaviours are GONE (no "contact support", no billing/account-dashboard redirect, no "managed at the platform level"), and it correctly scopes the limit to its own tools. **Residual, filed under ISS-149, NOT a regression:** the trailing clause "not available through this interface" still asserts unavailability of data that DOES exist (`total_cache_read_tokens=21376863`, `total_cache_write_tokens=814284`) and is rendered by `TokenUsageSummary.tsx` — the honesty rule curbed the fabrication but did not fully eliminate scope-claiming. The earlier blocked attempt is retained for the record: AWS SSO expired — confirmed by that session (`aws sts get-caller-identity --profile hex-uki` → `Token has expired and refresh failed`; default profile → `InvalidClientTokenId`) and independently corroborated by the immediately-prior session's own reproduction through the backend's actual Bedrock call path (FIX-248's blocker). Re-ask script `scratchpad/21-concierge.mjs` is ready, unmodified, for that follow-up (~$0.01 on Haiku, launches nothing). |
| FIX-248 | 2026-08-13 | **The narrator's "Delivered — open in Preview" milestone card rendered TWICE in the run chat lane after a mid-run SSE reconnect (ISS-146).** Two endpoints project the SAME durable `run_events` row differently: `GET /api/runs/{id}/events` (`api.ts:583-590`, since the BUG-018 fix) merges the row's `event_id`/`seq` COLUMNS over `payload_json`; the SSE endpoint's durable-replay step (`run_stream.py:202`) discarded the columns and served `payload_json` RAW. Harmless for engine-authored events — the engine stamps identity INTO the payload itself before persisting (`engine.py:1085-1086`) — but four app-layer types (`chat_reply`, `chat_message`, `chat_usage`, `run_resuming`) persist with NO identity embedded, so for those rows the columns are the ONLY identity that exists. The narrator's card is one such row (`chat_narrator.persist_milestone_card`); its real durable key (`chat_reply:{source_event_id}`, UNDERSCORE, keyed on the source milestone's own uuid) can never be reproduced by the frontend's fallback (`` chat-reply:${message_id} ``, HYPHEN) — so a card delivered once live (real key) and once via a replayed reconnect (anonymous, pre-fix) renders as two differently-keyed bubbles. **The production trigger is more specific than "any reconnect," and was traced end to end, not assumed:** `pipeline_complete` IS a member of the SSE endpoint's own `_STREAM_TERMINAL_TYPES` (`run_stream.py:79`), so the live drain `return`s the instant it is drained — the narrator card, queued by the engine immediately AFTER `pipeline_complete` (`engine.py:1104-1132`), is **never delivered live on that connection at all**. `useRunStream.ts:301` does not treat `pipeline_complete` as a non-live close (a separate, deliberately unfixed gap — see ISS-147), so the client reconnects, carrying `Last-Event-ID = <completeSeq>`; the server replays the card off the durable tail (pre-fix, anonymous) — racing the completion backfill (`page.tsx` → `getRunEvents`) that independently fetches the SAME row, WITH identity, via `detachRun`'s FIX-172 "additive insurance" (`page.tsx:967-980`). That insurance is defeated whenever the run's id is ALSO in `RunConnectionProvider`'s `autoIdsRef` (armed by an `online`/`visibilitychange` wake firing WHILE the run is still active — exactly what a mid-run network blip does), because `detachRun` only clears the sticky-focus half of `liveRunIds = union(autoIdsRef, focusedRunIdRef)`. **Fix (Option A, one line, server-side):** merge the row's `event_id`/`seq` columns over `payload_json` at the replay yield, adopting the convention the REST twin already used — `yield _sse_frame(r.seq, r.type, {**(r.payload_json or {}), "event_id": r.event_id, "seq": r.seq})`. No frontend file touched; the frontend's keying/dedup logic was already correct, it simply never received the identity to key on. | `run_stream.py:202`'s durable-replay yield served `r.payload_json` raw instead of merging the row's `event_id`/`seq` COLUMNS — the sole identity source for the 4 app-layer event types that embed none in their own payload. | `backend/app/api/run_stream.py`, `backend/tests/unit/test_sse_stream.py`, `frontend/e2e/fixtures/mockSse.ts`, `frontend/e2e/tests/ts-sse-resilience.spec.ts` | quick-260813-3wo | INV-1 ✅ (no workflow/pipeline-type literal) · INV-3 ✅ (goldens 10 passed / 0 failed, 0 golden files touched; `seq`/`event_id` already in `_VOLATILE_STRIP_KEYS`, `_normalize.py:129-130`, so the narrator's dormancy on all 5 goldens is untouched) · INV-12 ✅ (repairs a wire-contract divergence rather than adding a third mapping; the mock's temporary pre-fix-modeling flag was deleted in the same change, not left dormant) · Ports & Adapters ✅ (lint-imports 4 kept / 0 broken, run from `backend/`) | Done · tests: 1 new backend unit (`TestReplayIdentityProjection`, seen RED — `event_id` absent — then GREEN, `test_sse_stream.py` 43→44 passed/0 failed) + 1 new mounted-browser e2e (`TS-SSE-RESILIENCE-06`, seen RED — 2 cards — then GREEN — 1 card, full file 5 passed) ✅ · **LIVE-PROVEN 2026-08-13 by the orchestrator, after SSO was restored** — like-for-like with the SAME script that produced the pre-fix measurement (`scratchpad/20-live-control.mjs`), on a backend restarted onto this code: **`DROP=1` (`SSEPROOFDROP`) → `{delivered:1, openInPreview:1, runStarted:1, clarAnswered:1}`**, against the recorded pre-fix **`{delivered:2, openInPreview:2}`**; **`DROP=0` control (`SSEPROOFCTRL`) → `{delivered:1, openInPreview:1}`**, unchanged, so the normal path did not regress. Both runs reached `completed`. (Note: `scratchpad/24-live-control-v2.mjs`, written during the blocked session, is UNSOUND — it treats `planning` as "past clarify" and so fires the drop before the clarify gate, measuring all-zeros; use `20-live-control.mjs`.) The earlier blocked attempt is retained for the record: two `user_stories` attempts (`SSEFIXPROBEV2`/`V3`) failed before reaching a deliverable — `botocore.errorfactory.InvalidGrantException: Invalid refresh token provided` on BOTH the `hex-uki` and `hex-ai-fe` AWS SSO profiles, confirmed via `aws sts get-caller-identity`/`aws sso login` (the latter needs an interactive browser step no agent can complete). Infrastructure/credentials issue, unrelated to this fix — a human must complete an SSO login before a live pass can run; a corrected reproduction script (`scratchpad/24-live-control-v2.mjs`, handles a second clarify round + gates the network-drop on real run status) is ready for that follow-up. |
| FIX-250 | 2026-08-13 | **Every completed revision run silently lost 7 `WorkflowRun` output columns — the fourth caller `BUG-R03` never wired, and the row it defeated (`get_token_usage`, FIX-249) had shipped the SAME session (ISS-152).** `_drive_revision_to_queue` (`run_commands.py:2764`) is the app-layer dispatch path for EVERY `*_revision` pipeline run — Concierge disposal, chat, and `POST /{id}/revisions` all funnel into it. Its `_persist_terminal_status` closure wrote ONLY `status`/`completed_at`; it never called `_apply_terminal_output_columns` (`:2237`), the function documented as "the SINGLE event→WorkflowRun output-column mapping" and already wired into the launch driver and both resume entry points. `BUG-R03`/`QUICK-260719-hvn` (commit `d68b8b75`) wired three of four callers — its own investigation cleared the revision driver by grepping it for `.output`/`.agent_outputs` READS and finding none, never checking whether it WROTE them. Verified against `8a970205-5892-466a-89b0-f613b9dd021d` (the only revision that had ever completed in this database — rate 1/1): all 7 columns (`output`, `agent_outputs`, `token_usage`, `duration`, `model_id`, `deliverable_mimetype`, `deliverable_filename`) NULL despite a real deliverable existing. The engine was innocent: `_handle_revision` (`engine.py:6360-6612`) already forwards every event — including `pipeline_complete`'s full payload — verbatim through the caller-supplied `websocket_send_fn`; the driver simply never accumulated what it already received. **Blast radius:** Analytics counted every revision as $0 cost AND, as a zero-duration-numerator `completed` row, actively dragged down `avg_duration` in the denominator (`analytics.py:184-238`); the Run Detail KPI strip hid entirely (`GET /{id}/summary` returned `duration=None, token_usage={}, agents=[]`); `GET /{id}/chain-context` returned an empty context block for anything chained off a revision; and it silently defeated FIX-249's `get_token_usage` tool for every revision run (`{"available": false}` whenever the column was NULL, which for a revision was always). Severity MAJOR (matches the originating investigation and the orchestrator's independent agreement — top of band, not critical: nothing destroyed, the live deliverable itself was always correct, and the data is fully recoverable from `run_events`). **Fix, `run_commands.py` only, mirrors `_drive_launch_to_queue` byte-for-byte:** `_queue_send` now accumulates `raw_events` unconditionally; `monotonic_start = time.monotonic()` captured before dispatch; `_persist_terminal_status` calls `_apply_terminal_output_columns(swr, raw_events, model_id=getattr(user, "preferred_model", None) or None, duration_seconds=round(time.monotonic() - monotonic_start, 1))` inside its existing session before `sdb.commit()` — one session, one commit. Deliberately uses the driver's OWN monotonic clock, never `pipeline_complete.total_duration`, so ISS-150 (two disagreeing duration numbers) isn't made worse by a third source. `_apply_terminal_output_columns`'s own docstring (which enumerates its callers) corrected to name the revision driver as the fourth. No engine change; `_apply_terminal_output_columns` itself untouched apart from its docstring — stays the sole writer, no second implementation (INV-3/INV-12). **Two items surfaced, deliberately not performed here:** the `wr.error` omission in the SAME function (filed ISS-153 — a failed revision gets no error message at all) and the one-row backfill of `8a970205`'s 7 columns from its own durable `run_events` (lossless per this fix's own investigation; left as an open owner decision, not silently performed). **A genuine third finding surfaced during this fix's OWN live verification, NOT caused by this fix** (filed ISS-154): switching the in-family version picker to an older, non-latest revision whose `.output` is still NULL shows a blank preview (`PreviewPanel.tsx:528-543`) — a code path structurally impossible to exercise before this session, since no family had 3 versions to switch between until this fix's own live test created one. | `_drive_revision_to_queue`'s terminal-write closure (`_persist_terminal_status`) was never updated when `BUG-R03` introduced `_apply_terminal_output_columns` and wired it into the launch driver and both resume entry points — the revision driver was the fourth caller that shipped without the wiring, because the prior fix's own investigation audited it only as a reader of the output columns, never as a writer. | `backend/app/api/run_commands.py`, `backend/tests/unit/test_rest_revisions.py` | quick-260813-as6 | INV-3 ✅ (goldens 10 passed/0 touched; existing prototype/od_*/PPT/code-gen behavior deterministic-byte-identical, no engine edit) · INV-12 ✅ (no dual implementation — `_apply_terminal_output_columns` stays the SOLE writer, itself unmodified apart from its docstring) · SC-001 ✅ (no workflow-name literal; the fix is workflow-agnostic, matching the launch driver's own generic shape) · Ports & Adapters ✅ (lint-imports 4 kept/0 broken, run from `backend/`; app-layer-only change, no kernel/engine edit) | Done · tests: 1 new backend unit ✅ (`test_rest_revisions.py` isolated: RED `AssertionError: assert None == '<html>revised</html>'` → GREEN `1 passed`) · full regression set (`test_rest_revisions.py` + `test_rest_run_launch.py`, 39 pre-existing tests): 4 pre-existing failures, proven byte-identical against a disposable worktree at the pre-fix commit `48c76403` (never `git stash`), unrelated to this change (`_FakeUser` missing `.tier` — a documented pre-existing defect class, ISS-102/ISS-119) · goldens 10 passed/0 touched · lint-imports 4/0 · **LIVE-PROVEN 2026-08-13 on Bedrock** (backend restarted first to pick up the fix; SSO confirmed via a real model call, not `aws sts get-caller-identity`) — dispatched via the deterministic `POST /{id}/revisions` REST endpoint (the prescribed chat-driven script hit unrelated live-LLM tool-classification variance, not filed on one data point) against the existing completed `user_stories` parent `a5d059e1...`: new revision `5914e5f1...` completed in 8s with all 7 columns populated and mutually consistent (`token_usage.total_tokens=4245` == `agent_outputs`' own total == the Concierge's later answer); History row showed `v3 · 4.2K tokens` inline; reopened family rendered the new criterion verbatim; **the Concierge, asked "How many tokens did this run use in total, and what did it cost?", answered "This run used 4,245 total tokens and cost an estimated $0.01" — live proof the FIX-249 interaction is repaired for revisions**; Analytics rendered a real non-zero total ($18.28/22 completed runs) with a `<$0.01` revision-attributed slice where every revision previously forced a hard `$0.00`. Zero HTTP≥400 responses during the entire live pass. |
| FIX-255 | 2026-08-13 | **A run with no brief could be launched, and the pipeline it could launch is the 5-21M-token one.** `LaunchCommand.message` was a bare `str` and `launch_run` never checked it. Proven by a RED test on `785e40c1`: an empty brief returned **200 OK and minted a run**. | Not one bug but a chain of three, each individually defensible. (1) `LaunchWizard.canContinue` validated a PROXY — `isChaining \|\| brief.trim()` — waiving the brief while chaining on the assumption a context block would stand in, without ever checking the block exists. (2) `handleLaunch` falls through to `brief.trim()`, the empty string, when that block is absent — and dev's FIX-217 made "absent" MORE reachable by clearing `chain.context_block` up front while a failed `getChainContext` stays non-fatal. (3) dev's FIX-216c removed the last net, the `!draft.brief` staging guard, citing a wizard guard that does not cover the chained case. Nothing server-side ever looked. | `backend/app/api/run_commands.py`, `backend/tests/unit/test_rest_run_launch.py`, `frontend/src/components/workflow/LaunchWizard.tsx`, `frontend/src/app/dashboard/page.tsx` | 2026-08-13 dev merge (`785e40c1`); successor to dev's FIX-216c/FIX-217 | INV-1/3/12/SC-001 ✅ (goldens 10/10 zero regenerated; lint-imports 4/0; tsc 4 errors → 2) | Done · tests: 3 new (TEST-034), all seen RED first · **LIVE-VERIFIED** 2026-08-13: ingress returns 400 `empty_brief` for `""` and whitespace with run rows 46→46; wizard shows "Add a brief" + disabled Continue with no context block, and enabled Continue with one (dev's UX preserved) |
| FIX-256 | 2026-08-13 | **The FIX-201 legacy→store bridge could resurrect a stood-down run, and dev shipped a second never-called mechanism beside it.** Closes ISS-138 (open since 2026-08-12) and ISS-157. | The bridge ran `updatePipelineState(runId, () => snapshot)` — a WHOLESALE replace. `activelyBuildingRunIdRef` is only ever assigned, never cleared, and reopening a run from history assigns it (`page.tsx:2226`), so a TERMINAL run reaches the bridge while the legacy container still carries `isRunning:true` from a terminal-less replay. Live evidence (quick-260812-wir): the store flipped back to `isRunning:true` ~4s after being set false with no `handleFrame` involved; it cost a real false-green during FIX-245. dev's FIX-220 rAF-coalesced the loop but left the replace intact, and added `syncPipelineStateOnly` — never called, and justified by a FALSE premise ("handleFrame projects on every SSE frame"; it returns early before any projection on `waveIndex === undefined` and on `!pipelineFrameTypes.includes(msg.type)`). | `frontend/src/app/dashboard/page.tsx`, `frontend/src/hooks/useRunStateStore.ts`, `frontend/src/app/dashboard/bridgeTerminality.source.test.ts` | 2026-08-13 dev merge; closes ISS-138, ISS-157; keeps dev's FIX-220 rAF | INV-1/3/12/SC-001 ✅ (INV-12: terminality keeps ONE definition — the shared `applyTerminalStatus`, one-way by construction; the dead second mechanism DELETED) | Done · tests: 11 (TEST-035), seen RED 6 failed/5 passed on `785e40c1` · **LIVE A/B** 2026-08-13 vs a pre-fix frontend on :3001, same backend, 7 samples over 30s: **identical, both hold terminal** — the symptom was already masked by FIX-245, so this removes the LATENT hazard ISS-138 names, and the live pass proves no regression |
| FIX-257 | 2026-08-13 | **The Concierge's file-attachment prompt forbade a tool that no longer exists, and converted questions into steering notes.** | Two faults from dev's FIX-218 being authored against a superseded tool surface. (1) It says "NEVER call read_events when a file is attached" — FIX-233 DELETED `read_events`/`list_refs`/`get_ref` after measuring one question re-feeding 9,227,107 chars (~2.3M tokens); inert, but dead prompt text on every attachment turn. (2) CASE B routed "user just attached a file OR ASKED ABOUT STATUS" to `propose_steering_note`, which the later rule "NEVER call propose_steering_note for a status question" directly contradicts — so a question was silently converted into a steering note. | `backend/app/agents/chat/concierge.py` | 2026-08-13 dev merge; successor to dev's FIX-218 (KAN-170), depends on FIX-233 | INV-1/3/12/SC-001 ✅ (no workflow-name literal; prompt-only, goldens byte-identical) | Done · **LIVE-VERIFIED** on Bedrock 2026-08-13, run `9f3d81a7`, all three routes: A (revision+file) → `propose_revision` honouring the user's instruction; B (statement+file) → "injected its content into the pipeline agents" (dev's feature intact); C (question+file) → **answered** "the approval threshold is **50 units**" + the 500 GBP cap. Run rows 46→46 — the confirm-hold held, nothing launched. No automated test: prompt behaviour, verified live |
