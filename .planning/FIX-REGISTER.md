...
|--------|------|-------------|------------|---------------|---------------|------------|--------|
| FIX-152 | 2026-07-30 | KAN-143: CloudWatch agent liveness assertion missing from bootstrap — fetch-config returns 0 on fork, not on readiness; agent crashes undetected while bootstrap succeeds | `bootstrap-ec2.sh:1087-1092` (old): `fetch-config -s` wrapped with `|| exit 1` guard, but exit status only proves fetch-config succeeded, not that the agent reached running state. `systemctl restart` returns on fork, agent can crash milliseconds later while bootstrap proceeds to §15-§20, leaving fully-provisioned host with blind observability (crash-loop agent was observed on prod: restart counter 977 over 16h). Root: no liveness check after fetch-config. Fix: add `assert_cloudwatch_agent_running()` function with three windows (settle 60s poll until `is-active`, stability 12s verify stays active, NRestarts cross-check) + degraded-completion accumulator pattern: loud but non-fatal, appends to accumulator on failure, §20 deferred gate writes sentinel unconditionally then exits non-zero iff degraded. All §15-§20 run regardless, preventing application stranding (if abort at §14: loses deploy user, pg_dump, app start, sentinel). | `infra/scripts/bootstrap-ec2.sh` (lines 34-41 new accumulator, lines 1087-1215 new function+call, lines 1501-1516 new deferred gate) | quick-260730-b5a (infrastructure, no schema/migration, infra-only; infrastructure-only, no Flowin engine/app/schema) | INV-1/3/12/SC-001 ✅ (infra-only, no engine edits; characterization goldens not affected; INV-12 ✅ one implementation of assertion, called once; deferred pattern reusable for future loud-but-non-fatal checks) | Done |
