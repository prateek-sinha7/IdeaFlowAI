...
---
*Last updated: 2026-06-12 — Phase 14 COMPLETE (run_revision real revision loop, F2 end-to-end; 14/14 phases, milestone execution finished). The Phase-3 echo stub in `_handle_revision` is deleted: dispatch now resolves `get_pipeline_agents(<WR-06 alias>)` and drives the real revision pipelines through the normal `execute()` path with the composed three-section context as input, `od_context=None` (design wrinkle settled with evidence — zero revision agents declare `injects`; the 13-06 `missing_template_context` guard sits only on the `run_pipeline` ingress, so revision dispatch is exempt by construction, pinned executable by an injects-free CI test). Events forward verbatim with single-source seq (`_rev_sink`/`_stamped_send` duplicate-stamping path + fake `pipeline_complete` pair deleted, INV-12). Both run_revision manifests flipped `planner: run → skip` (data-only, INV-5) removing the indefinite clarify `event.wait()` hang; BOTH planner parity traps pin the two-id carve-out (the second trap, `test_id_alias_resolver`, was a planning-time discovery research had missed). WS `run_revision` moved off the inline await onto background-task + per-run queue + drainer (`_handle_revision_execution`) with event-derived terminal status (completed/failed/cancelled/degraded — failed revisions can never be offered as revision-of-revision parents) and registry-derived agent_count. Lineage: guarded exact-kind write (`derived_from=original.id`, `producer_agent=agents[-1].id`, owner + parent workspace, `visibility="workspace"`); revision-of-revision resolves via FR-014 chain link 1 (pinned). Both stub-pinning suites rewritten to the real-dispatch contract (8 ownership/falsy-owner guards kept byte-meaning-identical + new AUTHZ-03 falsy-owner pin). Code review: iteration 1 found 2 critical + 4 warnings, ALL 6 fixed same-phase (CR-01 run_revision overlap guard — cancel handle never overwritten, double-dispatch blocked; CR-02 pre-dispatch `planner != "skip"` ValueError from compiled-manifest data, SC-001-clean — client-controlled `target_artifact_type` can no longer park a run at the clarify gate; WR-01 post-terminal `state_restoration_failed` delivery via residual drain; WR-02 degraded-status fidelity; WR-03 reconnect `section` contract via inverse WR-06 transform; WR-04 stale AUTHZ comment) + 5 pinning tests; iteration 2 re-review: 0 critical / 0 warning / 5 info. Evidence: VERIFICATION passed 4/4; targeted battery 231 passed (zero golden re-baselines, INV-3 held with engine.py touched; lint-imports 4/0; SC-001/INV-12 greps clean). Deferred per convention to the milestone-end live pass: SC4 live-Bedrock FE-exact `run_revision` frame → revised deck in preview, 14-02 live cancel/reconnect, plus the pre-existing `test_pipeline_cancel` timing flake (proven identical pre-phase). Tracking warning logged at close: 113 REQ-IDs present in REQUIREMENTS.md body but missing from its Traceability table (project-wide doc drift, surfaced by `/gsd-progress`). Phase 14 SECURITY.md not yet generated — run `/gsd-secure-phase 14` before milestone close.*
