...
---
*Last updated: 2026-06-14 — Phase 21 (Saved Workflows) COMPLETE (21 phases, 104/104 plans). **Phase 21 — Saved / My Workflows** is the authoring layer atop the Phase 20 catalog: a user composes a custom workflow (agents + per-agent models), **names + saves** it from BOTH the catalog AND the main-page composer, and it appears as a renameable/deletable **"Your workflows"** catalog entry; launch replays `{base_pipeline_type, agent_ids, model_overrides}` through the EXISTING run path (re-validated at launch). Persistence **COMPLETES the half-built Phase-3/5 `workflows` table** (INV-12 — REUSE `source="user"`, NOT a new table): additive migration `0021` (+3 nullable cols `base_pipeline_type`/`model_overrides`/`description`) + a new owner-scoped `/api/user-workflows` CRUD router, and the dead `engine.py _persist_workflow_definition` writer it supersedes is **DELETED**. Reuse-first FE (NameWorkflowModal⟵DeleteModal, "Your workflows"+kebab⟵WorkflowHistory, api.ts CRUD⟵`admin*` fetchers, the `IdeaInputPage` `initialAgentIds` preload + guarded re-derive). Full auto chain (3 reuse-maps + a reuse-vs-new resolver → PRD→CONTEXT→pattern-map→plan→plan-check+fix→execute→code-review+fix→verify): plan-checker caught 2 broken verify-gates (a `grep -qvx` that always passed + unexecuted migration reversibility); code-review caught 4 (WR-01 picker clobbering seeded model overrides, empty-`agent_ids` orphan, `constitution_ref` overload, optimistic-delete-on-failure) — all fixed same-phase; the launch e2e caught a real pre-ship bug (saved custom agents resolve via `CUSTOM_AGENTS`, not `LIBRARY_AGENTS` → 0-agent no-op). Verify 5/5 with gates RUN: **INV-3 5 goldens byte-identical (42 passed)**, INV-12 (writer grep==0), additive migration (no `create_table`, reversible), `lint-imports` 4/0, 19 BE CRUD/IDOR tests, FE vitest 9 + Playwright `ts-z2` 2. Live re-check deferred to milestone-end. ——— Prior: Phase 20 (Workflow Catalog) COMPLETE (20 phases, 101/101 plans). **Phase 20 — Data-Driven Workflow Catalog** realizes WF-DB-01 and flips **ISS-015 from WONTFIX → SHIPPED**: a browse-and-launch catalog that reads the live `GET /api/workflows`, shows only `user_launchable` ∧ tier-entitled workflows, and launches through the EXISTING run/wizard paths — the SC-001 dividend made visible (**ZERO engine/kernel edits, NO new tables/migrations**). Additive backend = `user_launchable` + 4 display fields on the manifest schema (`_optional_bool` accept-only-bool; `_ALLOWED_TOP_KEYS` widened, `when/if/for/expr` still rejected = INV-5) surfaced through the existing endpoint; reuse-first FE = `WorkflowCatalog` grafts CreationHub rows/launch/tier-gating + the AgentModelPicker fetch shell, mounted as ONE new DashboardLayout `"catalog"` view + ONE AppHeader nav button (the hardcoded `WORKFLOWS` const DELETED → SC-001). Full auto chain (PRD→CONTEXT→pattern-map→plan→plan-check ×2→execute→code-review+fix→verify→complete): the plan-checker caught the INV-3-proof-not-executed blocker (goldens absent from the Task-3 verify command) **before** execution; code-review caught WR-01/02 (a BE `display_name` coalesce that defeated the FE friendly-label fallback, rendering the SPEC-forbidden raw id) → fixed same-phase (`accfe8fa`). Verify 5/5 against real code with gates RUN: **INV-3 5 goldens byte-identical (85 passed)**, `lint-imports` 4/0, FE vitest + mocked Playwright `ts-z.catalog` green. Live re-check deferred to the milestone-end Bedrock pass. ——— Prior: Phases 16–19 COMPLETE (99/99 plans) + live/Playwright re-confirmation. The entire 2026-06-13 deep root-cause investigation backlog (ISS-001…021; one Opus 4.8 agent per issue → root-cause/proper-fix/rejected-hack columns in `.planning/ISSUES-REGISTER.md`) is now closed (FIXED or WONTFIX) across four auto-driven GSD phases — each ran the full chain (investigate → CONTEXT → plan → plan-check → execute → code-review+fix → verify → complete) with INV-3 parity (5 goldens byte-identical) + SC-001 held at every step: **Phase 16** terminal-state/reconnect (ISS-016 major engine error-arm → `pipeline_failed`; ISS-007/002 cooperative cancel; ISS-017 degraded affordance; ISS-008/009 reconnect frame-contract); **Phase 17** test-infra (ISS-003 clarify-hang, ISS-010 in-memory OTLP test, ISS-011 skip-on-expiry — zero prod code); **Phase 18** custom-workflow UX (ISS-021 generic mimetype deliverable renderer; ISS-014 delete dead composer + relocate the per-agent model picker = MODEL-03 wired; ISS-019 wave-fold CSS; ISS-015 WONTFIX); **Phase 19** durable prompt/deliverable adherence (ISS-006 shared `getDatabase` contract; ISS-005 pure-stdlib `api_prefix` event-free post_step validator; ISS-004 engine streamed-`agent_chunk` tool-XML sanitizer). Plan-checkers caught 3 real pre-execution blockers (the WorkflowHistory reopen surface, the registry `discover()` gap, the cancel-insufficiency); code-reviews caught CR-01 (live custom HTML escaping) + ~10 warnings — all fixed same-phase. **Live + Playwright pass on `default` Bedrock** (`.planning/live-verification/CAMPAIGN-2026-06-13-phases16-19.md`): 6 fixes live+visually confirmed (ISS-019/021/007/014/005/006 — incl. a custom HTML "Habit Tracker" rendering in the generic Deliverable-Preview iframe, and a real Stop delivering `pipeline_cancelled` so cards clear), reconnect-contract live (008/009), and ISS-016(major)/017/004 offline-fault-injection proven (no clean live failure trigger on a healthy `default`). Remaining: 6 non-blocking backlog items (ISS-012 docs drift, ISS-018 srini/IT, ISS-020 harness note, ISS-022/025/026 pre-existing test failures, ISS-023/024 deferred cosmetics) — then `/gsd-complete-milestone`. ——— Phase 16 detail (full history in git): the engine consume loop gained an `elif etype=="error"` arm routing a swallowed `{"type":"error"}` (engine.py:2326-2377) into the existing `agent_error`→`pipeline_failed` machinery — the register's own `wait_for→timeout` cancel suggestion was proven necessary-but-insufficient; the cooperative `cancel_event` was the real fix.*

*(superseded detail — Phase 16) Closed the 6 root-caused issues of clusters A+B from the 2026-06-13 deep investigation (one Opus 4.8 agent per issue, 16 agents — full root-cause / proper-fix / rejected-hack analysis recorded in `.planning/ISSUES-REGISTER.md` → "Deep Root-Cause Investigation"). The phase made abnormal run outcomes faithful end-to-end: **ISS-016 (major)** — the engine agent event-consume loop now has an `elif etype=="error"` arm that routes a runner-surfaced model error (`ValidationException` etc.) into the existing `agent_error`→`_failed_agent_ids`→`pipeline_failed`/`status:degraded` machinery, so a failed run can no longer ride through as an empty `pipeline_complete` with DONE badges (the runner is unchanged; the arm is dormant on the scripted goldens; the discovery was a swallowed `{"type":"error"}` the consume loop had no branch for, at engine.py:2326-2377); **ISS-007/002** — `cancel_pipeline` now uses the engine's cooperative `cancel_event` instead of destructive `task.cancel()` (destructive kept only on disconnect) + closes the engine pre-agent cancel gap + `asyncio.timeout()` at the 3 drainers, so `pipeline_cancelled` actually reaches the live wire and the FE clears in-flight cards (`test_pipeline_cancel.py` now deterministic — the register's own `wait_for→timeout` suggestion was proven necessary-but-insufficient); **ISS-017** — the Preview shows a server-keyed degraded/failed affordance (not the neutral empty-state) for a terminal-empty run, on both live and history-reopen; **ISS-008/009** — durable-replay revision frames carry the owner-scoped `section` (WR-06 inverse) and the live-attach reconnect ack carries `live:true`. INV-3 held at every step (5 characterization goldens byte-identical, lint-imports 4/0, zero new tables/migrations); SC-001 honored (all new branches key on generic event types, never a workflow/model name). Code review: 0 critical / 3 warning + 2 real Info, all fixed same-phase (WR-02 error-text sanitized so no ARN/region/model-id reaches the browser; WR-01/IN-01 degraded-reopen + failed-agent wiring; WR-03 stale runner comment; IN-03 cancelled-wording). VERIFICATION passed 5/5; battery 31 phase tests + 44 parity/gate (7 skipped) + 9 FE degraded tests all green. Live Bedrock re-confirmation of SC1/SC2 (real ValidationException→pipeline_failed; real Stop→pipeline_cancelled on the wire) recorded as a DEFERRED next-live-pass item (defer-live-verification convention; `default` profile / acct 473293451041 available). Remaining backlog before ship: register clusters C (ISS-003/010/011 test-infra), D (ISS-004/005/006 prompt+deliverable adherence — needs the deferred live pass), E (ISS-014/015/019/021 custom-workflow UX), plus ISS-012 traceability-table drift + the deferred live re-checks — then `/gsd-complete-milestone`. ——— Prior: Phase 15 COMPLETE (Live-Pass Prompt Contract Closure; 84/84 plans — LV-02 deck contract closed live, sdlc/infra prompt residuals shipped; full history in git).*
