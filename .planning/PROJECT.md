...
---
*Last updated: 2026-06-11 — Phase 11 COMPLETE (verified 13/13): engine-owned fan-out + merge live end-to-end — kernel `run_fanout` single spawn path (both entry points), engine-decided `sub_sandbox`/`worktree` isolation, `MergeStrategy` port + 4 impls + `on_conflict` policies, enforcing `BudgetManager` (reserve-before-spawn, trust-conditional `Limits`, per-workspace ceilings, `BudgetSnapshot` on the 0014 column), additive 0019 `subagent_runs`, cancellation propagation + zero-residue teardown, SC-001 sample fan-out proof (registry 53→59). Code review: 6 Critical + 8 Warning dead-wiring findings found and ALL 14 fixed (live-path bindings for allowed_workers/isolated writes/on_conflict/wall-clock+tokens/depth/has_git+commit_all, grant enforcement at fulfilment, merged-document writes). Regression sweep: 505 prior-phase tests green after 2 stale-guard updates; 1 pre-existing phase-8/10 vocabulary debt recorded (`gate_blocked`).*
