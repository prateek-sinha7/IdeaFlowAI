...
---
*Last updated: 2026-06-11 — Phase 12 COMPLETE (final phase, verified 6/6 after gap closure): deterministic wave scheduler + durable resume live — `wave_scheduler` strategy with pure `build_waves` topo seam (CP-SAT seam left, prototype stays sequential), `json_tasks` parser, additive `0020 wave_runs` (registry 59→61), single engine-side `Step.retry` wrapper + content-hash artifact reuse, WS `after_seq` durable replay with server-side workspace recovery, step-granular three-way `restore_non_terminal_runs` with whole-wave re-run on mid-wave resume, FE wave/subagent tree panel with event_id dedup + per-run reset. Initial verification found 4 blockers (CR-01..CR-04/06); gap plans 12-05/06/07 closed all (RED→GREEN TDD). Re-review: 0 critical / 11 warning (WR-09..14 + deferred WR-02/04/06..08 tracked as quality debt). Regression: 268 backend + 6 FE vitest green, 10 characterization snapshots byte-identical, lint-imports 4/0. Live FE render/reconnect UAT deferred to milestone-end live pass (12-UAT.md). Security pass (/gsd-secure-phase 12) still owed.*
