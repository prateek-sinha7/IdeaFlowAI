...
key-decisions:
  - "The chrome WRAPS the reused renderer as children (ND-G) — renderDeliverable() is unchanged and the five renderers (UserStoryPreview/PPTPreview/PrototypePreview/AppBuilderPreview/MarkdownPreview) are NOT edited (they are not in files_modified; the Task-1 diff grep for those filenames is 0). A visible consequence: some renderers carry their OWN toolbar/device chrome, so the settled prototype shows a double-frame (our browser chrome + the renderer's own preview toolbar). That is the honest cost of ND-G reuse and is NOT a fidelity gap to 'fix' by editing a renderer — flagged for the human checkpoint."
...
3. **Complete the Preview plan — SUMMARY + STATE + ROADMAP** — this docs commit (docs).
...
- `frontend/src/components/preview/PreviewPanel.tsx` — added the `PreviewChrome` import; derived `previewFilename` + a client-only `handlePreviewOpen`; rewrote the Preview-tab body to a failure → streaming-chrome → empty → settled-chrome ladder; removed the tab-bar `RendererSwitcher` usage and DELETED the superseded `RendererSwitcher` `<select>` function (INV-3, no shadow).
...
Inherits **ND-A..ND-I** (39-01 ND-A..ND-G/I + 39-05 ND-H). This surface leans on three of them; no new ND-letter is added (each observation is an existing register entry):

- **ND-G — the renderers are REUSED, not rebuilt.** The chrome is a passive frame; the five renderers are untouched (Task-1 diff grep = 0). **Reviewer note (a consequence, not a gap):** some renderers carry their own toolbar/device chrome, so the settled prototype shows a *double-frame* — our browser chrome around the reused `PrototypePreview`'s own preview toolbar. The mock shows a single frame around raw website content because it hardcodes the site. Editing a renderer to remove its toolbar is out of scope (ND-G); this is registered here for the human sign-off.
...
- `npx vitest run src/components/preview/PreviewPanel.test.tsx`: **Test Files 1 passed (1) · Tests 8 passed (8)** — the 4 pre-existing header/tab tests + the 4 new chrome/switch/streaming/failed tests.
