...
requires:
  - phase: 39-run-screen-mock-fidelity-b5 (plan 01)
    provides: the intended-divergence register ND-A..ND-H + ND-U this surface inherits (esp. ND-D live data, ND-F no image placeholder, ND-G reuse the renderers)
...
- `npx vitest run src/components/preview/PreviewPanel.test.tsx`: **Test Files 1 passed (1) · Tests 10 passed (10)** — the 4 pre-existing header/tab tests + the 6 chrome/switch/streaming/failed + ND-V tests (the plain-deliverable chrome test now targets user_stories; two new ND-V tests assert prototype/app_builder render WITHOUT `preview-chrome` — keeping `renders-as-switch`).
