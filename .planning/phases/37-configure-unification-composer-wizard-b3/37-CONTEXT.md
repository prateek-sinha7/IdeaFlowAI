...
- **[AMENDED 2026-07-09 — plan-check drift finding, verified against current code].** Per-agent inline model selection is ALREADY IMPLEMENTED as the live-catalog **Model lever** (EMP-01/DECIDE-02, `AgentsPopup.tsx:1563-1604`; offers the whole catalog, threads `model_overrides` via `updateLever`→`onModelOverridesChange`), so the "mount `AgentModelPicker` inline" intent is **SATISFIED**. The standalone `AgentModelPicker.tsx` is confirmed **dead code** (imported only by its own test); RE-MOUNTING it would create an **INV-3 dual-implementation** and is therefore **REJECTED**. Phase-37 "reuse of AgentModelPicker" = reuse its fetch-shell pattern (already done in `CapabilityPaletteSection`, `:1008`) + the relocated inline Model lever — NOT re-mounting the superseded component. (This reconciles the stale literal instruction, written on a 2-day-old snapshot, with verified code reality; it honors both the decision's intent and INV-3. Surfaced for override.)
