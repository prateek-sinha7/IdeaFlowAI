...
type: bug
kind: event
title: BUG-019 — A fresh top-level launch shows the PREVIOUS run's name in the left-lane
  header title
status: done
severity: major
applies_to:
...
verification:
  ...
  status: passed
  test_files: []
compact_summary: 'Fresh-launch header fell back to `recentRuns[0]` (the previous run) because nothing inserts the new run into `recentRuns` at launch; fixed by resolving to undefined there.'
...
**Depends on:** [BUG-019-020-GROUNDED-CONTEXT](20260717-2007-BUG-019-020-GROUNDED-CONTEXT.md)

<!-- /RELATED -->

# BUG-019 — A fresh top-level launch shows the PREVIOUS run's name in the left-lane header title
