---
id: FIX-BUGFIX-SPEC-REVISION-CONTEXT
type: fix
kind: event
title: Bugfix brief — `update_specs` regenerates the spec; resume loses planning context
status: open
applies_to:
  phases: []
  modules:
  ...
  globs:
  - engine.py
  - agents/prompts/prototype-specify/AGENT.md
  - factory.py
  - AGENT.md
  - kernel_services.py
  - clarify_engine.py
  ...
  - test_characterization_od_prototype.py
  - useWorkflow.ts
  - concierge.py
...
verification:
  ...
  - tests/agents/_scripted_model.py
  - tests/agents/test_restart_resume.py
compact_summary: 'Diagnosed: update_specs never injects the prior spec into the revision prompt, and a resumed run rebuilds planning_context from a 291-char stub.'
...
**Depends on:** [FIX-039](20260707-0125-FIX-039.md), [FIX-048](20260707-1905-FIX-048.md)

**Referenced by:** [FIX-BUGFIX-NESTED-REVISION](20260811-2031-FIX-BUGFIX-NESTED-REVISION.md)
...
- `engine.py:3254-3290` — REQ-RESUME-17 gate re-entry (restart-parked gate)
...
- `agents/artifact_store/` no longer stores artifacts (persistence deleted in REQ-PERSIST-02); its `__init__.py` docstring is stale. The real substrate is `artifact_refs` via `ScopedStore` in `agents/authz.py`.
