...
type: fix
kind: event
title: Workflow says "done" while the validation + build agents keep running in a
  loop
status: done
applies_to:
  phases:
...
verification:
  ...
  - frontend/src/hooks/useRunStream.test.ts
  - frontend/src/lib/__tests__/wsRunScope.test.ts
  - src/lib/__tests__/wsRunScope.test.ts
compact_summary: 'A completed run''s agents kept flipping back to "running" because the shared reducer had no run-scoping; tagged every SSE frame with `runId` and dropped foreign frames.'
...
**Depends on:** [FIX-128](20260728-FIX-128.md)
...
**Phase(s):** Phase 29/44 (SSE transport + app-level provider fan-out), Phase 12 (REQ-RESUME-03 dedup + `seq` cursor), Phase 38 (notification feed).
