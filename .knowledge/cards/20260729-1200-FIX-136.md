...
type: fix
kind: event
title: Concurrent run shows "Done" in history while still running — preserve "running"
...
verification:
  ...
  status: passed
  test_files: []
compact_summary: 'A concurrent run''s completion refetched history and flipped a still-running run''s status to "Done" due to a DB-write/SSE-delivery race; preserved "running" until its own event fires.'
