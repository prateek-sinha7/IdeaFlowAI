...
type: bug
kind: event
title: BUG-021 — Left chat transcript BODY shows the PREVIOUS run's messages on a
  fresh launch (until the new run's frames arrive)
status: done
severity: minor
applies_to:
  phases: []
...
verification:
  ...
  - freshLaunchTranscriptReset.source.test.ts
compact_summary: '`useRunChat.messages` was never reset on a fresh top-level launch, so the previous run''s transcript lingered until new frames streamed in; fixed with a `seedRunChatTranscript([])` reset.'
last_updated: '2026-08-14'
author: 'Prateek Sinha <2000152842@hexaware.com>'
author_source: applies-to-glob
---

# BUG-021 — Left chat transcript BODY shows the PREVIOUS run's messages on a fresh launch (until the new run's frames arrive)
