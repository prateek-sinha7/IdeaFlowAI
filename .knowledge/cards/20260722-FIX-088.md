...
type: fix
kind: event
title: 'Issue 2: version chip stays at v1 after revision completes — contentSourceRunId
  not updated for revision completions'
status: done
applies_to:
  phases:
  - '25'
...
verification:
  ...
  status: passed
  test_files: []
compact_summary: 'Version chip stuck at v1 after a revision because `isForeignCompletion` blocked `contentSourceRunId` updates for the revision''s own completion event.'
