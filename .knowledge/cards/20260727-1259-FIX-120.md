...
type: fix
kind: event
title: 4 resume UI bugs — 409 race on fast-click, agent statuses reset to idle, task
  1/2 data lost, errors navigate home silently
status: done
applies_to:
  phases:
  - '12'
...
verification:
  ...
  status: passed
  test_files: []
compact_summary: 'Four resume bugs: a DB-write/SSE-drain race caused 409s on fast reclick, agent statuses reset to idle with no offset signal, task data was overwritten, and errors were silent.'
...
**Phase / trigger:** Phase 50/44/31 (REQ-RESUME-18/KAN-120)
...
#### Phase Context
- **Phase(s) involved:** Phase 50 (REQ-RESUME-18 / KAN-120 — user-resume endpoint), Phase 44 (SSE transport / pipeline_start), Phase 31 (RunChatLane terminal state)
