...
type: fix
kind: event
title: Fix version dropdown showing wrong relative age ("5h ago") due to timezone-naive
  datetime serialization — SQLAlchemy DateTime (no timezone=True) returns naiv...
status: done
applies_to:
  phases:
  - '05'
...
verification:
  ...
  status: passed
  test_files: []
compact_summary: 'SQLAlchemy DateTime columns return tz-naive datetimes that Pydantic serialized without a UTC suffix, so JS Date.parse read them as local time; added @field_serializer to force tzinfo=UTC.'
