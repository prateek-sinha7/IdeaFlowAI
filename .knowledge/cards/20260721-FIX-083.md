...
type: fix
kind: event
title: Replace first clarification card (amber styled) with plain text bubble "Before
  I build, I need to lock a few things down."
status: done
applies_to:
  phases:
  - '31'
...
verification:
  ...
  test_files:
  - backend/tests/unit/test_chat_narrator.py
compact_summary: 'Two paths both rendered a clarify box; kept the `AwaitingCard` and reduced the narrator''s card to a plain text bubble, not a styled amber card.'
...
**Phase / trigger:** Phase 31 (REQ-CHATUI-01) + Phase 43 (A6)
...
- **Phase(s) involved:** Phase 31 (REQ-CHATUI-01 — ResultCard narrator architecture), Phase 43 (A6 — `chat_narrator.py` narrator projection)
