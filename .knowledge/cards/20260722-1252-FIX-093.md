...
type: fix
kind: event
title: Remove duplicate outer timeline dot from StartingPointCard — aligns with ClarificationsCard
  flat-card style
status: done
applies_to:
  phases:
...
verification:
  ...
  status: passed
  test_files: []
compact_summary: 'StartingPointCard rendered both an outer timeline dot and its own header icon, showing two duplicate FileText icons; removed the redundant outer dot.'
