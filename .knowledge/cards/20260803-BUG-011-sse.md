...
type: bug
kind: event
title: 'BUG-011 — pipeline_complete → setContentSourceRunId is not run-scoped: a background
  run completing hijacks the VIEWED content-source'
status: done
severity: minor
applies_to:
  phases: []
...
compact_summary: '`pipeline_complete` repointed the viewed content-source for any attached run''s completion; guarded with an `isForeignCompletion` check against `trackedRunIdRef`.'
last_updated: '2026-08-14'
author: 'Bilal Arshad <bilala@hexaware.com>'
author_source: 'body-path'
---

# BUG-011 — pipeline_complete → setContentSourceRunId is not run-scoped: a background run completing hijacks the VIEWED content-source
