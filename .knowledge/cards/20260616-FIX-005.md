...
type: fix
kind: event
title: Prototype (and PPT) wizard pre-fills stale brief from previous run — draft
  never cleared
status: done
applies_to:
  phases:
...
verification:
  ...
  status: passed
  test_files: []
compact_summary: 'dashboard/page.tsx consumed but never cleared prototype.draft/ppt.draft, so the next wizard open restored the previous run''s full brief or context block.'
...
**Referenced by:** [FIX-005b](20260622-FIX-005b.md)
