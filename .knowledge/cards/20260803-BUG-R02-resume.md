...
type: bug
kind: event
title: BUG-R02 — review-gate approve/all-actions black-hole after a backend restart
  (same empty-roster re-arm no-op)
status: done
severity: major
applies_to:
  phases:
...
verification:
  ...
  status: passed
  test_files: []
compact_summary: 'Same alias-resolution gap as BUG-R01 hit the review-gate resume path, black-holing gate approve/all-actions after restart; fixed in the same one-line change.'
...
# BUG-R02 — review-gate approve/all-actions black-hole after a backend restart (same empty-roster re-arm no-op)
...
- **Expected:** after restart, all five gate actions (approve/reject/edit/redo/update_specs) over `POST /{id}/gate` work identically to a never-restarted run and drive the run downstream (Phase 49 SC-3; REQ-RESUME-17).
