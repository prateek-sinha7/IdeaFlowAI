...
type: fix
kind: event
title: 'Prototype pipeline fails with ModuleNotFoundError: No module named ''resource''
  — local.py does import resource at module level; resource is Unix-only, unavail...'
status: done
applies_to:
  phases:
  - 09
...
verification:
  ...
  status: passed
  test_files: []
compact_summary: 'local.py imported the Unix-only resource module at module level, crashing every sandbox read/write on Windows; guarded with try/except and a None check in _limits().'
...
**Referenced by:** [FIX-004b](20260622-FIX-004b.md), [FIX-007b](20260623-FIX-007b.md)
