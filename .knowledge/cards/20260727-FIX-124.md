...
type: fix
kind: event
title: Security gate failure UX — "What went wrong" security bullets, "Blocked by
  the security gate" inline Steps card, and Audit tab shows hook_runs (secret scan
  /...
status: done
applies_to:
  phases:
...
verification:
  ...
  status: passed
  test_files: []
compact_summary: 'Security-gate failures had no explanation UI: Audit tab never fetched hook_runs, failure bullets showed only generic names, and Steps had no inline block explanation.'
...
**Depends on:** [FIX-118](20260724-FIX-118.md), [ISS-016](20260613-1437-ISS-016.md)

<!-- /RELATED -->

# FIX-124

**Phase / trigger:** Phase 8 (REQ-HOOK-01/04 hook_runs), Phase 13 (F3 pipeline_failed/security gate), Phase 16 (ISS-016 terminal failure card)
...
#### Phase Context
- **Phase(s) involved:** Phase 8 (REQ-HOOK-01/04 — hook_runs), Phase 13 (F3 pipeline_failed), Phase 16 (ISS-016 terminal failure card)
...
#### Phase Context
- **Phase(s) involved:** Phase 31 (REQ-CHATUI-01 — `useRunChat` transcript), Phase 29 (chat backbone / mechanical router / `CHANNEL_REVISION`), Phase 43 (A.1 CRUX — Concierge seam)
