...
type: fix
kind: event
title: Clear stale questionnaireData on pipeline_cancelled/failed so AwaitingCard
  disappears — pipeline_cancelled handler cleared reviewGateData but not questionnai...
status: done
applies_to:
  phases:
  - '22'
...
verification:
  ...
  status: passed
  test_files: []
compact_summary: '`pipeline_cancelled`/`pipeline_failed` cleared `reviewGateData` but not `questionnaireData`, leaving `runLaneState` stuck on "clarify" instead of "terminal".'
...
- **Relevant register section:** Phase 42 §2 (REQ-RUNUI-08 — inline clarify/gate affordances)
