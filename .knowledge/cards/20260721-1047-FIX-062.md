...
type: fix
kind: event
title: Move "v1 draft" version chip to the left of the "Running · Streaming" status
  badge in RunHeader — Chip was placed after the flex-1 spacer in JSX order, landi...
status: done
applies_to:
  phases:
  - '39'
...
verification:
  ...
  test_files:
  - RunHeader.test.tsx
compact_summary: 'The version chip sat after the flex-1 spacer in RunHeader''s JSX, pushing it to the right; moved it before the status badge so it renders leftmost.'
...
**Phase / trigger:** Phase 39 (REQ-RUNUI-06/07)
...
#### Phase Context
- **Phase(s) involved:** Phase 39 (REQ-RUNUI-06/07) — `RunHeader` component built to match `Hexaware Run - Live.dc.html:159-166`
