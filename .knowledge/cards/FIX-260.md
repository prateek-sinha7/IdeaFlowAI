...
source: .planning/FIX-REGISTER.md#fix-260
ticket: KAN-167
...
> **Merge note (2026-08-17, dev -> feature/cognito-login).** This card was authored as `FIX-185`, but the merge base and the dev side already used `FIX-185` for the CR-02 revision-guard fix (KAN-156) and already used `FIX-165` — this fix's register id — for the analyze-gate double-click fix. The register row and this card were therefore re-homed to `FIX-260`; the fix itself is unchanged.
