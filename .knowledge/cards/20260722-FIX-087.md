...
type: fix
kind: event
title: Fix version switch showing wrong content, chained runs in version family, and
  context markers in titles — handleSelectVersion only updated WorkflowHistory lo...
status: done
applies_to:
  phases:
  - '25'
...
verification:
  ...
  status: passed
  test_files: []
compact_summary: 'Three bugs: version switch skipped page.tsx''s content-clearing router, chained runs got a `parent_run_id` grouping them as versions, and titles kept raw `===` markers.'
