...
type: fix
kind: event
title: Jump Back In shows raw od_ppt/od_prototype names and no Revised/Chained indicators
  — WORKFLOW_LABELS map missing 4 od_* entries; source_run_id not in Workflo...
status: done
applies_to:
  phases:
  - '05'
...
verification:
  ...
  status: passed
  test_files: []
compact_summary: '"Jump Back In" showed raw `od_ppt`/`od_prototype` type strings (missing label map entries) and no "(Chained)" indicator since `source_run_id` was never exposed.'
...
**Phase / trigger:** Phase 36 (REQ-SHELL-02 Jump Back In)
...
- **Phase(s) involved:** Phase 36 (REQ-SHELL-02 Jump Back In / HomeLaunchGrid), Phase 5 (WorkflowRunResponse additive fields), KAN-130 analysis
