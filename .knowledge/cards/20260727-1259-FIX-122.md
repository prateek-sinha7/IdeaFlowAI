...
type: fix
kind: event
title: Yellow "Reconnecting" banner and an apparently dead Stop button on every cancel,
  because sawNonLiveAttachRef was only set on stream_attached, so the stream c...
status: done
applies_to:
  phases:
  - '44'
...
verification:
  ...
  status: passed
  test_files: []
compact_summary: 'FIX-121''s fix raced React''s async state update against the synchronous SSE reader; set the non-reconnect ref synchronously inside the frame dispatcher instead.'
...
**Depends on:** [FIX-121](20260727-1259-FIX-121.md)
