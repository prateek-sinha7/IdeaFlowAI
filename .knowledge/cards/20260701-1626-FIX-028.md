...
type: fix
kind: event
title: Prototype nav-validation hardening (render_check + static_check), 5 rounds
...
verification:
  ...
  status: passed
  test_files:
  - test_nav_coverage.py
  - test_phase5_fixloop_selection.py
  - test_route_table.py
compact_summary: 'render_check and static_check fail-opened on dynamic/hash-router navigation and false-positived on alias/parametric routes; hardened over 5 rounds with a shared route_table resolver.'
...
**Phase(s) involved:** Phase 07/08 (prototype validators REQ-VALID-01/03/04) · IMPLEMENTATION-REGISTER **Phase 24** (post-milestone).
