...
type: bug
kind: event
title: BUG-010 — Run-scope ref mistracks when a background build is active while viewing
  a different run (j1u WARNING-2)
status: open
applies_to:
  phases: []
  modules:
...
verification:
  ...
  status: required
  test_files: []
compact_summary: 'Investigated as a false alarm: `activePipelineRunId` is null while a run is actively building, so `trackedRunIdRef` resolves correctly — closed won''t-fix.'
last_updated: '2026-08-14'
author: 'Bilal Arshad <bilala@hexaware.com>'
author_source: 'body-path'
---

# BUG-010 — Run-scope ref mistracks when a background build is active while viewing a different run (j1u WARNING-2)
