...
type: fix
kind: event
title: Delete pipeline from history does nothing — FK constraint on 9 child tables
  + silent frontend error
status: done
applies_to:
  phases:
  - '04'
...
verification:
  ...
  status: passed
  test_files: []
compact_summary: 'delete_run did a bare ORM delete with no cascade across 9 FK child tables under SQLite PRAGMA foreign_keys=ON, causing a silent 500; fixed with explicit child deletes plus a surfaced FE error.'
...
**Referenced by:** [FIX-004b](20260622-FIX-004b.md)
