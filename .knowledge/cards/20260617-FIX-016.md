...
type: fix
kind: event
title: '"Invalid presentation output" — PPT composer runs 12.9s → 662 chars (no HTML
  deck)'
status: done
applies_to:
  phases:
...
verification:
  ...
  status: passed
  test_files: []
compact_summary: 'The engine''s clarify routing only handled mode==''auto'', never mode==''skip'', so od_ppt''s skip-mode manifest still hit ClarifyEngine and agents ran without real context; added the missing skip branch.'
...
**Phase / trigger:** Phase 4 (REQ-MAN-04)
...
The design intent of `mode: skip` is: "the wizard already collected all context (brief + template + design system) — don't ask clarifying questions, skip straight to PROCEED". The engine implemented the `mode: auto` side of REQ-MAN-04 but never implemented the `mode: skip` side.

#### Phase Context
- **Phase(s) involved:** Phase 4 — Manifest + Compiler [1A] (REQ-MAN-04: clarify routing concern sourced from compiled plan)
- **Relevant register section:** `_register-parts/04-manifest-compiler-1a.md` §3 (REQ-MAN-04), engine.py comment block at line ~1433
...
- **Locked decisions respected:** REQ-MAN-04 says clarify.mode is sourced from the compiled plan — the fix stays entirely within the clarify-mode routing seam. INV-1 is honored: the gate is on `compiled.clarify.mode` (a generic routing value), never on `pipeline_type` string.

#### Fix Applied
| File | Change | Why |
|------|--------|-----|
| `backend/agents/execution_engine/engine.py` | Added `clarify_skip = clarify_mode == "skip"` and a new `if clarify_skip and gate_verdict == "CLARIFY_REQUIRED":` branch that overrides verdict to PROCEED; restructured existing `clarify_auto` block as `elif` | Implements the missing `mode="skip"` side of REQ-MAN-04: when the manifest says `skip`, force gate_verdict=PROCEED regardless of what the planner returned, so ClarifyEngine is never invoked |
...
- **SC-001** (zero engine edits for new workflows): Engine edit is justified and authorized — this IS the REQ-MAN-04 clarify-mode routing seam. The engine comment at line 1437 explicitly says this concern belongs here.
