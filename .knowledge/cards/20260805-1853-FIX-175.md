...
type: fix
kind: event
title: 2x "Run started" and 2x "Delivered" on single od_ppt run — Event_id mismatch
  between live SSE delivery and DB-fetched delivery. The engine yielded live chat_...
status: done
applies_to:
  phases:
  - '43'
...
verification:
  ...
  - backend/tests/agents/test_restart_resume.py
  - backend/tests/unit/test_chat_narrator.py
compact_summary: 'Live SSE chat_reply used a different event_id than the DB row for ''Run started'' cards, so dedup missed it; made both share the same reply_eid.'
...
**Referenced by:** [FIX-176](20260804-1156-FIX-176.md), [ISS-078](20260812-0236-ISS-078.md)
