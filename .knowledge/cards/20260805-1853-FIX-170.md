...
type: fix
kind: event
title: Revision UI frozen after launch — pipeline_start/agent events discarded as
  foreign; revision message not shown; no "Revision started" card
status: done
applies_to:
  phases:
  - '29'
...
verification:
  ...
  status: passed
  test_files: []
compact_summary: 'postRevision launches never updated trackedRunIdRef/activelyBuildingRunIdRef, so isForeignRunFrame dropped all of the new revision run''s SSE events, freezing the UI.'
...
**Depends on:** [FIX-167](20260804-1156-FIX-167.md), [FIX-168](20260805-1853-FIX-168.md)
