...
type: fix
kind: event
title: '"Revision started" appearing at top of chat; "Delivered" shown twice on history-reopen
  — Family chat seed used sort by data.seq to merge frames across runs —...'
status: done
applies_to:
  phases:
  - '31'
...
verification:
  ...
  status: passed
  test_files: []
compact_summary: 'Family chat seed sorted by per-run-scoped `seq` across different runs'' own counters, putting ''Revision started'' at the top; fixed to merge frames by family.members chronological order instead.'
...
**Phase / trigger:** Phase 31 (REQ-CHATUI-01/02/03), FIX-167 (KAN-154 gap 1 family seed)
...
#### Phase Context
- **Phase(s) involved:** Phase 31 (REQ-CHATUI-01/02/03), FIX-167 (KAN-154 gap 1 family seed)
