...
type: bug
kind: event
title: BUG-001 — Run-screen left-lane title shows the most-recent run's title, not
  the viewed run's
status: done
severity: major
applies_to:
...
verification:
  ...
  status: passed
  test_files: []
compact_summary: 'DashboardLayout read recentRuns[0].title for the lane header unconditionally instead of the viewed run; fixed by looking up the run via contentSourceRunId.'
last_updated: '2026-08-14'
author: 'Bilal Arshad <bilala@hexaware.com>'
author_source: 'body-path'
---

# BUG-001 — Run-screen left-lane title shows the most-recent run's title, not the viewed run's
