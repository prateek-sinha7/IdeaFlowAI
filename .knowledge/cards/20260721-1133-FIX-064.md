...
type: fix
kind: event
title: Hide Web/Deck toggle in template picker — show only relevant gallery per launch
  context
status: done
applies_to:
  phases:
...
verification:
  ...
  - frontend/src/components/workflow/WizardStepper.test.tsx
compact_summary: 'WizardStepper always rendered the Web/Deck toggle; added a showToggle prop defaulting false so the gallery is locked to the launch mode.'
