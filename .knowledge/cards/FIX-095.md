...
summary: >-
  Fix .split is not a function crash + multi-select chip highlighting when "Use
  recommended" clicked — recommendedAnswer/recommendedDisplay can be non-string
  (array/number) from the API — all .split() calls lacked String() coercion
source: .planning/FIX-REGISTER.md#fix-095
