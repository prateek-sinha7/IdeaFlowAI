...
type: bug
kind: event
title: BUG-015 — Completed run's SSE stream reconnects FOREVER ("Reconnecting…" banner
  flaps constantly after a run finishes)
status: done
severity: major
applies_to:
  phases:
...
verification:
  ...
  status: passed
  test_files: []
compact_summary: 'FE reconnected on every stream close regardless of terminality, looping ~14k-event re-replays after completion; fixed by skipping reconnect after a non-live stream_attached.'
last_updated: '2026-08-14'
author: 'Imran Yousaf <imrany@hexaware.com>'
author_source: applies-to-glob
---

# BUG-015 — Completed run's SSE stream reconnects FOREVER ("Reconnecting…" banner flaps constantly after a run finishes)
