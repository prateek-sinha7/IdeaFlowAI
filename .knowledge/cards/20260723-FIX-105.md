...
type: fix
kind: event
title: '"Run Again" resumes cancelled pipeline from stopped step — 4-part gap: backend
  eligibility != "failed" blocked cancelled runs; in-memory state machine termin...'
status: done
applies_to:
  phases:
  - '31'
...
verification:
  ...
  status: passed
  test_files: []
compact_summary: '"Run Again" on a cancelled run failed on 4 gaps: backend eligibility rejected "cancelled" status, the in-memory state machine locked it terminal, and no FE resume path existed.'
...
**Referenced by:** [FIX-152](20260730-1825-FIX-152.md), [FIX-381](20260829-0220-FIX-381.md), [ISS-220](20260828-1712-ISS-220.md), [ISS-316](20260828-1952-ISS-316.md)
...
#### Phase Context
- **Phase(s) involved:** Phase 50 (REQ-RESUME-18 endpoint), Phase 44 (LOCK-B REST+SSE), Phase 31/32 (RunChatLane terminal cards)
