...
- [backend/agents/capabilities/tools/playwright.py](../../backend/agents/capabilities/tools/playwright.py)
- [backend/agents/capabilities/tools/pptx.py](../../backend/agents/capabilities/tools/pptx.py)
...
- `capabilities/` — the self-registering registry (`registry.py`). A module-level
  ...
  `task_loop`, `fanout_batch`, `wave_scheduler`), `gates/` (`human` / `before-human` — the
  same class registered twice, POST- and PRE-step; `conditional`, `validation`, `approval`,
...
- `execution_engine/` — the kernel proper. [engine.py](../../backend/agents/execution_engine/engine.py) (`ExecutionEngine`) is the single entry
  ...
  gates, and emits the WebSocket event envelope. Order is topological but not strictly
  forward: a `conditional` gate returns a `route` outcome that MOVES the dispatch cursor
  (spec 014) — backwards to loop, sideways to branch, or out to a separate child run —
  so the same step id can run more than once (`step_visit_counts`). [context.py](../../backend/agents/execution_engine/context.py)
  ...
  [kernel_services.run_trigger_workflow](../../backend/agents/execution_engine/kernel_services.py) additionally mints and spawns the INDEPENDENT
  child `WorkflowRun` a `trigger: workflow` route outcome dispatches to (linked by
  `parent_run_id`, depth-guarded by `trigger_depth`).
