...
- MOD-backend-agents
watched_files: 18
code_signature: 1ef896ba88ed
symbols_signature: 0fd3d22086de
prose_signature: 1ef896ba88ed
prose_symbols_signature: 0fd3d22086de
last_synced: '2026-08-18'
---

<!-- AUTO-GENERATED BELOW THIS LINE — regenerated by tools/knowledge/build_architecture.py -->

### Members (8 files across 1 modules)
...
### Imported by, from outside this domain (102)
...
`BudgetManager` in DOMAIN-execution-kernel. `intersect_permissions` computes the
effective `ToolPermissions` here, but the runtime allow/deny is
DOMAIN-workspace-isolation. What `(kind, name)` even resolves to — the
...
- **The typed contract.** [plan.py](../../backend/agents/workflows/plan.py) holds `CompiledWorkflow`, `Step`, `Task`
  and the nested policy types, plus `intersect_permissions` / `_and_mask` —
  the only primitive that resolves permissions, and one that can never raise
  one.
...
sequenceDiagram
    ...
    loop per raw step dict
        ...
        MOD-backend-agents->>MOD-backend-agents: plan.py::intersect_permissions
    end
...
2. [run_commands.py::launch_run](../../backend/app/api/run_commands.py) rejects an empty brief, then
...
10. Still per step: `_compile_tool_grant` parses the `tools:` block,
    ...
    [plan.py::intersect_permissions](../../backend/agents/workflows/plan.py) resolves the effective `ToolPermissions`
    against the trust-conditional workflow ceiling.
...
anyway, because a persisted row can be tampered with after it was saved.
