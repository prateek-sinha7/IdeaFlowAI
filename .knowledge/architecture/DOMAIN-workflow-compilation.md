...
- MOD-backend-agents
watched_files: 20
code_signature: 497fbfe7cea1
symbols_signature: 593c618f617a
prose_signature: 9c8d01aa6058
prose_symbols_signature: 730e7ada1eed
...
### Imported by, from outside this domain (17)
...
### Imports, from outside this domain (25)
...
- **The user-authored entrance.** [selections.py::synthesize_manifest](../../backend/agents/workflows/selections.py) turns a
  ...
  drift.
- **Workflow overrides.** [execution_engine/overrides.py::merge_override_steps](../../backend/agents/execution_engine/overrides.py)
  merges a saved user's override steps onto a file-compiled plan. A user may bind
  their own step list to a built-in workflow; the override row lives in the
  `workflows` table with a non-NULL `overrides_pipeline_type`, and
  `merge_override_steps` returns a NEW `CompiledWorkflow` (never mutating the cached
  original) via `dataclasses.replace`. Critically, only steps are merged; every
  workflow-level field (deliverable, context_providers, planner, clarify, seed_files, limits)
  still comes from the file manifest on every resolve, which sidesteps the trust
  wall without making any privileged capability user-grantable (spec 016).
...
3. Agent ids are allow-listed via `registry.py::allowed_custom_agent_ids`, each
   ...
   For `base_pipeline_type == "custom"` specifically, `launch_run` now rejects an
   absent/empty `agent_ids` PRE-MINT with `no_agents_selected` ([FIX-343](../cards/20260828-2039-FIX-343.md)/ISS-234) —
   `custom`'s roster IS the caller's composition, so a falsy `agent_ids` means
   "nothing selected", never "fall back to `custom/workflow.yaml`'s own 9-agent
   default membership" the way every other `base_pipeline_type` does two branches
   down. Before this guard, a 0-agent composer state (a dead-linked canvas, a
   swallowed `getWorkflowDetail` failure, a brand-new unsaved composition) silently
   substituted that unrelated 9-agent pool and spent real Bedrock tokens on it.
...
6. [engine.py::ExecutionEngine._execute_impl](../../backend/agents/execution_engine/engine.py) calls
   [engine.py::compile_for_run](../../backend/agents/execution_engine/engine.py), which is `@lru_cache(maxsize=None)` and shared
   process-wide across every user and run. The returned `CompiledWorkflow` must never
   be mutated — [execution_engine/overrides.py::merge_override_steps](../../backend/agents/execution_engine/overrides.py) uses
   `dataclasses.replace` to return a NEW object whenever merging a user's override.
...
**Workflow overrides.** At launch time, [run_commands.py](../../backend/app/api/run_commands.py) (in the `FILE_PIPELINE`
case) resolves the caller's override row for the target `base_pipeline_type` and
calls [execution_engine/overrides.py::merge_override_steps](../../backend/agents/execution_engine/overrides.py) to merge the override's steps
onto the compiled file plan. An override is rejected silently — degrading to the
file plan with a log warning — if the row is malformed, if a step references a
capability not registered `user_allowed=True`, or if compilation fails; a tampered
row must not take a run down. The merge produces a NEW `CompiledWorkflow` via
`dataclasses.replace` (never mutating the cached original); the run launches with
the merged steps but every workflow-level field still from the file manifest.
...
does not, so a genuinely broken manifest takes the whole catalog down. Both endpoints
check for the caller's own workflow override (spec 016) and serve the override's steps
when one is enabled, reporting `is_overridden` / `has_override` / `override_id`. The
`?original=true` query param forces the file projection while still reporting `has_override`
so the UI knows an override exists.
