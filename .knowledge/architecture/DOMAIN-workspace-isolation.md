...
watched_files: 17
code_signature: c853734e3f36
symbols_signature: 173e4b5f6a51
...
- **The disk primitive** is [sandbox.py::RunSandbox](../../backend/app/agents/sandbox.py): segment sanitisation
  ...
  lazily built `LocalWorkspace` so the disk IO has one implementation. [sandbox.py::RunSandbox.list_files](../../backend/app/agents/sandbox.py)
  enumerates regular files with metadata (path, size, mtime) for the Workspace tab,
  excluding the reserved subtrees. [sandbox.py::agent_output_relpath](../../backend/app/agents/sandbox.py) and
  [sandbox.py::write_agent_output](../../backend/app/agents/sandbox.py) persist each step's streamed agent output into
  the run sandbox under the `.agents/` prefix, workspace-visible but never delivered.
- **Reserved sandboxes subtrees** — four prefixes that carry different meanings: `.uploads/` (the
  owner's own staging for file ingestion), `.logs/` (the engine's structured run trace),
  `.verify/` (a gate tool's evidence and reports), and `.agents/` (each step's streamed
  agent output). All are excluded from the deliverable walk; only `.uploads/` and `.logs/`
  are omitted from [sandbox.py::RunSandbox.list_files](../../backend/app/agents/sandbox.py) (the Workspace tab).
  `.agents/` and `.verify/` surface there so a run's work and its approval are both visible.
  [sandbox.py::is_deliverable_relpath](../../backend/app/agents/sandbox.py) centralizes the deliverable-or-not decision so the
  workspace API applies the same rule as the walk.
...
11. [local.py::LocalWorkspace.write_file](../../backend/app/agents/runtime/local.py) resolves the relpath through
    ...
    Separately, [engine.py::execute](../../backend/agents/execution_engine/engine.py) calls
    [sandbox.py::write_agent_output](../../backend/app/agents/sandbox.py) after the agent completes, persisting the
    streamed output under the name [sandbox.py::agent_output_relpath](../../backend/app/agents/sandbox.py) computes —
    `.agents/{index+1:02d}-{agent_id}.md`, ONE-based because a person reads these in the
    Workspace tab and a leading `00-` reads as a numbering bug, with the loop identity
    appended when the same step runs more than once. Best-effort; a full disk or write
    error cannot fail a completed step.
...
13. If the count is non-zero it calls `serialize_sandbox_deliverable`, which walks
    the tree via `_collect_deliverable_relpaths`, filtering each path through
    [sandbox.py::is_deliverable_relpath](../../backend/app/agents/sandbox.py) — excluding the four reserved subtrees
    (`.uploads/`, `.logs/`, `.verify/`, `.agents/`), `PLANNER.md`, and the `exclude` set. Sorts by relative
    path and reads each file with `read_bytes().decode("utf-8")` — never `read_text`, so CRLF survives intact.
...
- *Forgetting to call `is_deliverable_relpath` in new callers.* The workspace API
  lists files and tags them `isDeliverable` based on the same predicate. If a new
  endpoint walks `RunSandbox.root` directly without routing through `is_deliverable_relpath`,
  it can surface reserved data (agent transcripts, gate evidence, logs) as deliverables,
  or the workspace API's tagging drifts from reality. See the unified definition in
  [sandbox.py::is_deliverable_relpath](../../backend/app/agents/sandbox.py).
- *Writing a new bookkeeping file into the sandbox root.* Exclusion is by exact
  basename (any in the `exclude` set, e.g., `PLANNER.md`) and by four reserved prefixes
  (`.uploads/`, `.logs/`, `.verify/`, `.agents/`). Any file outside those becomes a
  deliverable block in the UI — and an entry in the Workspace file list if outside
  `.uploads/` and `.logs/`.
