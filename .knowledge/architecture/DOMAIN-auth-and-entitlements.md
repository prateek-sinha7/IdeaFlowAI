...
watched_files: 24
code_signature: 7d300ca1892e
symbols_signature: cb9c07cb4bae
prose_signature: df8255eebd24
prose_symbols_signature: b8178ba00298
...
### Imported by, from outside this domain (49)
...
### Imports, from outside this domain (20)
...
- **Tier and role** — [entitlements.py](../../backend/app/core/entitlements.py) is a pure table plus `can_run_pipeline`, returning
  `(allowed, reason)` where the reason is already user-facing upgrade copy. It now also carries a
  second, parallel table for model access: `TIER_MODEL_COST_CLASSES` plus `can_use_model(tier, model_id)`,
  fail-closed to `basic`, called from [settings.py::update_preferences](../../backend/app/api/settings.py) before a `preferred_model` write
  is persisted ([FIX-361](../cards/20260829-0029-FIX-361.md)/FIX-382) — closing a bug where a basic-tier account could select and run a
  premium model despite `can_run_pipeline` never being touched. Unlike the pipeline table below, this
  model table has **no frontend mirror yet**: `GET /api/settings/preferences` still returns the full,
  unfiltered model catalog, so the AI Model settings page shows every option regardless of tier —
  the server-side write gate is the only enforcement (tracked open in [ISS-430](../cards/20260829-0029-ISS-430.md)). For Cognito users
...
- **Editing one tier table and not the other.** `TIER_PIPELINES` in
  ...
  nothing and returns an unexplained 403 from `_reject`. `TIER_MODEL_COST_CLASSES`
  (added alongside `can_use_model`) has no frontend counterpart at all yet, so this
  failure mode is currently permanent for model selection, not just possible.
