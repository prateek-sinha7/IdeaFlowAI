...
watched_files: 20
code_signature: 76aa451d0b00
symbols_signature: e194ec9eccd8
...
### Imports, from outside this domain (47)
...
*stop and ask a human* — and if it asks, the run has to survive the wait. Six
registered `GateHandler` capabilities express the policies (`human`,
`before-human`, `approval`, `security`, `validation`, `conditional`), one dataclass in [gates/base.py](../../backend/agents/capabilities/gates/base.py)
...
**The HITL vocabulary is phase-named** ([ADR-0013](../cards/20260824-1630-ADR-0013.md)). A gate's name says WHEN it
runs, because the two timings do opposite things and were previously
indistinguishable in a manifest:

| name | phase | reviews | edit lands on | redo |
|---|---|---|---|---|
| `human` | POST-step (`_POST_STEP_GATES`) | this step's OWN output | this step's artifact | yes |
| `before-human` | PRE-step | the PREVIOUS step's output | that step's artifact, so it becomes THIS step's input | no |
| `approval` / `security` | PRE-step | the step about to run | — | no |

Before the split both were spelled `human`, and which behaviour you got was
decided by whether the step's agent had an `AGENT.md` declaring
`gate: Human_Gate` — invisible from the manifest. `prototype/workflow.yaml`
still shows the residue: it declares `gates: [human]` on three steps that were
DEAD lines, because the inline gate won every time. They are live now, and only
the `inline_gated` dedupe — which had to follow `human` into the post phase —
stops them double-prompting.

**One human-family gate per step.** `gate_key` is
`f"{run_id}:{agent_id}:{visit_count}"` and carries no gate name, so two HITL
pauses on one step would share a durable slot and the second would read the
first's resolution. The compiler rejects the combination by name rather than
letting it mis-resume at runtime.
...
19. `_evaluate_gates` maps a `block` from an `_HITL_GATES` member to `cancel`,
    otherwise yields `(None, outcome, detail)` and short-circuits if a blocking outcome
    is reached. The PRE-step dispatch loop in
    [engine.py::ExecutionEngine._execute_impl](../../backend/agents/execution_engine/engine.py) skips the `None` event when
    forwarding and applies any edit via
    [engine.py::ExecutionEngine._apply_declared_gate_edit](../../backend/agents/execution_engine/engine.py). A `block` outcome
    transitions the state machine to `failed`, persists the budget snapshot with
    force, yields ONE `pipeline_failed` event naming the blocked step, and returns —
    **terminating the run, not merely the step**. `wait_human` instead skips the step
    (`cursor += 1; continue`) and the run carries on.
    **The POST-step loop is different, and only partly by design.** It consumes
    ONLY the `route` outcome; a `block` there is never acted on. For `validation`
    that is deliberate — the deliverable was already produced, so the block is
    surfaced as an additive `gate_blocked` event and nothing more. For
    `conditional` it is a KNOWN GAP: a gate that fails closed with
    `"no matching route outcome and no default_next"`
    ([conditional.py](../../backend/agents/capabilities/gates/conditional.py)) emits
    `gate_blocked` and the run still proceeds to the next step and can reach
    `pipeline_complete`, so a route with nowhere to go is indistinguishable from a
    clean pass at run-status level. The pre-step termination above does NOT cover
    this — `conditional` is a post-step gate (`_POST_STEP_GATES`).
