...
- MOD-backend-agents
watched_files: 23
code_signature: dbebef970d8c
symbols_signature: bff21f249b6a
prose_signature: dbebef970d8c
prose_symbols_signature: bff21f249b6a
last_synced: '2026-08-18'
---

<!-- AUTO-GENERATED BELOW THIS LINE — regenerated by tools/knowledge/build_architecture.py -->

### Members (16 files across 1 modules)
...
### Imported by, from outside this domain (114)
...
### Imports, from outside this domain (56)
...
- **Roster.** [agents/registry.py](../../backend/agents/registry.py) holds `PIPELINE_AGENTS` (ordered agent ids per pipeline),
  ...
  specs and sorts by `order`. This is *membership*, not execution order.
...
- **Audit trace.** `execution_engine/run_log.py::RunLog` writes an append-only JSONL trace to
  ...
  (step_start, agent_error, etc.) keyed by agent_id. Tracing is best-effort and swallows its
  own exceptions so a logging failure never breaks a run.
- **Second ingress.** `execution_engine/ndjson_adapter.py::run_od_prototype_pipeline` runs the
...
8. The compiled step ids are asserted equal to `get_pipeline_agents(compiled.id)` membership;
   drift raises `RuntimeError` at run entry rather than diverging silently.
...
16. Every event the strategy yields passes back up to the `execute()` wrapper, which stamps a
    monotonic `data["seq"]` and a uuid `data["event_id"]`, persists one `run_events` row via
    `_RunEventSink.persist`, and only then yields it to the driver's queue. **This is the only
    place either field is assigned.**
...
**NDJSON.** `execution_engine/ndjson_adapter.py::run_od_prototype_pipeline` runs the identical
...
[test_banned_patterns.py](../../backend/tests/agents/test_banned_patterns.py)'s own comments record that they were deleted (not relaxed) and the
...
are template instances with no `AGENT.md` and can never satisfy it. That card is marked
`active`, but the assertion is still present in [execution_engine/engine.py](../../backend/agents/execution_engine/engine.py) — the decision is
recorded, not yet applied. `ADR-0004` decided that manifest-declared `depends_on` order wins
