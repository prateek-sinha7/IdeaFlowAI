...
watched_files: 26
code_signature: b131ce1ca045
symbols_signature: bcb9a59f77cd
prose_signature: 73162ee97b46
prose_symbols_signature: b53805357599
...
- **The resolver family.** Five capability classes, each satisfying the
  ...
  import-pure.
- **The shared transforms.** [_artifact.py](../../backend/agents/capabilities/deliverables/_artifact.py) holds four passes —
  `strip_code_fence`, `sanitize_carousel_deck_html`, `strip_pre_slide_body_text`, `unwrap_artifact` —
...
- **A step that writes the deliverable but was never granted `tools.write_files`.**
  The runner denies `write_file` at execution, the named file never reaches the
  sandbox, and `single_file` does exactly what it should — falls back to
  `ctx.last_streamed`. The run then ships streamed prose that merely *looks* like a
  deliverable, and nothing in the resolver is at fault. The tell is in the log, not
  the output: `write_file DENIED at execution` followed by `single_file: <name> not
  written by agent — falling back to streamed output`. Two unrelated fixtures hit
  this on the same day ([FIX-314](../cards/20260826-0124-FIX-314.md)), because the
  symptom points at the resolver rather than at the permission that caused it.
