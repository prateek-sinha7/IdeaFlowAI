...
watched_files: 29
code_signature: 8854d1d47732
symbols_signature: 3ecb2fd2ccf5
...
specified and base type = `prototype`), [run_commands.py::_dispose_concierge_proposal](../../backend/app/api/run_commands.py) calls
[run_commands.py::_classify_revision_tier](../../backend/app/api/run_commands.py) to detect the tier (small / large / feature) and selects the correct
manifest branch (`prototype_revision` / `prototype_large_revision` /
`prototype_feature_revision`) before minting the child run. The full
analysis — the `== REVISION ANALYSIS ==` block that agents consume — is
produced by the manifest's step 0 (`prototype-revision-analyzer`), which carries
`produces_solution_plan: true`. After that step completes, the engine's
`produces_solution_plan` post-step hook (in [execution_engine/engine.py](../../backend/agents/execution_engine/engine.py)) calls
[agents/revision_analyzer.py::parse_analyzer_output](../../backend/app/agents/revision_analyzer.py) to extract the solution and seeds
`ectx.analyzer_solution` so downstream build agents in the same run inherit the
analysis.
...
[run_commands.py::_classify_revision_tier](../../backend/app/api/run_commands.py) — a lightweight one-word LLM call that detects
whether the revision is small / large / feature-scoped before the child run is
minted. The tier determines which manifest is used (`prototype_revision` /
`prototype_large_revision` / `prototype_feature_revision`). The full analysis
(solution plan, rationale) runs later as step 0 of the selected manifest, with
`produces_solution_plan: true` triggering the engine's post-step hook to populate
`ectx.analyzer_solution` (INV-1 compliance: the kernel never reads the tier, only
the solution). This architecture separates concerns: the app layer picks the
manifest by tier, the kernel runs the analyzer and composes its output into
context. SC-001 is satisfied: workflow-type literals (the tier branches) are
app-side, not kernel-side.
