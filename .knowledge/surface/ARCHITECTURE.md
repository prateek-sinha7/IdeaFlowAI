...
| `frontend/src/components` | UI components | 170 | 186 files |
...
Full text: `ctx.py --show <ID>`. Rules by area: `ctx.py --rules <area>`. Areas carrying history: `agents` (189), `workflow` (180), `frontend` (155), `sse` (125), `artifacts` (78), `backend` (74), `auth` (71), `resume` (40).
...
- Only 1 decision card exists against 224 fixes and bugs. Most rules this project actually follows are still implicit in fix prose — run `knowledge-consolidate` to promote them.
