...
| `frontend/src/app` | Next.js routes | 46 | 22 files |
| `frontend/src/components` | UI components | 180 | 189 files |
| `frontend/src/hooks` | client state + stream handling | 22 | 22 files |
...
Full text: `ctx.py --show <ID>`. Rules by area: `ctx.py --rules <area>`. Areas carrying history: `agents` (191), `workflow` (183), `frontend` (160), `sse` (126), `artifacts` (78), `backend` (74), `auth` (71), `resume` (40).
...
- Only 1 decision card exists against 229 fixes and bugs. Most rules this project actually follows are still implicit in fix prose — run `knowledge-consolidate` to promote them.
