...
|---|---|---:|---|
| `backend/app/api` | HTTP + SSE surface — the only caller of the kernel | 34 | 27 files |
...
Full text: `ctx.py --show <ID>`. Rules by area: `ctx.py --rules <area>`. Areas carrying history: `agents` (187), `workflow` (179), `frontend` (153), `sse` (125), `artifacts` (78), `auth` (71), `backend` (70), `resume` (40).
...
- Only 1 decision card exists against 218 fixes and bugs. Most rules this project actually follows are still implicit in fix prose — run `knowledge-consolidate` to promote them.
