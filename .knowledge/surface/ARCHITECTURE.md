...
- Last activity: 2026-08-11 — Completed quick task 260811-mxg: fix update_specs prior-spec injection and resume planning-context rehydration
...
| `backend/agents/execution_engine` | the execution kernel | 26 | 11 files |
...
Full text: `ctx.py --show <ID>`. Rules by area: `ctx.py --rules <area>`. Areas carrying history: `agents` (199), `workflow` (186), `frontend` (170), `sse` (131), `artifacts` (79), `backend` (79), `auth` (73), `resume` (43).
...
- Only 1 decision card exists against 242 fixes and bugs. Most rules this project actually follows are still implicit in fix prose — run `knowledge-consolidate` to promote them.
