...
| `frontend/src/components` | UI components | 173 | 187 files |
...
Full text: `ctx.py --show <ID>`. Rules by area: `ctx.py --rules <area>`. Areas carrying history: `agents` (190), `workflow` (180), `frontend` (157), `sse` (125), `artifacts` (78), `backend` (74), `auth` (71), `resume` (40).
...
- Only 1 decision card exists against 226 fixes and bugs. Most rules this project actually follows are still implicit in fix prose — run `knowledge-consolidate` to promote them.
