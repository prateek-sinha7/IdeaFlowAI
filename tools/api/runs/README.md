...
./run-http.sh user_stories   # or: ppt, prototype, app_builder,
                              ...
                              #     hello_html, sample_subagents,
                              #     sample_subagents_parallel
```

The last three are small, unattended workflows (`planner: skip`,
`clarify: {mode: skip}`) meant for exercising the engine rather than producing
anything useful. `sample_subagents` is the nested-tree shape;
`sample_subagents_parallel` is three independent siblings under one parent,
dispatched concurrently through the kernel fan-out — run it when you want to
check that sibling parallelism still overlaps. Both take an optional topic:

```bash
SUBAGENTS_TOPIC="urban beekeeping" ./run-http.sh sample_subagents_parallel
```
