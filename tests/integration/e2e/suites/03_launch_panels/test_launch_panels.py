...
def test_the_prototype_wizard_offers_three_tabs_and_a_blank_canvas_option(page, shot):
    ...
    # FIX-323 removed the pre-set review gates from the prototype wizard — the
    # `gate: Human_Gate` frontmatter came out of prototype-specify/plan/analyze so
    # nothing fires unless the user opts in. The wizard therefore opens at "no
    # gates", exactly like the simple panel, and the section still has to be THERE
    # and reporting its state — that is what this asserts now.
    expect(page.locator(L.REVIEW_GATES)).to_contain_text(re.compile(r"no gates|pause for review"))
