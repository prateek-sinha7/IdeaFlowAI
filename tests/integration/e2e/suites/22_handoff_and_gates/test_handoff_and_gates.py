...
@pytest.mark.skip(reason=_NEEDS_A_HANDOFF)
...
def test_another_users_handoff_token_is_refused_identically(page, shot):
    """Scenario: Another user's handoff token is refused identically

    Asserted as a SAMENESS: two different unusable tokens must produce the same
    screen. A foreign token cannot be minted here, so the pair compared is two
    invalid ones — which still fails if the route ever starts distinguishing
    cases in its copy.
    """
    screens = []
    for token in ("invalid-token", "00000000-0000-0000-0000-000000000000"):
    ...
    assert screens[0] == screens[1], (
        "two unusable handoff tokens render different screens — the difference "
        "is information about which ones exist"
    )
