...
export default function PPTTemplatesPage() {
  ...
  // PPT pipeline agents — starts from the static defaults, but can be augmented
  // via AgentsPopup (add/remove/reorder). Mirrors the IdeaInputPage pattern.
  ...
  const defaultAgentIds = useMemo(
    () => new Set(LIBRARY_AGENTS.filter((a) => a.pipeline_type === "ppt").map((a) => a.id)),
    [],
  );
  ...
  }, [selectedTemplateId, selectedDsId, dsRequired, brief, customDsBody, customTemplateBody, pipelineAgents]);
  ...
  return (
    <div className="min-h-screen" style={{ background: "#f5f5f0" }}>
          ...
          <section>
            ...
            <div className="mt-3 rounded-2xl border border-gray-200/70 bg-white">
              <textarea
                ...
                placeholder={isListening ? "Listening... speak your idea" : "e.g. A pitch deck for our Series A fundraise — $5M ask, B2B SaaS, 10 slides for investors."}
                rows={5}
      ...
      {showSaveModal && (
        <NameWorkflowModal
          title="Save presentation workflow"
