...
import { parseViewPath, routes, type ParsedView } from "@/lib/routes";
...
import { DashboardLayout, type MainView, type SavedComposition } from "@/components/layout/DashboardLayout";
// SC-001 fix: the settings sub-tab type, seeded from parseViewPath below and
// threaded through DashboardLayout to AccountSettings' own initialSection prop.
...
// T8 (015-frontend-routing, FR-011): the single-saved-workflow fetch —
...
// MainView covers it, so it renders outside DashboardLayout's switch.
import { WorkflowDetailView } from "@/components/savedworkflows/WorkflowDetailView";
// T6 (retry, FR-001/FR-003): rendered directly (not routed to) for
...
const REOPEN_TERMINAL_STATUSES = new Set(["completed", "failed", "cancelled", "degraded", "diverted"]);

// F14 (015-frontend-routing, SC-002/T11 audit fix): how long to wait before
// retrying a 403/404 run-detail fetch that runConnection.liveRunIds does not
// (yet) corroborate — long enough for a just-launched run's backend read-path
// to catch up, short enough not to stall the cold-mount reattach noticeably.
// Module scope (not component-local) so it's not a fresh literal on every
// render — handleSelectWorkflowRun's useCallback below reads it without
// needing it in the deps array.
const RUN_ACCESS_RETRY_DELAY_MS = 1200;

/**
 * T6 (015-frontend-routing, FR-001/FR-003): map a parsed URL screen onto
 * DashboardLayout's coarser `MainView` union — the Route → Screen table in
 * data-model.md is the source of truth. Returns undefined for screens with
 * no MainView equivalent (e.g. `/create/ppt`, which routes to a separate
 * wizard page, or `/workflows/{id}`'s read view, which data-model.md flags
 * as not built yet) — those fall back to DashboardLayout's existing default.
 * Id-bearing screens resolve to the right shell here; T7/T8 populate their
 * data on cold mount.
 */
function initialMainViewFor(parsed: ParsedView): MainView | undefined {
  switch (parsed.screen) {
    case "home":
      return "home";
    case "create":
      // Correction to data-model.md's `/create` → `"catalog"` row: `"catalog"`
      // is a `MainView` union member (`DashboardLayout.tsx:319`) with NO render
      // branch (grepped `mainView === "catalog"` — only the header's
      // `currentPage` highlight reads it, `DashboardLayout.tsx:2265`) and is
      // never `setMainView`'d from anywhere in the app today. The UXFIX-03/D-20
      // redesign (`DashboardLayout.tsx:2444-2471` comment) folded the create
      // catalog into the fused `"home"` screen (`HomeLaunchGrid`, sourced from
      // `GET /api/workflows`) — there is no longer a distinct catalog screen to
      // point at. Mapping to `"catalog"` as data-model.md states would render a
      // blank content pane on cold `/create`, failing this task's own AC1.
      // `"home"` is what actually renders the create-catalog experience today.
      return "home";
    case "create-app":
    ...
    case "workflow":
      // No MainView exists for the saved-workflow read view yet
      // (data-model.md T4) — fall back to the list rather than invent one.
      return "saved-workflows";
    case "library":
    case "library-agent":
    case "library-skill":
...
/**
 * T7 (015-frontend-routing, FR-003/SC-001): the run id carried by a reopened
 * run-detail route — every `execution` (reopened) row in data-model.md's
 * Route → Screen table except `run-stream` (T11's cold-mount live-reattach
 * case, handled separately). Returns undefined for every other screen.
 */
 ...
 * documents `run-stream` as excluded and handled separately.
 */
function liveStreamRunIdFor(parsed: ParsedView): string | undefined {
  return parsed.screen === "run-stream" ? parsed.runId : undefined;
}
...
/**
 * SC-001 fix: which `AccountSettings` tab a cold-opened `/settings/*` route
 * should land on. `parseViewPath` already distinguishes all four settings
 * sub-paths; this maps that screen onto `AccountSettings`' own `SettingsSection`
 * union so the distinction isn't discarded before reaching the component.
 * Returns undefined for every other screen (AccountSettings' own default,
 * "profile", applies unchanged).
 */
function initialSettingsSectionFor(parsed: ParsedView): SettingsSection | undefined {
  switch (parsed.screen) {
    case "settings-profile":
      return "profile";
    case "settings-ai-model":
...
/**
 * T8 (015-frontend-routing, FR-011): the saved-workflow id carried by
 * `/workflows/{id}/edit` — the only screen the Composer's cold-mount fetch
 * covers. `workflow-run` (a different screen, `handleLaunchSaved`'s own
 * click-driven route) is out of scope here.
 */
function workflowEditIdFor(parsed: ParsedView): string | undefined {
  return parsed.screen === "workflow-edit" ? parsed.workflowId : undefined;
}

/**
 * T30 (015-frontend-routing, FR-011): the saved-workflow id carried by
 * `/workflows/{id}` — the read-only view's own screen, distinct from both
 * `workflowEditIdFor` (Composer) and `workflowRunIdFor` (launch panel).
 */
function workflowDetailIdFor(parsed: ParsedView): string | undefined {
  return parsed.screen === "workflow" ? parsed.workflowId : undefined;
}

/**
 * T6 (015-frontend-routing, FR-001/FR-003, data-model.md T4 findings):
 * `/create/ppt` and `/create/prototype` don't map to any `MainView` — a
 * click-driven open of these never touches `DashboardLayout` at all
 * (`HomeLaunchGrid.handleClick` → `selectWorkflowWizardPath`,
 * `globalSlice.ts:224-230`), it `router.push`es straight to the standalone
 * `/workflow/create` wizard page. Returns the wizard `mode` for a cold mount
 * of either path, undefined for every other screen (including the plain
...
/**
 * T6 (015-frontend-routing, FR-001/FR-003): the saved-workflow id carried by
 * `/workflows/{id}/run` — distinct from `workflowEditIdFor` above (a
 * different screen/action). The cold-mount effect below fetches this id to
 * learn `base_pipeline_type`: the ppt/prototype branch redirects to the
 * wizard (no `MainView`); every other type resolves to "composer"/"input"
 * via `workflowRunMainView`, mirroring `handleLaunchSaved`
...
export default function DashboardPage({
  ...
  // own `savedComposition` state (the fetch is async and resolves AFTER
  // DashboardLayout has already mounted, so a useState initializer there —
  // T6's `initialMainView` pattern — would silently miss it). T6 (retry,
  // data-model.md's `/workflows/{id}/run` row) reuses this SAME state for
  // the non-ppt/prototype branch of `/workflows/{id}/run` — the two screens
  // are mutually exclusive by URL, so there is no collision.
  const [initialSavedComposition, setInitialSavedComposition] = useState<SavedComposition | null>(null);
  // T6 (retry, FR-001/FR-003, data-model.md's `/workflows/{id}/run` row):
  // the Composer/Input `MainView` resolved for the non-ppt/prototype branch
  // of `/workflows/{id}/run` (`custom` → "composer", everything else →
  // "input" — `handleLaunchSaved`, `DashboardLayout.tsx:1433-1439`). Unlike
  // every other screen, this can't be read synchronously off the URL in
  // `initialMainViewFor` — it depends on the fetched `base_pipeline_type` —
  // so it's resolved here and merged into the `initialMainView` prop below.
  const [workflowRunMainView, setWorkflowRunMainView] = useState<MainView | null>(null);
  // T6 (retry, FR-001/FR-003): the wizard mode resolved for the ppt/prototype
  // branch of `/workflows/{id}/run` — the sibling of `workflowRunMainView`
  ...
  // not-found) from the loading gap before the fetch resolves.
  const [workflowDetail, setWorkflowDetail] = useState<UserWorkflowSummary | null>(null);
  const [workflowDetailFailed, setWorkflowDetailFailed] = useState(false);
  // T24 (015-frontend-routing, FR-009): completes T7's stubbed failure branch
  // (and T11's, which shares the same handleSelectWorkflowRun fetch) — a
  // 403/404 on the reopened-run/live-stream getWorkflow() call sets this so
  // the render below shows not-found instead of an empty "execution" shell.
  // Reset on every handleSelectWorkflowRun call alongside the other reopen
  // signals so a stale denial never leaks onto the next run opened.
  const [runAccessDenied, setRunAccessDenied] = useState(false);
  // T24 (015-frontend-routing, FR-009): same as runAccessDenied above, for
  // T8's stubbed failure branch — a 403/404 on the /workflows/{id}/edit
  // cold-mount fetch renders not-found instead of an empty Composer shell.
  const [workflowEditAccessDenied, setWorkflowEditAccessDenied] = useState(false);
  ...
  const handleWebSocketMessage = useCallback((msg: StreamMessage, sourceRunId?: string) => {
    ...
    const pipelineTypes = [
      ...
      "pipeline_cancelled",
      // pipeline_diverted (014-conditional-gates, T38) — was missing entirely, so the
      // frame never reached handleWebSocketMessage's routing below: not the reducer
      // (useWorkflow.ts's existing "pipeline_diverted" case), and not the recentRuns
      // refetch further down. Both now fire once the type is recognized here.
      "pipeline_diverted",
    ...
    if (pipelineTypes.includes(msg.type)) {
      ...
      // T38 (014-conditional-gates, R-20 live case): a trigger:"workflow" route
      // outcome fired — refetch so `recentRuns` picks up the diverting run's new
      // `status: "diverted"` plus the newly-minted target run (`parentRunId`
      // pointing back at it). `recentRuns` is threaded into WorkflowHistory as a
      // prop (DashboardLayout -> WorkflowHistory) purely as a change-signal: an
      // already-open history panel keeps its OWN `runs` state (fetched
      // independently on mount), so WorkflowHistory re-fetches itself when this
      // reference changes — see the `recentRuns`-keyed effect in
      // WorkflowHistory.tsx. Its buildDivertLinks then derives the linked-cards
      // UI from THAT re-fetched state's persisted `status`/`parentRunId` fields
      // (contracts/sse-pipeline-diverted.md). Without this refetch (and that
      // wiring), the live case showed nothing until the panel was closed and
      // reopened. Ungated on isForeignFrame, same as pipeline_complete above:
      // the divert may belong to any tab's run, but recentRuns is global.
      if (msg.type === "pipeline_diverted" && msg.data) {
        const currentToken = getToken();
        if (currentToken) {
          getWorkflows(currentToken, { limit: 50 })
            .then(({ runs }) => setRecentRuns(runs))
  ...
  // F14 (015-frontend-routing, SC-002/T11 audit fix): same ref-sync idiom as
  // detachRunRef above — handleSelectWorkflowRun's retry (below) re-invokes
  // ITSELF via a bare setTimeout closure, which would otherwise keep reading
  // the runConnection snapshot from the render that scheduled the retry (a
  // real closure-staleness gap: a plain `runConnection.liveRunIds` read inside
  // that closure never sees a liveRunIds update that lands DURING the delay).
  // Reading this ref instead always reflects the CURRENT liveRunIds at the
  // moment the retry actually runs.
  const liveRunIdsRef = useRef<string[]>(runConnection.liveRunIds);
  useEffect(() => {
    liveRunIdsRef.current = runConnection.liveRunIds;
  }, [runConnection.liveRunIds]);
  ...
  const handleSelectWorkflowRun = useCallback(
    // F14: `isRetry` bounds the reconciliation retry below to exactly one
    // attempt — undefined/false on every real caller (they all call with one
    // argument), true only on the single recursive retry this function itself
    // issues, so a genuinely inaccessible run still resolves to not-found
    // (FR-009) instead of retrying forever.
    async (run: WorkflowRun, isRetry = false) => {
      ...
      // T24 (FR-009): clear a prior denial before this fetch resolves — same
      // reasoning as the ISS-017 reset above.
      setRunAccessDenied(false);
      ...
      try {
        ...
        // T24 (FR-009): a 403 (not owned) or 404 (deleted/unknown) on the
        // run-detail fetch above is an ownership failure, not a transient
        // error — flag it so the render below shows not-found instead of a
        // silent empty shell. Every other status (network failure, 5xx, and
        // 401 — already redirected to login by lib/api.ts's request(), T22)
        // keeps the prior log-and-fall-through behavior so the two paths
        // never collide.
        if (err instanceof ApiError && (err.status === 403 || err.status === 404)) {
          // F14 (SC-002/T11 audit fix): live-reproduced (Playwright, twice) —
          // a hard refresh on /runs/{id}/stream can hit this 403/404 while the
          // run's SSE stream is genuinely live, because this REST read-path
          // lags the run's propagation to the backend by a beat. Reconcile
          // BEFORE permanently denying access, instead of denying on the
          // first miss:
          //  1. One bounded retry after a short delay — the best outcome,
          //     since a SUCCEEDING retry falls through to this same
          //     function's normal reopen/attach/replay path (full recovery,
          //     not just "don't 404").
          //  2. If the retry ALSO 403/404s, fall back to the live-stream
          //     signal: liveRunIdsRef mirrors runConnection.liveRunIds
          //     (server-derived, independent of this failed fetch —
          //     RunConnectionProvider's own GET /api/runs query, or this
          //     tab's attachRun) but stays FRESH across the retry's
          //     setTimeout delay (see the ref's own comment), so its
          //     presence proves the run is real and owned by this session
          //     even though both REST reads lagged.
          // A run absent from liveRunIds AND still 403/404-ing after the
          // retry falls through to the permanent denial below, so a truly
          // inaccessible/deleted/other-account run still correctly resolves
          // to not-found (FR-009, unregressed).
          if (!isRetry) {
            setTimeout(() => {
              void handleSelectWorkflowRun(run, true);
            }, RUN_ACCESS_RETRY_DELAY_MS);
            return;
          }
          if (liveRunIdsRef.current.includes(run.id)) {
            return;
          }
          setRunAccessDenied(true);
        }
      }
    },
  ...
  // T7 (015-frontend-routing, FR-003/SC-001): "execution" covers both the live
  // stream and every reopened run-detail tab, distinguished by fetched state —
  // T6's initialMainView alone renders the right shell with NO data. On a cold
  // mount of /runs/{id}* (run-stream excluded — T11's job), fetch and populate
  // the SAME state a click-driven history-reopen sets, by reusing
  // handleSelectWorkflowRun itself rather than a second, diverging fetch path.
  // handleSelectWorkflowRun only reads `run.id` before its own getWorkflow()
  // call resolves, so a minimal `{ id: runId }` stub is safe here.
  useEffect(() => {
    if (!isAuthenticated) return;
    const runId = reopenedRunIdFor(parsedView);
    if (!runId) return;
    // Already loaded/loading — a click-driven open (once T9 lands), a live
    // launch, or an earlier cold-mount of this exact run already set
    ...
    // page's own instance across a client-side navigation to a new URL under
    // the same catch-all — confirmed live: a fresh render-instance id appears
    // right after router.push, before trackedRunIdRef.current even changes).
    // The remount wipes the local trackedRunIdRef back to null even though
    // this exact run is already being actively driven by this tab. Without
    // this check, the guard above always misses on the very next render after
    // T9's push, and the redundant handleSelectWorkflowRun refetch below races
    // the backend's own not-yet-committed write for a run created moments ago
    // — a transient 403/404 that setRunAccessDenied()/notFound() (T24) then
    // treats as a genuine ownership failure. `runConnection` is owned by
    // RunConnectionProvider, mounted ABOVE the router in app/layout.tsx (see
    // that file's own doc comment), so it — unlike this page's local refs —
    // survives the remount; `liveRunIds` already includes this run because
    // attachRun() (called synchronously at launch/reopen time) ran long before
    // this remount. Re-seed the refs to what handleSelectWorkflowRun's own
    // synchronous prelude would have set, and skip the redundant fetch:
    // subsequent live frames for this run rebuild pipelineState as they arrive.
    ...
    // T7 fix-retry: request the tab AFTER the fetch/replay settles, not before
    // it starts, so the deep-link nonce doesn't mint before this fetch's own
    // data (reopenedRunStatus/pipelineState → headerRunState) even exists.
    // NOTE (T7 2nd retry): this `.finally()` positioning alone does NOT decide
    // which tab wins — it only changes when the deep-link nonce updates
    // relative to this fetch's own setState calls, and both can still land in
    // the SAME React commit (this whole promise chain runs as one uninterrupted
    // microtask flush once the fetch resolves). Confirmed live: for a COMPLETED
    // run this landed the nonce bump in the same commit as headerRunState's
    // flip to "complete", and PreviewPanel's state-keyed default-tab effect
    // (declared after the deep-link consumer) still won, stomping back to
    // "preview" on /steps, /files, /audit. The actual fix — making deep-link
    // precedence explicit and commit-order-independent — lives in PreviewPanel
    // itself (PreviewPanel.tsx, the merged deep-link/default-tab effect, see
    // its comment). This `.finally()` positioning is still correct/desirable
    // (the deep link should target data that already exists), it just isn't
    // what makes the race resolve correctly by itself.
  ...
  // T11 (015-frontend-routing, FR-003/SC-002): re-attach the live stream on a
  // COLD MOUNT of /runs/{id}/stream — a hard refresh must re-open the run's
  // SSE connection from the URL alone, not only after a client-side switch
  // (handleSwitchToLiveRun) or a fresh launch. Reuses handleSelectWorkflowRun
  // for the same reason T7's effect above does (single fetch path, INV-12):
  // it fetches the run, attaches the live SSE stream only when the run is
  // non-terminal (the REOPEN_TERMINAL_STATUSES gate inside it — no dead-stream
  // reconnect loop), and replays durable events so a run that finished before
  // the refresh renders its terminal state in place instead of a dead
  // connection.
  useEffect(() => {
    if (!isAuthenticated) return;
    const runId = liveStreamRunIdFor(parsedView);
    if (!runId) return;
    // Already tracked — a client-side switch to this exact run
    // (handleSwitchToLiveRun, a fresh launch, or an earlier cold mount of this
    // same URL) already attached it synchronously before/while the URL
    // changed, so re-running here would double-attach the SSE connection.
    if (runId === trackedRunIdRef.current) return;

    // T6 (retry — V2 attempt 8 regression, FR-001/FR-003): same remount race
    // as T7's effect above (see its comment for the full trace) — T9's
    // router.push to routes.runStream() remounts this page, resetting the
    // local trackedRunIdRef even for the run this tab is actively streaming.
    // `runConnection.liveRunIds` lives in RunConnectionProvider, mounted above
    // the router, so it survives the remount and already lists this run (its
    // attachRun() fired synchronously at launch, well before this remount).
    // Re-seed the refs and skip the redundant getWorkflow() refetch, which
    // would otherwise race the backend's not-yet-committed write for a run
    // created moments ago and misfire T24's ownership-denial notFound().
    if (runConnection.liveRunIds.includes(runId)) {
      activelyBuildingRunIdRef.current = runId;
      trackedRunIdRef.current = runId;
      ...
      // is gone, and useWorkflow's agent_start case is a no-op for any agent
      // id not already in state (useWorkflow.ts's agentIdx === -1 early
      // return), so every agent_start/agent_complete since would otherwise
      // silently vanish — confirmed live: the Steps tab never showed the
      // running agent after a launch-triggered remount. Replay the run's
      // durable event log through the SAME router (handleWebSocketMessage)
      // handleSelectWorkflowRun's own reopen path already uses (below) so
      // pipeline_start's roster — and every frame since — lands back in
      // pipelineState; the live SSE tail (never detached, RunConnectionProvider
      // survives the remount) then continues seamlessly, deduped against the
      // same seenEventIdsRef this replay populates. Deliberately bypasses
      // handleSelectWorkflowRun's own getWorkflow() ownership check: liveRunIds
      // already proves this run belongs to THIS session (attachRun ran at
      // launch time), so a transient failure here is backend read lag, never a
      // real ownership denial — swallow it rather than risk T24's notFound()
      // firing on a run this tab is legitimately driving.
      const currentToken = getToken();
      if (currentToken) {
        void getRunEvents(currentToken, runId)
          .then((durableFrames) => {
            for (const frame of durableFrames) {
              handleWebSocketMessage(
      ...
      // T11 (retry, same remount): the durable replay above restores agents[]
      // but every agent starts back at "idle" until its own agent_start frame
      // streams in — AgentThinkingTab's hasAnyData gate (:109) renders the
      // empty state until EITHER an agent goes non-idle OR runInput
      // (submittedBrief) is set, and submittedBrief is the same page-local
      // useState wiped by the remount. A run just past pipeline_start with no
      // agent progress yet would otherwise show the empty state despite being
      // genuinely live. Rehydrate the brief onStartPipeline persisted at
      // launch time (same file, its .then() above) — synchronous and
      // same-tab-only, so unlike a getWorkflow() REST fetch it cannot 404 on
      // a run whose backend write is still settling.
      try {
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    // T24 (FR-009): clear a prior denial before this fetch resolves — same
    // reasoning as handleSelectWorkflowRun's ISS-017-style reset above.
    setWorkflowEditAccessDenied(false);
    userWorkflowsApi
      .get(workflowId)
      .then((saved) => {
        if (cancelled) return;
        // Mirrors handleLaunchSaved's savedComposition mapping
        // (DashboardLayout.tsx:1359+) minus the ppt/prototype wizard-redirect
        // branches — /workflows/{id}/edit always opens the Composer (T6's
        // initialMainViewFor), never the wizard.
        setInitialSavedComposition({
          agentIds: saved.agent_ids,
          // Takes precedence over agentIds inside ComposerPage (frozen-at-create
          // vs. current — see the initialManifestSteps comment on SavedComposition).
          manifestSteps: saved.manifest?.steps,
          ...
          id: saved.id,
          runConfig: {
            ...(saved.manifest?.deliverable ? { deliverable: saved.manifest.deliverable } : {}),
            ...(saved.manifest?.planner ? { planner: saved.manifest.planner } : {}),
      ...
      .catch((err) => {
        if (cancelled) return;
        console.error("cold-mount workflow fetch failed:", workflowId, err);
        // T24 (FR-009) / T33 sweep: unlike handleSelectWorkflowRun's fetch
        // (lib/api.ts's request()), userWorkflowsApi.get() goes through
        // store/api/http.ts's axios pipeline, whose interceptor already calls
        // the shared handleSessionExpiry() on 401 (T22). This mirrors that
        // same call rather than reimplementing the clear+redirect inline.
        if (err instanceof ApiError && err.status === 401) {
          handleSessionExpiry();
          return;
        }
        if (err instanceof ApiError && (err.status === 403 || err.status === 404)) {
          setWorkflowEditAccessDenied(true);
        }
      });
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [viewPathKey, isAuthenticated]);

  // T30 (015-frontend-routing, FR-011): fetch the saved workflow for the
  // standalone read view on a cold (or client-side) mount of
  // `/workflows/{id}`. Unlike T7/T8's stubbed failure branches (T24 fills
  // those in later), this screen's own AC requires the real not-found
  // outcome now — a 403/404 sets workflowDetailFailed, rendered as
  // `notFound()` below rather than deferred.
  useEffect(() => {
    if (!isAuthenticated) return;
    const workflowId = workflowDetailIdFor(parsedView);
    if (!workflowId) return;
    ...
    userWorkflowsApi
      ...
      .then((saved) => {
        if (cancelled) return;
        setWorkflowDetail(saved);
        setWorkflowDetailFailed(false);
      })
      .catch((err) => {
        if (cancelled) return;
        console.error("workflow detail fetch failed:", workflowId, err);
        // T24 (FR-009) / T33 sweep: same userWorkflowsApi.get()/axios gap as
        // the /workflows/{id}/edit catch above — its interceptor already
        // calls the shared handleSessionExpiry() on 401 (T22); mirror that
        // call here too rather than falling into workflowDetailFailed (which
        // is reserved for 403/404/not-found).
        if (err instanceof ApiError && err.status === 401) {
          handleSessionExpiry();
          return;
        }
        // F7 (015-frontend-routing, FR-009 audit fix): narrowed to match the
        // run-detail (line ~2807) and /workflows/{id}/edit (line ~3093) catch
        // blocks above — only a genuine 403 (not owned) or 404 (deleted/
        // unknown) is an ownership failure. Every other error (5xx, network
        // blip) now falls through to the log-and-continue behavior (the
        // console.error above) instead of unconditionally rendering
        // not-found for a transient failure on a legitimately-owned,
        // existing workflow.
        if (err instanceof ApiError && (err.status === 403 || err.status === 404)) {
          setWorkflowDetail(null);
          setWorkflowDetailFailed(true);
        }
      });
  ...
  // findings): a cold mount of `/create/ppt` or `/create/prototype` never
  // sets a `MainView` (both are undefined in `initialMainViewFor` above).
  // The original plan here was `router.push(routes.workflowCreateLegacy(mode))`
  // — mirroring the click-driven `HomeLaunchGrid.handleClick` path — but that
  // target is a dead end: next.config.ts's T17 redirect sends
  // `/workflow/create?mode=X` straight back to `/create/{mode}`, which would
  // re-trigger this same cold mount and bounce forever (confirmed live by
  // the validator, 3/3 and 8/8 reproductions). `/create/{mode}` IS that
  // redirect's own destination, so `LaunchWizard` is rendered directly there
  // instead — see the `wizardMode` early-return a few hundred lines down,
  // right after the T30 `workflow` read-view return. No effect is needed for
  // this branch: the mode is derivable synchronously from `parsedView`.

  // T6 (015-frontend-routing, FR-001/FR-003, data-model.md T4 findings): a
  // cold mount of `/workflows/{id}/run` must first fetch the saved workflow
  // to learn `base_pipeline_type` (the click-driven path, `handleLaunchSaved`
  // inside `DashboardLayout`, already has it in hand from the list it was
  // opened from). Two outcomes, both mirrored from `handleLaunchSaved`
  // (`DashboardLayout.tsx:1324-1441`, not reachable from here, so its logic
  // is reproduced inline rather than imported):
  //   - ppt/prototype (`:1356-1408`): no `MainView` at all — stage the same
  //     sessionStorage draft and resolve `workflowRunWizardMode`, which the
  //     `wizardMode` early-return below (not a `router.push`, see that
  //     effect's comment above) renders directly as `LaunchWizard`.
  //   - everything else (`:1410-1439`, data-model.md's `/workflows/{id}/run`
  //     row): `custom` → "composer", every other type → "input", with the
  //     SAME `SavedComposition` shape T8 already threads through as
  //     `initialSavedComposition` for `/workflows/{id}/edit`. The resolved
  //     `MainView` can't live in `initialMainViewFor` (synchronous, URL-only)
  //     since it depends on this fetch, so it's carried in
  //     `workflowRunMainView` and merged into the `initialMainView` prop
  //     below instead.
  //
  // RETRY NOTE (validator failure, PASS criterion 1): the prior pass only
  // implemented the ppt/prototype branch and left every other
  // `base_pipeline_type` as a bare `return` — cold-opening `/workflows/{id}/run`
  // for a `custom` (or any non-ppt/prototype) saved workflow rendered the
  // plain Home screen instead of the pre-filled launch panel. This effect now
  // covers that branch too.
  useEffect(() => {
    if (!isAuthenticated) return;
    const workflowId = workflowRunIdFor(parsedView);
    if (!workflowId) return;
    ...
    userWorkflowsApi
      ...
      .then((saved) => {
        if (cancelled) return;
        if (saved.base_pipeline_type === "ppt" || saved.base_pipeline_type === "prototype") {
          // Mirrors handleLaunchSaved's draft construction
          // (DashboardLayout.tsx:1357-1377 / 1385-1404) exactly, including the
          // id/name/description fields (ISS-167 — carries the saved row's
          // identity through so re-saving updates it in place).
          ...
          const draft: Record<string, unknown> = {
            ...
            ...(saved.model_overrides && Object.keys(saved.model_overrides).length > 0
              ? { modelOverrides: saved.model_overrides }
              : {}),
          ...
          sessionStorage.setItem(`${saved.base_pipeline_type}.draft`, JSON.stringify(draft));
          setWorkflowRunWizardMode(saved.base_pipeline_type);
          return;
        }

        // Mirrors handleLaunchSaved's non-wizard branch
        // (DashboardLayout.tsx:1410-1439) exactly — same SavedComposition
        // shape T8's /workflows/{id}/edit cold mount already populates, plus
        // the composer-vs-input choice (`COMPOSER_ENABLED_TYPES` is a
        // DashboardLayout-private constant containing only "custom" today —
        // `DashboardLayout.tsx:330-332` — reproduced as a direct check here
        // for the same "not reachable from here" reason as the wizard draft
        // above).
        setInitialSavedComposition({
          agentIds: saved.agent_ids,
          manifestSteps: saved.manifest?.steps,
      ...
      .catch((err) => {
        if (cancelled) return;
        console.error("cold-mount workflow-run fetch failed:", workflowId, err);
        // T24 (FR-009) / T33 sweep: same userWorkflowsApi.get()/axios gap as
  ...
  // T6 (015-frontend-routing, FR-001/FR-003): an unrecognized path falls
  // through to Next's not-found rather than a blank/home render.
  if (parsedView.screen === "unknown") {
    notFound();
  }

  // T24 (015-frontend-routing, FR-009): completes T7's (and, via the same
  // handleSelectWorkflowRun fetch, T11's) stubbed failure branch — a
  // 403/404 on the reopened-run/live-stream fetch renders not-found instead
  // of the empty "execution" shell the stub left in place. A 401 never sets
  // runAccessDenied (lib/api.ts's request() already redirected to
  // routes.login() before this render, T22), so the two paths don't collide.
  if ((reopenedRunIdFor(parsedView) || liveStreamRunIdFor(parsedView)) && runAccessDenied) {
    notFound();
  }

  // T24 (015-frontend-routing, FR-009): completes T8's stubbed failure
  ...
  // T30 (015-frontend-routing, FR-011): `/workflows/{id}` read view — no
  // existing MainView covers it (data-model.md T4), so it renders standalone
  // rather than through DashboardLayout below. A fetch failure (unknown/
  // inaccessible id) falls through to not-found; while the fetch is in
  // flight this renders nothing (same pattern as the mounted/auth gate above).
  if (parsedView.screen === "workflow") {
    if (workflowDetailFailed) {
      notFound();
    }
    if (!workflowDetail) {
      return null;
    }
    return <WorkflowDetailView workflow={workflowDetail} />;
  }

  // T6 (retry, 015-frontend-routing, FR-001/FR-003, data-model.md T4
  // findings): `/create/ppt`, `/create/prototype`, and the resolved
  // ppt/prototype branch of `/workflows/{id}/run` render `LaunchWizard`
  // directly, same standalone-outside-DashboardLayout pattern as the T30
  // block above — see the two effects near the top of this component for
  // why this is a render-time return rather than a `router.push` to the
  // legacy `/workflow/create?mode=` page. `workflowRunWizardMode` is `null`
  // (not yet resolved, or a non-wizard type) for every other `workflow-run`
  // id, in which case this falls through to DashboardLayout below exactly
  // as it did before this branch existed.
  const wizardMode =
    wizardModeFor(parsedView) ??
  ...
  return (
    <DashboardLayout
      ...
      // T6 (retry, FR-001/FR-003): `initialMainViewFor` alone can't resolve
      // `/workflows/{id}/run`'s non-ppt/prototype branch (it depends on the
      // fetched base_pipeline_type — see `workflowRunMainView` above), so
      // it's merged in here, scoped to that exact screen so a stale value
      ...
      onStartPipeline={(type, message, agentIds, attachedHooks, extraParams) => {
        ...
        void Promise.resolve(
          ...
          if (launchedRunId) {
            ...
            persistLaunchedIds();
            // T11 (015-frontend-routing, FR-003/SC-002): persist this launch's
            // brief keyed by run id, same same-tab sessionStorage channel as
            // launchedRunIdsRef above. T9's router.push to routes.runStream()
            // remounts this page (see the cold-mount stream re-attach effect
            // below), wiping the page-local submittedBrief useState set just
            // above. Unlike a history reopen, a REST re-fetch of a run this
            // fresh can genuinely 404 (the backend write may still be
            // settling), so the remount can't safely re-derive it that way —
