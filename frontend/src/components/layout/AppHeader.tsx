...
export function AppHeader({ currentPage, onNavigate, onLogout, userEmail }: AppHeaderProps) {
  ...
  return (
    <header className="flex items-center justify-between px-6 py-3 border-b border-[#e8e6dc] bg-white/80 backdrop-blur-md z-40 relative">
      {/* Left — Logo */}
      <div className="flex items-center gap-3">
        <div className="flex h-8 w-8 items-center justify-center rounded-xl bg-gradient-to-br from-[#c96442] to-[#d97757] shadow-sm">
          <Sparkles className="h-4 w-4 text-white" />
        </div>
        <span className="text-sm font-bold text-[#141413] tracking-tight">
          IdeaFlow <span className="text-[#87867f] font-medium">AI</span>
        </span>
      </div>

      {/* Center — Navigation */}
      <nav className="flex items-center gap-1 rounded-xl bg-[#f0eee6] border border-[#e8e6dc] p-1">
        <button
