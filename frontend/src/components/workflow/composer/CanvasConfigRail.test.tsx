...
describe("CanvasConfigRail — 51-07 fan-out control (FANOUT-01)", () => {
  ...
  it("disables the fan-out toggle for the first agent (no upstream) with a hint", () => {
  ...
  it("renders a non-blocking warning when the chosen source is not a known producer", () => {
    renderRail({
      selection: {
        strategy: "fanout_batch",
  ...
  it("no warning renders when the chosen source IS a known producer", () => {
    renderRail({
      selection: {
        strategy: "fanout_batch",
        task_source: { kind: "parsed", parser: "heading_tasks", source_step: "prototype-plan" },
