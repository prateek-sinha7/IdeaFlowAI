...
describe("ComposerPage — manifest/deliverable persistence (BUG-20260828-005700-workflows-ppt-canvas)", () => {
  ...
  it.fails("ISS-196: Save as copy on the ppt built-in persists the real ppt deliverable, not the streamed_text default", async () => {
    renderComposer({
  ...
  // remount key).
  it.fails("ISS-206: runConfig resyncs when initialRunConfig arrives after first mount, not just at mount", async () => {
  ...
  it.fails("ISS-203: Save as copy on the ppt_v2 built-in persists its single_file/presentation.html deliverable", async () => {
    renderComposer({
  ...
  it.fails("ISS-204: a from-scratch save of all-stock agents persists a Workflow-tab deliverable change (no built-in involved)", async () => {
    renderComposer({
  ...
  it.fails("ISS-205: Run once dispatches with the built-in's real deliverable after a pure reorder", async () => {
    const onRun = vi.fn();
    const { rerender } = renderComposer({
      onRun,
      builtinCanvasType: "ppt",
