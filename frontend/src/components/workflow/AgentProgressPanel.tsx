          ...
          {agents.map((agent, idx) => {
            ...
            return (
              <motion.div key={agent.id} initial={{ opacity: 0, x: -8 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: idx * 0.02 }}
                className={`flex items-center gap-3 rounded-lg px-3 py-2.5 transition-all ${isActive ? "bg-blue-50 border border-blue-100" : "border border-transparent"}`}>
                <StatusIcon className={`h-4 w-4 flex-shrink-0 ${statusColor} ${isActive ? "animate-spin" : ""}`} />
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2">
                    <span className="text-sm">{agent.icon}</span>
                    <span className={`text-[12px] font-medium ${agent.status === "idle" ? "text-[#87867f]" : "text-[#141413]"}`}>{agent.name}</span>
                  </div>
