...
export function PPTTemplateGallery({
  ...
  return (
    <div className="rounded-2xl border border-line-border bg-surface-white overflow-hidden">
      {/* Detail modal */}
      {detailTemplate && (
            ...
            {CATEGORIES.map((cat) => (
              <button
                ...
                className={`flex-shrink-0 flex items-center gap-1.5 rounded-t-lg px-3.5 py-1.5 text-[12px] font-medium transition-colors whitespace-nowrap ${
                  activeCategory === cat ? "bg-surface-warm text-ink-900 font-semibold" : "text-ink-500 hover:text-ink-800"
                }`}
              >
                {cat}
                {cat === "Custom" && savedCustomTemplates.length > 0 && (
                  <span className={`inline-flex h-4 min-w-[16px] items-center justify-center rounded-full px-1 text-[9px] font-bold ${
                    activeCategory === "Custom" ? "bg-brand text-white" : "bg-surface-warm text-ink-600"
                  }`}>
          ...
          filteredCustom.length === 0 ? (
            <div className="flex h-40 flex-col items-center justify-center gap-3">
              ...
              <div className="text-center">
                ...
                <p className="mt-0.5 text-[11px] text-ink-400">Upload an HTML file or paste a URL</p>
              </div>
              <button type="button" onClick={() => setShowUploadModal(true)}
                className="flex items-center gap-1.5 rounded-lg bg-brand px-3 py-1.5 text-[11px] font-medium text-white hover:bg-surface-near-black transition-colors">
...
function CustomTemplateCard({ ct, selected, onSelect, onDelete }: {
  ...
  return (
    <div className={`group relative flex flex-col overflow-hidden rounded-lg border bg-surface-white text-left transition-all hover:shadow-sm ${
      selected ? "border-brand ring-2 ring-brand/25 shadow-sm" : "border-line-border hover:border-line-control"
    }`}>
      {selected && (
      ...
      <button type="button" onClick={onSelect} className="flex flex-col flex-1 text-left focus:outline-none">
        <div className="flex h-[80px] items-center justify-center bg-gradient-to-br from-brand-fill to-brand-fill">
        ...
        <div className="px-2 py-1.5">
          <span className="block truncate text-[11px] font-medium text-ink-800 leading-tight">{ct.name}</span>
          <span className="text-[9px] uppercase tracking-wide text-ink-400">{ct.source === "file" ? "HTML file" : "From URL"}</span>
        </div>
      </button>
    </div>
  );
...
function CompactPPTCard({ template, selected, onOpenDetail }: {
  ...
  return (
    <button ref={cardRef} type="button" onClick={onOpenDetail}
      ...
      <div className="relative overflow-hidden bg-surface-warm" style={{ height: "80px" }}>
        {thumbnailUrl && shouldMount ? (
          ...
          <div className="flex h-full items-center justify-center bg-gradient-to-br from-surface-warm to-surface-card">
            <span className="text-[9px] text-ink-400 text-center px-2 leading-tight font-medium">{template.name}</span>
          </div>
        )}
      </div>
      <div className="px-2 py-1.5">
        <span className="block truncate text-[11px] font-medium text-ink-800 leading-tight">{template.name}</span>
        {template.mode && (
          <span className="text-[9px] uppercase tracking-wide text-ink-400">{template.mode}</span>
        )}
      </div>
    </button>
  );
...
function PPTTemplateDetailModal({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      {/* Modal */}
      <div className="relative z-10 flex w-full max-w-4xl flex-col overflow-hidden rounded-2xl border border-line-border bg-surface-white shadow-2xl"
        style={{ maxHeight: "90vh" }}>

        {/* Header */}
        <div className="flex items-center justify-between border-b border-line-divider px-5 py-3.5 flex-shrink-0">
          <div className="flex items-center gap-3">
            <div className="flex h-8 w-8 items-center justify-center rounded-lg bg-brand-fill">
              <Presentation className="h-4 w-4 text-brand" />
            </div>
            <div>
              <h2 className="text-[14px] font-semibold text-ink-900">{template.name}</h2>
              {template.description && (
                <p className="text-[11px] text-ink-500 mt-0.5 max-w-md truncate">{template.description}</p>
              )}
            </div>
          </div>
          <div className="flex items-center gap-2">
            {previewUrl && (
              <button
                ...
                className="flex h-7 w-7 items-center justify-center rounded-lg border border-line-border text-ink-400 hover:bg-surface-warm hover:text-ink-700 transition-colors"
              >
        ...
        <div className="flex-1 min-h-0 bg-surface-near-black relative" style={{ aspectRatio: "16/9", maxHeight: "60vh" }}>
          {previewUrl ? (
            <>
              {!previewLoaded && (
                <div className="absolute inset-0 flex items-center justify-center bg-surface-near-black">
        ...
        {/* Footer */}
        <div className="flex items-center justify-between border-t border-line-divider px-5 py-3.5 flex-shrink-0 bg-surface-white">
          <div className="flex items-center gap-3">
            {template.mode && (
              <span className="rounded-full bg-surface-warm px-2.5 py-1 text-[10px] font-medium text-ink-600 uppercase tracking-wide">
                {template.mode}
              </span>
            )}
          ...
          <button
            ...
            className={`flex items-center gap-2 rounded-xl px-5 py-2.5 text-[13px] font-semibold transition-all ${
              isSelected
                ...
                : "bg-brand text-white hover:bg-surface-near-black"
            }`}
          >
            {isSelected ? (
