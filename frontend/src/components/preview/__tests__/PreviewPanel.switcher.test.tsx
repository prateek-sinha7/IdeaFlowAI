...
describe("PreviewPanel — typed-renderer switcher (plan 07, manual override over generic dispatch)", () => {
  ...
  it("OVERRIDE: selecting a typed renderer forces it over the auto-selected one (markdown auto → HTML override renders the iframe)", () => {
    ...
    const switcher = screen.getByTestId("renderer-switcher");
    fireEvent.change(switcher, { target: { value: "html" } });
  ...
  it("CLEAR: returning the switcher to Auto restores the generic auto-dispatch", () => {
    ...
    fireEvent.change(switcher, { target: { value: "auto" } });
    expect(screen.getByTestId("markdown-preview")).toBeInTheDocument();
    expect(container.querySelector("iframe")).toBeNull();
  });
  ...
  it("SC-001: the switcher option VALUES are generic renderType/mimetype tokens, never a workflow name", () => {
    ...
    const switcher = screen.getByTestId("renderer-switcher") as HTMLSelectElement;
    const values = Array.from(switcher.options).map((o) => o.value);
    // Every option is a structural renderType / mimetype token.
    const ALLOWED = new Set(["auto", "html", "markdown", "zip", "user_stories", "ppt", "prototype", "app_builder"]);
    ...
    for (const bad of FORBIDDEN) expect(values).not.toContain(bad);
  });
});
