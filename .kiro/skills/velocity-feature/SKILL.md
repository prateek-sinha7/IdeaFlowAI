...
Build the folder path `.planning/quick/<YYMMDD>-<id>-<slug>`:
- `<YYMMDD>` — today's date.
...
- `<slug>` — a short kebab-case slug of the feature.

Create the folder and its `PLAN.md` with your file-writing tool directly — don't assume a
specific shell or scripting language is available.

### If full phase — create the folder and reserve the number

List `.planning/phases/` and find the highest existing two-digit phase number; reserve the next
one, zero-padded (e.g. `07` → `08`). Build the folder path `.planning/phases/<NEXT>-<slug>` (same
slug convention as above) and create it with your file-writing tool.
...
```
uv run pytest tests/agents/ -v     # new agent/registry/guardrail work
uv run pytest tests/unit/ -v       # engine/API/manifest work
```
